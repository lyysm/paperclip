  ...
  return (
    <div>
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
        {title ?? t("dashboard.agentsPanelTitle", { defaultValue: "Agents" })}
      </h3>
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">
            {emptyMessage ?? t("dashboard.noRecentRuns", { defaultValue: "No recent agent runs." })}
          </p>
        </div>
      ) : (
      ...
      {showMoreLink && hiddenRunCount > 0 && (
        <div className="mt-3 flex justify-end text-xs text-muted-foreground">
          <Link to="/dashboard/live" className="hover:text-foreground hover:underline">
            {t("dashboard.moreActiveRuns", {
              defaultValue: "{{count}} more active/recent runs",
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
              <span>{isActive
                ? t("dashboard.liveNow", { defaultValue: "Live now" })
                : run.finishedAt
                  ? t("dashboard.finishedAt", { defaultValue: "Finished {{time}}", time: relativeTime(run.finishedAt) })
                  : t("dashboard.startedAt", { defaultValue: "Started {{time}}", time: relativeTime(run.createdAt) })}</span>
            </div>
          </div>
