...
function CollapsedFeedGroup({
  ...
  const actorName = actor?.name
    ?? (group.latestEvent.actorType === "system" ? t("activity.system", { defaultValue: "System" })
      : group.latestEvent.actorType === "user" ? t("activity.board", { defaultValue: "Board" })
      : t("activity.unknownActor", { defaultValue: "Unknown" }));
    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          ...
          <span data-fc="verb" className="ml-1 text-(--hex-959596)">{t("activity.madeUpdatesTo", { defaultValue: "made {{n}} updates to", n: group.events.length })}</span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (agents.length === 0) {
      return {
        text: t("activity.emptyNoAgents", { defaultValue: "No agents set up yet. Add an agent to get started." }),
        showPulse: false,
      };
    }
    ...
    if (allPaused) {
      return {
        text: t("activity.emptyAllPaused", { defaultValue: "All agents are paused. Resume agents from the sidebar to see activity." }),
        showPulse: false,
      };
    }
    return {
      text: t("activity.emptyRunning", { defaultValue: "Your agents are running — activity will appear here shortly." }),
      showPulse: true,
    };
  }, [agents, t]);
    ...
    <aside className={cn("flex min-h-0 min-w-0 flex-1 flex-col bg-background", className)}>
      ...
      <div className="relative flex shrink-0 items-start justify-between gap-2 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold">{t("activity.feedTitle", { defaultValue: "Agent Feed" })}</h3>
          <p className="text-xs text-muted-foreground">
            {t("activity.feedSubtitle", { defaultValue: "Live activity from your agents" })}
          </p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t("activity.groupByTaskAria", { defaultValue: "group by task" })}
          ...
          <DropdownMenu>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("activity.filterByAria", { defaultValue: "filter by" })}
                  >
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              >
                {FILTER_OPTIONS.map(({ value, label }) => (
                  <DropdownMenuRadioItem key={value} value={value}>
                    {t(`activity.filter.${value}`, { defaultValue: label })}
                  </DropdownMenuRadioItem>
                ))}
              </DropdownMenuRadioGroup>
              <DropdownMenuSeparator />
              ...
              >
                {t("activity.showAllActivity", { defaultValue: "Show all activity" })}
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-(--sz-16rem)">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("activity.emptyFallback", { defaultValue: "Activity from your agents will appear here." })}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (
