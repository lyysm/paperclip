...
export function ActivityRow({ event, agentMap, userProfileMap, entityNameMap, entityTitleMap, className }: ActivityRowProps) {
  ...
  const actorName = actor?.name ?? (event.actorType === "system"
    ? t("activity.system", { defaultValue: "System" })
    : userProfile?.label ?? (event.actorType === "user"
      ? t("activity.board", { defaultValue: "Board" })
      : event.actorId || t("activity.unknownActor", { defaultValue: "Unknown" })));
