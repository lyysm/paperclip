...
export function RunButton({
  ...
  const resolvedLabel = label ?? t("agentActions.runNow", { defaultValue: "Run now" });
  return (
  ...
  if (isPaused) {
    return (
      <Button variant="outline" size={size} onClick={onResume} disabled={disabled}>
        <Play className="h-3.5 w-3.5 sm:mr-1" />
        <span className="hidden sm:inline">{t("agentDetail.resume", { defaultValue: "Resume" })}</span>
      </Button>
    );
  }
...
export function AgentActionButtons({
  ...
  const reportError = useCallback(
    (message: string) => {
      if (onActionError) {
        ...
        pushToast({ title: t("agentDetail.actionFailed", { defaultValue: "Action failed" }), body: message, tone: "error" });
      }
    },
  ...
  const agentAction = useMutation({
    ...
    onError: (err) => {
      reportError(err instanceof Error ? err.message : t("agentDetail.actionFailed", { defaultValue: "Action failed" }));
    },
  });
  ...
  const duplicateAgent = useMutation({
    mutationFn: async () => {
      if (!resolvedCompanyId) {
        throw new Error(t("agentActions.agentNotReadyToDuplicate", { defaultValue: "Agent is not ready to duplicate" }));
      }
    ...
    onSuccess: async (createdAgent) => {
      ...
      pushToast({
        title: t("agentActions.agentDuplicatedTitle", { defaultValue: "Agent duplicated" }),
    ...
    onError: (err) => {
      const message =
        err instanceof Error
          ? err.message
          : t("agentActions.failedToDuplicateAgent", { defaultValue: "Failed to duplicate agent" });
      onActionError?.(message);
      pushToast({
        title: t("agentActions.couldNotDuplicateAgent", { defaultValue: "Could not duplicate agent" }),
        body: message,
        tone: "error",
      });
    },
  ...
  const handleDuplicateAgent = useCallback(() => {
    ...
    const confirmed = window.confirm(
      t("agentActions.duplicateConfirm", {
  ...
  const resetTaskSession = useMutation({
    ...
    onError: (err) => {
      reportError(
        err instanceof Error ? err.message : t("agentActions.failedToResetSession", { defaultValue: "Failed to reset session" }),
      );
    },
  });
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={() => agentAction.mutate("pause")}>
                {t("agentActions.pauseAnyway", { defaultValue: "Pause anyway" })}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
          ...
          <button
            ...
            onClick={() => {
              void copyTextToClipboard(agent.id).catch(() => {
                pushToast({
                  title: t("agentDetail.copyFailed", { defaultValue: "Copy failed" }),
                  body: t("agentDetail.clipboardUnavailable", { defaultValue: "Clipboard access is unavailable." }),
                  tone: "error",
                });
              });
              setMoreOpen(false);
