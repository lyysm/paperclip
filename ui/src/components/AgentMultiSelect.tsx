...
export function AgentSelect({
  ...
  const placeholderText = placeholder ?? t("agentMultiSelect.selectAgent");
  const emptyMessageText = emptyMessage ?? t("agentMultiSelect.noAgentsYet");
      ...
      <PopoverTrigger asChild>
        ...
        >
          <span className={cn("min-w-0 truncate", !selectedAgent && "text-muted-foreground")}>
            {selectedAgent?.name ?? placeholderText}
          </span>
      ...
      <PopoverContent className="w-(--radix-popover-trigger-width) p-0" align="start">
        <div className="border-b border-border p-3">
          <Input
            ...
            placeholder={t("agentMultiSelect.filterAgents")}
            className="h-8"
            autoFocus
          />
        </div>
        {agents.length === 0 ? (
          <div className="px-3 py-4 text-sm text-muted-foreground">{emptyMessageText}</div>
        ) : (
          <div className="max-h-60 overflow-y-auto py-1">
            {filteredAgents.map((agent) => (
              <button
                ...
                aria-label={t("agentMultiSelect.selectAgentAria", { name: agent.name })}
                onClick={() => {
                  onChange(agent.id);
                  setOpen(false);
                }}
    ...
    <div className="space-y-2">
      ...
      >
            ...
            <span className="flex min-w-0 items-center">
              ...
              <span className="truncate">
                {triggerLabel ?? (selectedCount === 0
                  ...
                  : t("agentMultiSelect.selectedCount", { count: selectedCount }))}
              </span>
            </span>
            <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" />
          </Button>
        ...
        {loading ? (
          ...
          <div className="px-3 py-4 text-sm text-muted-foreground">
            {emptyMessage ?? t("agentMultiSelect.noAgentsYet")}
          </div>
        ) : (
          <div className="max-h-60 overflow-y-auto py-1">
            {filteredAgents.map((agent) => {
              ...
              return (
                ...
                >
                  <Checkbox
                    ...
                    aria-label={t("agentMultiSelect.allowAgent", { name: agent.name })}
          ...
          <div className="flex items-center justify-between border-t border-border px-3 py-2">
            <span className="text-xs text-muted-foreground">
              {workingAgentIds.size === 0
                ? t("agentMultiSelect.noAgentsSelected")
                : t("agentMultiSelect.selectedCountPlain", { count: workingAgentIds.size })}
            </span>
            <div className="flex items-center gap-2">
              {staged ? (
                <Button type="button" variant="ghost" size="sm" onClick={() => setOpen(false)} disabled={pending}>
                  {t("agentMultiSelect.cancel")}
                </Button>
              ) : null}
              <Button
                type="button"
              ...
              >
                {staged
                  ? pending
                    ? t("agentMultiSelect.saving")
                    : t("agentMultiSelect.save")
                  : t("agentMultiSelect.done")}
              </Button>
            </div>
          </div>
        </PopoverContent>
        ...
        <div className="space-y-0.5">
          ...
          {selectedAgents.length > 3 ? (
            <p className="px-1.5 pt-0.5 text-xs text-muted-foreground">
              {t("agentMultiSelect.andMore", { count: selectedAgents.length - 3 })}
            </p>
          ) : null}
        </div>
      ) : null}
