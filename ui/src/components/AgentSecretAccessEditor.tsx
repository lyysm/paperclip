...
function DeliveryBadge({ mode }: { mode: "env" | "api" }) {
  ...
  if (mode === "env") {
    return (
      ...
      >
        <Variable className="size-3" /> {t("agentConfig.secretAccessEditor.envVar")}
      </Badge>
    );
  }
  return (
    ...
    >
        <ServerCog className="size-3" /> {t("agentConfig.secretAccessEditor.apiAccess")}
    </Badge>
  );
}
  ...
  return (
    <div className="space-y-3">
      ...
      {hasBindings ? (
        ...
        <p className="text-sm text-muted-foreground">{t("agentConfig.secretAccessEditor.noBindings")}</p>
      )}
      ...
      {bindingProposals.length > 0 && onApproveProposal && onRejectProposal ? (
        <div className="space-y-2">
          <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
            {t("agentConfig.secretAccessEditor.proposedAccess")}
          </div>
              ...
              >
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <p className="flex flex-wrap items-center gap-1 text-muted-foreground">
                    <span>{t("agentConfig.secretAccessEditor.proposedBy", { name: proposal.proposedBy.name })}</span>
      ...
      <div className="space-y-2">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
          {t("agentConfig.secretAccessEditor.apiAccessNoEnv")}
        </div>
        {rows.length > 0 ? (
          <div className="space-y-2">
            {rows.map((row) => {
                ...
                <div key={row.id} className="space-y-1">
                  <div className="grid grid-cols-(--gtc-65) items-start gap-1.5">
                    <div>
                      <Input
                        ...
                        placeholder={t("agentConfig.secretAccessEditor.aliasPlaceholder", { defaultValue: "ALIAS" })}
                        aria-label={t("agentConfig.secretAccessEditor.accessAlias")}
                        disabled={disabled}
                        className={cn(
                    ...
                    <div className="flex min-w-0 items-start gap-1.5">
                      ...
                      <select
                        ...
                        aria-label={t("agentConfig.secretAccessEditor.version")}
                      >
                    ...
                    <button
                      ...
                      disabled={disabled}
                      aria-label={t("agentConfig.secretAccessEditor.removeApiAccess")}
                  ...
                  {aliasInvalid ? (
                    <p className="pl-0.5 text-(length:--text-micro) text-destructive">
                      {t("agentConfig.secretAccessEditor.invalidAlias")}
                    </p>
                  ) : aliasDuplicate ? (
                    <p className="pl-0.5 text-(length:--text-micro) text-destructive">{t("agentConfig.secretAccessEditor.duplicateAlias")}</p>
                  ) : null}
                </div>
              );
            })}
        ...
        >
          ...
          {t("agentConfig.secretAccessEditor.addApiAccess")}
        </button>
      </div>

      <p className="text-(length:--text-micro) text-muted-foreground/70">
        {t("agentConfig.secretAccessEditor.apiAccessDescription")}
      </p>
    </div>
  );
}
