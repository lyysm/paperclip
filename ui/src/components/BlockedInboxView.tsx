...
export function BlockedInboxView({
  ...
  if (error) {
    const message =
      error instanceof Error ? error.message : t("blockedInbox.loadFailedTitle", { defaultValue: "Couldn't load the Blocked tab." });
    return (
      <div
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 space-y-1">
            <p className="text-sm font-medium">{t("blockedInbox.loadFailedTitle", { defaultValue: "Couldn't load the Blocked tab." })}</p>
            <p className="text-xs opacity-80">
              {t("blockedInbox.otherTabsWork", { defaultValue: "Other Inbox tabs still work. {{message}}", message })}
            </p>
          </div>
          <Button
            type="button"
            ...
            disabled={isFetching}
          >
            {isFetching ? t("blockedInbox.trying", { defaultValue: "Trying…" }) : t("common.tryAgain", { defaultValue: "Try again" })}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium text-foreground">{t("blockedInbox.emptyTitle", { defaultValue: "No work is stopped." })}</p>
          <p className="text-xs text-muted-foreground">
            {t("blockedInbox.emptyBody", { defaultValue: "Tasks that need a decision, recovery, or external action will appear here." })}
          </p>
        </div>
      </Card>
    );
