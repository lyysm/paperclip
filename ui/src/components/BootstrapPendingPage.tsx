  ...
  return (
    <div className="mt-6 border-t border-border pt-5">
      <div className="flex items-center gap-2 text-sm font-medium">
        ...
        <span>{t("accessGate.preferHost")}</span>
      </div>
      <p className="mt-2 text-sm text-muted-foreground">
        {hasActiveInvite
          ? t("accessGate.activeInviteCommand")
          : t("accessGate.noActiveInviteCommand")}
      </p>
...
function claimErrorCopy(error: BootstrapPendingPageProps["claimError"]) {
  if (error?.status === 409) {
    return {
      title: translate("accessGate.alreadyClaimedTitle"),
      body: translate("accessGate.alreadyClaimedBody"),
    };
  }
  if (error?.status === 401) {
    return {
      title: translate("accessGate.sessionExpired"),
      body: "",
    };
  }
  return {
    title: translate("accessGate.serverUnavailable"),
    body: "",
  };
}
...
export function BootstrapPendingPage({
  ...
  if (!claimAvailable) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">{t("accessGate.waitingForFirstAdmin")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("accessGate.inviteOnlyDescription")}
        </p>
        <CliFallback hasActiveInvite={hasActiveInvite} />
        <p className="mt-4 text-xs text-muted-foreground">
          {t("accessGate.publicClaimDisabled")}
        </p>
      </StateChrome>
    );
  }
    ...
    return (
      <StateChrome>
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold">{t("accessGate.instanceAdmin")}</h1>
            <p className="mt-2 text-sm text-muted-foreground">
              {t("accessGate.setupComplete")}
            </p>
          </div>
        </div>
        <div className="mt-5 flex items-center gap-3">
          <Loader2 className="size-4 animate-spin text-muted-foreground" aria-hidden />
          <span className="text-sm text-muted-foreground">{t("accessGate.redirecting")}</span>
        </div>
        <div className="mt-5">
          <Button asChild variant="outline">
            <a href="/">{t("accessGate.continueDashboard")}</a>
          </Button>
        </div>
      </StateChrome>
    );
  ...
  if (!session) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">{t("accessGate.finishSetup")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("accessGate.signedOutDescription")}
        </p>
        <div className="mt-5">
          <Button asChild>
            <Link to="/auth?next=/">{t("accessGate.signInCreateAccount")}</Link>
          </Button>
        </div>
        <CliFallback hasActiveInvite={hasActiveInvite} />
      </StateChrome>
  ...
  return (
    <StateChrome>
      <h1 className="text-xl font-semibold">{t("accessGate.finishSetup")}</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        {t("accessGate.claimDescription")}
      </p>
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <Button onClick={onClaim} disabled={isClaiming}>
          {isClaiming && <Loader2 className="mr-2 size-4 animate-spin" aria-hidden />}
          {isClaiming ? t("accessGate.claiming") : t("accessGate.claimThisInstance")}
        </Button>
        <span className="text-sm text-muted-foreground">
          {t("accessGate.signedInAs")} <span className="font-medium text-foreground">{displayIdentity(session)}</span>
        </span>
      </div>
      <p className="mt-3 text-xs text-muted-foreground">
        {t("accessGate.wrongAccount")} {" "}
        <Link to="/auth?next=/" className="underline underline-offset-2">
          {t("accessGate.switchAccount")}
        </Link>
        .
      </p>
      {claimError && (
