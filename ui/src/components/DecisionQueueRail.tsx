...
export function DecisionQueueRail({ companyId, activeQueueKey = null }: DecisionQueueRailProps) {
  ...
  return (
    <nav
      ...
      aria-label={t("decisions.queuesAriaLabel", { defaultValue: "Decision queues" })}
      data-decision-queue-rail
    >
      <Chip href={decisionsHref(null)} active={activeQueueKey == null} label={t("decisions.all", { defaultValue: "All" })} />
      {queues.map((queue) => {
    ...
    >
      {recent && (
        <span
          ...
          aria-label={translate("decisions.recentActivityAria", { defaultValue: "Recent activity" })}
        />
      )}
