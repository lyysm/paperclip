...
function severityLabel(severity: string): string {
  switch (severity) {
    case "critical":
      return translate("decisions.severityCritical", { defaultValue: "Critical" });
    case "high":
      return translate("decisions.severityHigh", { defaultValue: "High" });
    case "medium":
      return translate("decisions.severityMedium", { defaultValue: "Medium" });
    case "low":
      return translate("decisions.severityLow", { defaultValue: "Low" });
    default:
      return severity;
  }
}
    ...
    <div className="flex items-center gap-2">
      {visibleCount > 0 && (
        <span className="text-sm text-muted-foreground">
          {visibleCount === 1
            ? t("decisions.decisionCount_one", { defaultValue: "1 decision" })
            : t("decisions.decisionCount_other", { defaultValue: "{{count}} decisions", count: visibleCount })}
        </span>
      )}
      {/* Filter */}
      <Popover>
        <PopoverTrigger asChild>
          <Button
            ...
            title={t("decisions.filterLabel", { defaultValue: "Filter" })}
            aria-label={t("decisions.filterLabel", { defaultValue: "Filter" })}
          >
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            ...
            title={t("decisions.groupLabel", { defaultValue: "Group" })}
            aria-label={t("decisions.groupLabel", { defaultValue: "Group" })}
          >
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            ...
            title={t("decisions.sortLabel", { defaultValue: "Sort" })}
            aria-label={t("decisions.sortLabel", { defaultValue: "Sort" })}
          >
    ...
    <div className="max-h-(--sz-70vh) overflow-y-auto">
      <div className="flex items-center justify-between px-3 py-2">
        <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          {t("decisions.filterLabel", { defaultValue: "Filter" })}
        </span>
        {hasActive && (
          <button
            type="button"
      ...
      {options.sourceKinds.length > 1 && (
        <FilterSection title={t("decisions.filterType", { defaultValue: "Type" })}>
      ...
      {options.severities.length > 1 && (
        <FilterSection title={t("decisions.filterSeverity", { defaultValue: "Severity" })}>
      ...
      {(options.projects.length > 0 || options.hasNoProject) && (
        <FilterSection title={t("decisions.filterProject", { defaultValue: "Project" })}>
          ...
          {options.hasNoProject && (
            <FilterRow
              label={t("decisions.noProject", { defaultValue: "No project" })}
      ...
      {(options.workspaces.length > 0 || options.hasNoWorkspace) && (
        <FilterSection title={t("decisions.filterWorkspace", { defaultValue: "Workspace" })}>
          ...
          {options.hasNoWorkspace && (
            <FilterRow
              label={t("decisions.noWorkspace", { defaultValue: "No workspace" })}
