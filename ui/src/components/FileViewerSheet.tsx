...
function normalizeError(error: unknown): FileViewerErrorShape {
  ...
  return { status: 0, code: "", message: t("fileViewer.somethingWentWrong", { defaultValue: "Something went wrong." }) };
}
...
export function describeDenial(code: string, fallback: string): { title: string; body: string; icon: ReactNode } {
  ...
  if (lower.includes("policy") || lower.includes("denied") || lower.includes("sensitive")) {
    return {
      ...
      title: t("fileViewer.viewerBlocked", { defaultValue: "Viewer blocked for this file" }),
      body: t("fileViewer.viewerBlockedBody", { defaultValue: "This file is not available through the viewer because it may contain sensitive data." }),
    };
  }
  if (lower.includes("outside") || lower.includes("traversal")) {
    return {
      icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
      title: t("fileViewer.pathOutsideWorkspace", { defaultValue: "Path is outside the workspace" }),
      body: t("fileViewer.pathOutsideWorkspaceBody", { defaultValue: "The viewer can only open files that live under the issue's workspace." }),
    };
  }
  if (lower.includes("archive") || lower.includes("cleaned")) {
    return {
      icon: <FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />,
      title: t("workspaceFiles.workspaceUnavailable", { defaultValue: "Workspace is no longer available" }),
      body: t("workspaceFiles.workspaceUnavailableBody", { defaultValue: "The isolated worktree for this issue has been cleaned up, so files cannot be previewed." }),
    };
  }
  if (lower.includes("remote")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("workspaceFiles.remotePreviewUnsupported", { defaultValue: "Remote workspace preview not supported" }),
      body: t("workspaceFiles.remotePreviewUnsupportedBody", { defaultValue: "This workspace is hosted remotely and is not available for inline preview yet." }),
    };
  }
  if (lower.includes("too_large") || lower.includes("size")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("fileViewer.fileTooLarge", { defaultValue: "File is too large to preview" }),
      body: t("fileViewer.fileTooLargeBody", { defaultValue: "This file exceeds the supported preview size." }),
    };
  }
  if (lower.includes("binary") || lower.includes("unsupported")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("fileViewer.previewUnsupported", { defaultValue: "Preview not supported for this file type" }),
      body: t("fileViewer.previewUnsupportedBody", { defaultValue: "This file does not have a text, image, or video preview available." }),
    };
  }
  return {
    icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
    title: t("fileViewer.cantPreview", { defaultValue: "Can't preview this file" }),
    body: fallback || t("fileViewer.loadFailed", { defaultValue: "The viewer was unable to load this file." }),
  };
}
        ...
        <>
          ...
          {state?.line ? (
            <>
              <span aria-hidden="true" className="opacity-50">·</span>
              <span>
                {t("fileViewer.lineColumn", { defaultValue: "Line {{line}}{{column}}", line: state.line, column: state.column ? t("fileViewer.columnSuffix", { defaultValue: ", Col {{column}}", column: state.column }) : "" })}
              </span>
            </>
          ) : null}
        </>
...
export function FileContentViewer({ content, highlightedLine, onLoaded }: FileContentViewerProps) {
  ...
  useEffect(() => {
    if (!lines) return;
    onLoaded?.(t("fileViewer.fileLoaded", { defaultValue: "File loaded, {{count}} line{{plural}}.", count: lines.length, plural: lines.length === 1 ? "" : "s" }));
  }, [lines, onLoaded, t]);
  ...
  if (resource.previewKind === "image") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("fileViewer.imagePreviewUnavailable", { defaultValue: "Image preview unavailable" })}
        />
      );
    }
    return (
  ...
  if (resource.previewKind === "video") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("fileViewer.videoPreviewUnavailable", { defaultValue: "Video preview unavailable" })}
        />
      );
    }
    return (
      <div className="flex flex-1 items-center justify-center overflow-auto bg-black p-4">
        <video
          ...
          preload="metadata"
          playsInline
          aria-label={t("fileViewer.videoPreview", { defaultValue: "Video preview: {{title}}", title: resource.title })}
  ...
  if (resource.previewKind === "unsupported" || !lines) {
    return (
      <FileViewerStateView
        icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
        title={t("fileViewer.previewUnsupported", { defaultValue: "Preview not supported for this file type" })}
        body={resource.contentType ? t("fileViewer.contentType", { defaultValue: "Content type: {{type}}", type: resource.contentType }) : undefined}
      />
    );
  }
  ...
  const rawSourceView = (
    <div
      ref={codeScrollRef}
      role="region"
      aria-label={t("fileViewer.source", { defaultValue: "{{title}} source", title: resource.title })}
      tabIndex={0}
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      {markdownMode === "raw" ? (
        ...
        <div
          role="region"
          aria-label={t("fileViewer.renderedMarkdownFor", { defaultValue: "{{title}} rendered Markdown", title: resource.title })}
          tabIndex={0}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-muted-foreground">
        ...
        {t("fileViewer.loadingPreview", { defaultValue: "Loading file preview..." })}
      </div>
    </div>
  );
}
...
export function FileViewerSheet({
  ...
  useEffect(() => {
    if (resolveQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("fileViewer.loadFileFailed", { defaultValue: "Unable to load file." }));
    }
  ...
  useEffect(() => {
    if (contentQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("fileViewer.loadFileContentFailed", { defaultValue: "Unable to load file content." }));
    }
  ...
  const handleCopyContent = useCallback(() => {
    ...
    void (async () => {
      ...
      const message = content.content.encoding === "base64" ? t("fileViewer.copiedFileData", { defaultValue: "Copied file data" }) : t("fileViewer.copiedContents", { defaultValue: "Copied contents" });
  ...
  const description = state
    ...
    : t("fileViewer.browseWorkspaceDescription", { defaultValue: "Search and preview files from this issue's workspace." });
                ...
                downloadUrl ? (
                  ...
                  >
                    <a
                      ...
                      aria-label={t("fileViewer.downloadFile", { defaultValue: "Download file" })}
                      title={t("fileViewer.downloadFile", { defaultValue: "Download file" })}
                    >
  ...
  if (resolveQuery.isError) {
    ...
    if (normalized.status === 404) {
      return (
        <FileViewerStateView
          ...
          title={t("fileViewer.fileNotFound", { defaultValue: "File not found" })}
          body={t("fileViewer.fileNotFoundBody", { defaultValue: "That file was not found in the active workspace." })}
          actions={
            <>
              {onFallbackToProject ? (
              ...
              <Button type="button" variant="ghost" size="sm" onClick={onRetry}>
                <RefreshCcw aria-hidden="true" className="mr-1 h-3 w-3" /> {t("common.tryAgain", { defaultValue: "Retry" })}
              </Button>
            </>
          }
        />
    ...
    if (normalized.status === 422) {
      return (
        <FileViewerStateView
          icon={<FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />}
          title={t("fileViewer.noWorkspaceAvailable", { defaultValue: "No workspace available" })}
          body={t("fileViewer.noWorkspaceAvailableBody", { defaultValue: "This issue does not have a workspace that supports preview yet." })}
        />
      );
    }
  ...
  if (resource.kind === "remote_resource") {
    return (
      <FileViewerStateView
        ...
        title={t("fileViewer.remotePreviewComingSoon", { defaultValue: "Remote workspace preview coming soon" })}
        body={t("fileViewer.remotePreviewComingSoonBody", { defaultValue: "This workspace is hosted remotely; inline previews are not supported yet." })}
      />
    );
  }
