...
export function IssueAssignedBacklogNotice({
  ...
  const assigneeLabel = assigneeAgent?.name ?? t("issueAssignedBacklog.assignee");
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          <p className="leading-5">
            <span className="font-medium">{t("issueAssignedBacklog.parked")}</span> —{" "}
            <span className="font-medium">{assigneeLabel}</span>{" "}
            {t("issueAssignedBacklog.parkedDescription")}
          </p>
          {assigneeAgent ? (
            <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
              {t("issueAssignedBacklog.commentsDescription")}
            </p>
          ) : null}
          {onResume ? (
            <div className="pt-0.5">
              ...
              >
                {resuming ? t("issueAssignedBacklog.resuming") : t("issueAssignedBacklog.resumeNow")}
              </Button>
            </div>
          ) : null}
        </div>
