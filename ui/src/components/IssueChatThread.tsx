    ...
    <div className="mb-3 rounded-md border border-orange-300/70 bg-orange-50/90 px-3 py-2.5 text-sm text-orange-950 shadow-sm dark:border-orange-500/40 dark:bg-orange-500/10 dark:text-orange-100">
      <div className="flex items-start gap-2">
        ...
        <p className="min-w-0 leading-5">
          {t("issueChat.assigneePausedNotice", {
            defaultValue: "{{name}} is paused. New runs will not start until the agent is resumed.",
            name: agent.name,
          })} {pauseDetail}
        </p>
      </div>
...
function fallbackAuthorLabel(message: ThreadMessage) {
  ...
  if (message.role === "assistant") return t("issueChat.authorAgent", { defaultValue: "Agent" });
  if (message.role === "user") return t("issueChat.authorYou", { defaultValue: "You" });
  return t("issueChat.authorSystem", { defaultValue: "System" });
}
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="space-y-1">
            <p className="font-medium">
              {t("issueChat.fallbackTitle", { defaultValue: "Chat renderer hit an internal state error." })}
            </p>
            <p className="text-xs opacity-80">
              {t("issueChat.fallbackSubtitle", {
                defaultValue: "Showing a safe fallback transcript instead of crashing the tasks page.",
              })}
            </p>
          </div>
        </div>
...
function humanizeValue(value: string | null) {
  if (!value) return t("issueChat.noneValue", { defaultValue: "None" });
...
export function resolveIssueChatHumanAuthor(args: {
  ...
  const resolvedAuthorName = profile?.label?.trim()
    ...
    || (authorUserId === "local-board"
      ? t("issueChat.boardAuthor", { defaultValue: "Board" })
      : (isCurrentUser
        ? t("issueChat.authorYou", { defaultValue: "You" })
        : t("issueChat.authorUser", { defaultValue: "User" })));
...
function toolCountSummary(toolParts: ToolCallMessagePart[]): string | null {
  ...
  const parts: string[] = [];
  if (commands > 0) {
    parts.push(t("issueChat.ranCommands", { defaultValue: "ran {{count}} command", count: commands }));
  }
  if (other > 0) {
    parts.push(t("issueChat.calledTools", { defaultValue: "called {{count}} tool", count: other }));
  }
  return parts.join(", ");
}
...
function IssueChatChainOfThought({
  ...
  if (isActive) {
    headerVerb = t("issueChat.cotWorking", { defaultValue: "Working" });
    if (liveElapsed) headerSuffix = t("issueChat.cotForDuration", { defaultValue: "for {{duration}}", duration: liveElapsed });
  } else if (segmentTiming) {
    const durationMs = segmentTiming.endMs - segmentTiming.startMs;
    const durationText = formatDurationWords(durationMs);
    headerVerb = t("issueChat.cotWorked", { defaultValue: "Worked" });
    if (durationText) headerSuffix = t("issueChat.cotForDuration", { defaultValue: "for {{duration}}", duration: durationText });
  } else {
    headerVerb = t("issueChat.cotWorked", { defaultValue: "Worked" });
  }
  ...
  return (
    <div className="group/pre relative">
      ...
      <button
        ...
        title={t("issueChat.copyAria", { defaultValue: "Copy" })}
        aria-label={t("issueChat.copyAria", { defaultValue: "Copy" })}
        onClick={() => {
          void copyTextToClipboard(children).then(() => {
            ...
            toastActions?.pushToast({
              title: t("issueChat.copyFailedToast", { defaultValue: "Copy failed" }),
              body: error instanceof Error
                ? error.message
                : t("issueChat.unableToCopyText", { defaultValue: "Unable to copy text" }),
              tone: "error",
            });
          });
        }}
        ...
        {open ? (
          <div className="mt-1 space-y-2 pb-1">
            {nonIntentDetails.length > 0 ? (
              <div>
                <div className="mb-1 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-eyebrow) text-muted-foreground/60">
                  {t("issueChat.toolInputHeader", { defaultValue: "Input" })}
                </div>
            ...
            {result !== undefined ? (
              <div>
                <div className="mb-1 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-eyebrow) text-muted-foreground/60">
                  {t("issueChat.toolResultHeader", { defaultValue: "Result" })}
                </div>
...
function IssueChatUserMessage({
  ...
  const queueBadgeLabel = queueReason === "hold"
    ? `\u23f8 ${t("issueChat.deferredWakeBadge", { defaultValue: "Deferred wake" })}`
    : t("issueChat.queuedBadge", { defaultValue: "Queued" });
      ...
      >
        {queued ? (
          <div className="mb-1.5 flex items-center gap-2">
            ...
            {queueTargetRunId && onInterruptQueued ? (
              ...
              >
                {isInterruptingQueuedRun
                    ? t("issueChat.interrupting", { defaultValue: "Interrupting..." })
                    : t("issueChat.interrupt", { defaultValue: "Interrupt" })}
              </Button>
            ) : null}
            {onCancelQueued ? (
              <Button
              ...
              >
                {t("issueChat.cancelQueued", { defaultValue: "Cancel" })}
              </Button>
            ) : null}
          </div>
        ) : null}
        {deleted ? (
          <div className="text-sm italic text-muted-foreground">
            {t("issueChat.commentDeleted", { defaultValue: "Comment deleted" })}
          </div>
        ) : (
        ...
        >
          ...
          {!deleted ? (
            <button
              ...
              title={t("issueChat.copyMessageAria", { defaultValue: "Copy message" })}
              aria-label={t("issueChat.copyMessageAria", { defaultValue: "Copy message" })}
              onClick={() => {
                ...
                void copyTextToClipboard(text).then(() => {
                  ...
                  toastActions?.pushToast({
                    ...
                    body: error instanceof Error
                ? error.message
                : t("issueChat.unableToCopyMessage", { defaultValue: "Unable to copy message" }),
                    tone: "error",
                  });
                });
              }}
          ...
          {canDeleteComment ? (
            <button
              ...
              title={t("issueChat.deleteCommentAria", { defaultValue: "Delete comment" })}
              aria-label={t("issueChat.deleteCommentAria", { defaultValue: "Delete comment" })}
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {t("issueChat.deleteCommentDescription", {
                defaultValue: "This will replace the comment with a deleted-comment marker.",
              })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteDialogOpen(false)}>
              {t("issueChat.cancel", { defaultValue: "Cancel" })}
            </Button>
            <Button variant="destructive" onClick={confirmDeleteComment}>
              {t("issueChat.deleteCommentButton", { defaultValue: "Delete comment" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function IssueChatAssistantMessage({
  ...
  const {
    ...
    stoppingRunLabel = t("issueChat.stoppingRun", { defaultValue: "Stopping..." }),
    stopRunVariant = "stop",
  ...
  const authorName = typeof custom.authorName === "string"
    ...
    : typeof custom.runAgentName === "string"
      ? custom.runAgentName
      : t("issueChat.authorAgent", { defaultValue: "Agent" });
  const authorAgentId = typeof custom.authorAgentId === "string" ? custom.authorAgentId : null;
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            title={t("issueChat.moreActionsAria", { defaultValue: "More actions" })}
            aria-label={t("issueChat.moreActionsAria", { defaultValue: "More actions" })}
          >
        ...
        <DropdownMenuContent align="end">
          ...
          >
            ...
            {t("issueChat.copyMessageAria", { defaultValue: "Copy message" })}
          </DropdownMenuItem>
      ...
      <div id={anchorId}>
        <div className="group flex flex-col items-start py-1.5">
          ...
          <div className="mb-1 flex items-center gap-1.5 px-1">
            ...
            {followUpRequested ? (
              <Badge variant="outline" className="text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow)">
                {t("issueChat.followUpBadge", { defaultValue: "Follow-up" })}
              </Badge>
            ) : null}
          </div>
  ...
  return (
    <>
      <button
        ...
        title={t("issueChat.feedbackHelpful", { defaultValue: "Helpful" })}
        aria-label={t("issueChat.feedbackHelpful", { defaultValue: "Helpful" })}
      ...
      <Popover open={reasonOpen} onOpenChange={setReasonOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            title={t("issueChat.feedbackNeedsWork", { defaultValue: "Needs work" })}
            aria-label={t("issueChat.feedbackNeedsWork", { defaultValue: "Needs work" })}
        ...
        <PopoverContent side="top" align="start" className="w-80 p-3">
          <div className="mb-2 text-sm font-medium">
            {t("issueChat.feedbackBetterQuestion", { defaultValue: "What could have been better?" })}
          </div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("issueChat.feedbackNotePlaceholder", { defaultValue: "Add a short note" })}
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            >
              {t("issueChat.feedbackDismiss", { defaultValue: "Dismiss" })}
            </Button>
            <Button
              type="button"
              size="sm"
            ...
            >
              {isSaving
                ? t("issueChat.savingDots", { defaultValue: "Saving..." })
                : t("issueChat.feedbackSaveNote", { defaultValue: "Save note" })}
            </Button>
          </div>
        </PopoverContent>
      </Popover>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t("issueChat.sharePrefTitle", { defaultValue: "Save your feedback sharing preference" })}
            </DialogTitle>
            <DialogDescription>
              {t("issueChat.sharePrefDescription", {
          ...
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>{t("issueChat.sharePrefLocal", { defaultValue: "This vote is always saved locally." })}</p>
            <p>
              {t("issueChat.sharePrefChoose", { defaultValue: "Choose" })}{" "}
              <span className="font-medium text-foreground">
                {t("issueChat.sharePrefAlwaysAllow", { defaultValue: "Always allow" })}
              </span>{" "}
              {t("issueChat.sharePrefToShare", {
                defaultValue: "to share this vote and future voted AI outputs. Choose",
              })}{" "}
              <span className="font-medium text-foreground">
                {t("issueChat.sharePrefDontAllow", { defaultValue: "Don't allow" })}
              </span>{" "}
              {t("issueChat.sharePrefToKeep", {
                defaultValue: "to keep this vote and future votes local.",
              })}
            </p>
            <p>
              {t("issueChat.sharePrefChangeLater", { defaultValue: "You can change this later in Settings > General." })}
            </p>
            {termsUrl ? (
              <a
                href={termsUrl}
              ...
              >
                {t("issueChat.sharePrefReadTerms", { defaultValue: "Read our terms of service" })}
              </a>
            ) : null}
          </div>
          <DialogFooter>
  ...
  const handleCopy = () => {
    void copyTextToClipboard(bodyText).then(() => {
      ...
      toastActions?.pushToast({
        ...
        body: error instanceof Error
          ? error.message
          : t("issueChat.unableToCopyNotice", { defaultValue: "Unable to copy system notice" }),
        tone: "error",
      });
    });
  };
  ...
  const handleCopyLink = () => {
    ...
    void copyTextToClipboard(url).then(() => {
      ...
      toastActions?.pushToast({
        ...
        body: error instanceof Error
          ? error.message
          : t("issueChat.unableToCopyNoticeLink", { defaultValue: "Unable to copy system notice link" }),
        tone: "error",
      });
    });
  };
        ...
        <div className="mt-1 flex items-center justify-end gap-1.5 px-1 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          {anchorId ? (
            <button
              type="button"
              className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
              title={t("issueChat.copyLinkAria", { defaultValue: "Copy link" })}
              aria-label={t("issueChat.copyNoticeLinkAria", { defaultValue: "Copy link to system notice" })}
          ...
          <button
            type="button"
            className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
            title={t("issueChat.copyNoticeTextAria", { defaultValue: "Copy notice text" })}
            aria-label={t("issueChat.copyNoticeAria", { defaultValue: "Copy system notice" })}
            onClick={handleCopy}
          >
            {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
          </button>
        ...
        setComposerAttachments((prev) => prev.map((item) =>
          item.id === attachmentId
            ? {
                  ...item,
                  status: "error",
                  error: t("issueChat.fileTypeNotAllowed", { defaultValue: "This file type cannot be attached here" }),
                }
            : item,
        ));
      }
        ...
        item.id === attachmentId
          ? {
              ...
              error: err instanceof Error
                ? err.message
                : t("issueChat.uploadFailed", { defaultValue: "Upload failed" }),
            }
          : item,
      ));
    }
  ...
  return (
    ...
    >
          ...
          <div className="flex max-w-md items-center gap-3 rounded-md bg-background/80 px-3 py-2 text-left shadow-sm ring-1 ring-border/60">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 text-xs leading-5 text-muted-foreground">
                {t("issueChat.dropOverlayHint", {
                  defaultValue: "Images insert into the reply. Other files are added to this task.",
                })}
              </div>
            </div>
          </div>
      ...
      <MarkdownEditor
        ...
        value={body}
        onChange={setBody}
        placeholder={t("issueChat.replyPlaceholder", { defaultValue: "Reply" })}
        mentions={mentions}
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
          ...
          {(onImageUpload || onAttachImage) ? (
            <>
              ...
              <Button
                ...
                disabled={attaching}
                title={t("issueChat.attachFileAria", { defaultValue: "Attach file" })}
              >
                <Paperclip className="h-4 w-4" />
              </Button>
            </>
            ...
            <Popover open={workModeMenuOpen} onOpenChange={setWorkModeMenuOpen}>
              ...
              >
                ...
                <div className="mt-1 border-t px-2 py-1.5 text-(length:--text-nano) text-muted-foreground">
                  {t("issueChat.workModeCycleHint", { defaultValue: "Cmd/Ctrl+. cycles modes" })}
                </div>
              </PopoverContent>
            </Popover>
          ) : null}
        ...
        <Button size="sm" disabled={!canSubmit} onClick={() => void handleSubmit()}>
          {submitting
            ...
            : t("issueChat.send", { defaultValue: "Send" })}
        </Button>
      </div>
      ...
      <AlertDialog open={noAssigneeDialogOpen} onOpenChange={setNoAssigneeDialogOpen}>
        ...
        >
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("issueChat.noResponsibleDialogTitle", { defaultValue: "No responsible selected" })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("issueChat.noResponsibleDialogBody", {
                defaultValue: "This comment will be posted without an assignee, so no agent will be woken to act on it. Go back to pick a responsible, or send anyway.",
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {t("issueChat.goBack", { defaultValue: "Go back" })}
            </AlertDialogCancel>
            <AlertDialogAction
            ...
            >
              {t("issueChat.sendAnyway", { defaultValue: "Send anyway" })}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
  ...
  const resolvedEmptyMessage = emptyMessage
    ?? (variant === "embedded"
      ...
      : t("issueChat.emptyConversation", {
          defaultValue: "This task conversation is empty. Start with a message below.",
        }));
      ...
      <div className={cn(variant === "embedded" ? "space-y-3" : "space-y-4")}>
        {resolvedShowJumpToLatest ? (
          <div className="flex justify-end">
            ...
            >
              {t("issueChat.jumpToLatest", { defaultValue: "Jump to latest" })}
            </button>
          </div>
        ) : null}
              ...
              {showComposer ? (
                <div data-testid="issue-chat-thread-notices" className="space-y-2">
                  ...
                  {legacyRecoverySourceIssue ? (
                    <SystemNotice
                      tone="info"
                      label={t("issueChat.legacyRecoveryLabel", { defaultValue: "Legacy recovery task" })}
                      body={
                        <span>
                          {t("issueChat.legacyRecoveryBody", {
                            defaultValue: "Legacy recovery task. Newer recovery actions live on the source task",
                          })}
