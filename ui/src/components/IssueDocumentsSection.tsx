  ...
  if (revision.createdByUserId) {
    ...
    return {
      ...
      name: profile?.label ?? (revision.createdByUserId === "local-board"
        ? translate("issueDocuments.boardActor")
        : revision.createdByUserId.slice(0, 8)),
      imageUrl: profile?.image ?? null,
    };
  }
  return { kind: "system", name: translate("issueDocuments.systemActor") };
}
...
export function IssueDocumentsSection({
  ...
  const deleteDocument = useMutation({
    mutationFn: (key: string) => documentSubject.deleteDocument
      ...
      : Promise.reject(new Error(t("issueDocuments.deletionUnavailable"))),
    onSuccess: () => {
      setError(null);
  ...
  const restoreDocumentRevision = useMutation({
    mutationFn: ({ key, revisionId }: { key: string; revisionId: string }) =>
      documentSubject.restoreDocumentRevision
        ...
        : Promise.reject(new Error(t("issueDocuments.restoreUnavailable"))),
  ...
  const setDocumentLock = useMutation({
    mutationFn: ({ key, locked }: { key: string; locked: boolean }) =>
      documentSubject.setDocumentLock
        ...
        : Promise.reject(new Error(t("issueDocuments.lockUnavailable"))),
    onSuccess: (document) => {
      syncDocumentCaches(document);
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("issueDocuments.lockUpdateFailed"));
    },
  });
  ...
  const newDocumentKeyError =
    draft?.isNew && draft.key.trim().length > 0 && !DOCUMENT_KEY_PATTERN.test(draft.key.trim())
      ? t("issueDocuments.invalidKeyHint")
      : null;
  ...
  const commitDraft = useCallback(async (
    ...
    if (!DOCUMENT_KEY_PATTERN.test(normalizedKey)) {
      setError(t("issueDocuments.invalidKey"));
      if (options?.trackAutosave) {
        resetAutosaveState();
      }
      return false;
    ...
    try {
      ...
      if (isLockedDocumentError(err)) {
        setError(t("issueDocuments.documentLocked"));
        resetAutosaveState();
        invalidateIssueDocuments();
        return false;
      }
      if (isDocumentConflictError(err)) {
        try {
          ...
          setError(t("issueDocuments.remoteLatestUnavailable"));
          return false;
        }
      }
  ...
  const previewRevision = useCallback((doc: IssueDocument, revisionId: string) => {
    ...
    if (documentConflict?.key === doc.key || documentHasUnsavedChanges(doc, draft)) {
      setError(t("issueDocuments.saveOrCancelBeforeRevision"));
      return;
    }
    resetAutosaveState();
  ...
  const toggleDocumentLock = useCallback((doc: IssueDocument, locked: boolean) => {
    ...
    if (locked && (documentConflict?.key === doc.key || documentHasUnsavedChanges(doc, draft))) {
      setError(t("issueDocuments.saveOrCancelBeforeLock"));
      return;
    }
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            >
              {upsertDocument.isPending
                ? t("issueDocuments.saving")
                : t("issueDocuments.createDocument")}
            </Button>
          </div>
        </div>
      )}
                ...
                actionsSlot={
                  <>
                    ...
                    <Button
                      ...
                      aria-label={t(
                        isLocked ? "issueDocuments.unlockDocumentAria" : "issueDocuments.lockDocumentAria",
                        { key: doc.key },
                      )}
                    ...
                    >
                      ...
                      <span title={t("issueDocuments.lockedDocument")} aria-label={t("issueDocuments.lockedDocument")} className="inline-flex h-6 w-6 items-center justify-center text-amber-700 dark:text-amber-300">
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {t("issueDocuments.historicalPreviewDescription")}
                          </p>
                        </div>
                          ...
                          {!isLocked ? (
                            ...
                            >
                              {restoreDocumentRevision.isPending && restoreDocumentRevision.variables?.key === doc.key
                                ...
                                : t("issueDocuments.restoreRevision")}
                            </Button>
                          ) : null}
                        </div>
                      </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          >
                            {activeConflict.showRemote
                              ? t("issueDocuments.hideRemote")
                              : t("issueDocuments.reviewRemote")}
                          </Button>
                          <Button
                            variant="outline"
                            size="sm"
                          ...
                          >
                            {upsertDocument.isPending
                              ? t("issueDocuments.saving")
                              : t("issueDocuments.overwriteRemote")}
                          </Button>
                        </div>
                      </div>
                      {activeConflict.showRemote && (
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    >
                      {deleteDocument.isPending
                        ? t("issueDocuments.deleting")
                        : t("issueDocuments.delete")}
                    </Button>
                  </div>
                </div>
              )}
