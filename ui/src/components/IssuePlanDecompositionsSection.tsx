  ...
  if (status === "completed") {
    return (
      <span className="inline-flex items-center gap-1 rounded-sm border border-emerald-500/50 bg-emerald-500/10 px-2 py-0.5 text-(length:--text-micro) font-medium text-emerald-900 dark:text-emerald-100">
        ...
        {t("issuePlanDecomposition.completed")}
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 rounded-sm border border-amber-500/50 bg-amber-500/10 px-2 py-0.5 text-(length:--text-micro) font-medium text-amber-900 dark:text-amber-100">
      ...
      {t("issuePlanDecomposition.inFlight")}
    </span>
  );
}
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-medium text-muted-foreground">
          {t("issuePlanDecomposition.title")}
        </h3>
        <span className="text-(length:--text-micro) text-muted-foreground/80">
          {t(
            items.length === 1
              ? "issuePlanDecomposition.acceptedPlanRevision_one"
              : "issuePlanDecomposition.acceptedPlanRevision_other",
            { count: items.length },
          )}
        </span>
      </div>
      ...
      <ul className="space-y-3">
        {items.map((record) => {
          ...
          const ownerName = record.ownerAgentId
            ? agentMap?.get(record.ownerAgentId)?.name ?? t("issuePlanDecomposition.agent")
            : null;
          const revisionLabel =
            record.acceptedPlanRevisionNumber != null
              ? t("issuePlanDecomposition.revision", { id: record.acceptedPlanRevisionNumber })
              : t("issuePlanDecomposition.revision", { id: record.acceptedPlanRevisionId.slice(0, 8) });
          const completedAt =
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("issuePlanDecomposition.planRevision", { revision: revisionLabel })}
                </span>
                ...
                <span className="inline-flex items-center gap-1 text-xs text-foreground">
                  ...
                  {t(
                    requested === 1
                      ? "issuePlanDecomposition.childTasksCreated_one"
                      : "issuePlanDecomposition.childTasksCreated_other",
                    { created, requested },
                  )}
                </span>
                {record.status === "completed" && requested > 0 ? (
                  <span
                    ...
                    title={t("issuePlanDecomposition.idempotentClaimDescription")}
                  >
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-(length:--text-micro) text-muted-foreground">
                {ownerName
                  ? <span>{t("issuePlanDecomposition.owner", { name: ownerName })}</span>
                  : null}
                {startedAt ? (
                  <span title={formatDateTime(startedAt)}>
                    {t("issuePlanDecomposition.started", { time: relativeTime(startedAt) })}
                  </span>
                ) : null}
                {completedAt ? (
                  <span title={formatDateTime(completedAt)}>
                    {t("issuePlanDecomposition.completedAt", { time: relativeTime(completedAt) })}
                  </span>
                ) : updatedAt ? (
                  <span title={formatDateTime(updatedAt)}>
                    {t("issuePlanDecomposition.updated", { time: relativeTime(updatedAt) })}
                  </span>
                ) : null}
                {issueIdentifier ? (
                  <Link
                  ...
                  >
                    {t("issuePlanDecomposition.planDocument")}
                  </Link>
                ) : null}
              </div>
