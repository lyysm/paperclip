...
export function NewAgentDialog() {
  ...
  function handleAskCeo() {
    ...
    openNewIssue({
      ...
      title: t("newAgentDialog.createNewAgentIssueTitle"),
      description: t("newAgentDialog.createNewAgentIssueDescription"),
    });
  }
  ...
  async function copyText(text: string, unavailableBody: string) {
    try {
      ...
      pushToast({
        title: t("newAgentDialog.clipboardUnavailable"),
  ...
  const createAgentInviteMutation = useMutation({
    ...
    onSuccess: async (invite) => {
      ...
      setLatestAgentPromptCopied(false);
      setMode("prompt");
      const copied = await copyText(prompt, t("newAgentDialog.copyPromptUnavailableBelow"));

      await queryClient.invalidateQueries({ queryKey: inviteHistoryQueryKey });
      pushToast({
        title: t("newAgentDialog.inviteCreated"),
        body: copied
          ? t("newAgentDialog.promptReadyCopied")
          : t("newAgentDialog.promptReady"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("newAgentDialog.inviteFailed"),
        body: error instanceof Error ? error.message : t("newAgentDialog.unknownError"),
        tone: "error",
      });
    },
  });
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          ...
          <Button
            ...
            aria-label={t("newAgentDialog.close")}
            onClick={() => {
              resetDialogState();
              closeNewAgent();
            }}
          ...
          {mode === "choices" ? (
            <>
              ...
              <div className="text-center space-y-3">
                ...
                <p className="text-sm text-muted-foreground">
                  {t("newAgentDialog.choicesDescription")}
                </p>
              </div>
              ...
              <div className="grid gap-2">
                <Button variant="outline" className="w-full" onClick={handleAdvancedConfig}>
                  ...
                  {t("newAgentDialog.configureRuntime")}
                </Button>
                <div className="space-y-1">
                  <Button variant="outline" className="w-full" onClick={handleInviteExternalAgent}>
                    <MailPlus className="h-4 w-4 mr-2" />
                    {t("newAgentDialog.inviteExternal")}
                  </Button>
                  <p className="text-xs text-muted-foreground text-center">
                    {t("newAgentDialog.inviteExternalHint")}
                  </p>
                </div>
              </div>
            </>
            ...
            <>
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  {t("newAgentDialog.runtimeDescription")}
                </p>
              </div>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <h2 className="text-sm font-semibold">{t("newAgentDialog.inviteExternal")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("newAgentDialog.inviteDescription")}
                  </p>
                </div>
              </div>

              <label className="block space-y-2">
                <span className="text-sm font-medium">{t("newAgentDialog.optionalMessage")}</span>
                <Textarea
                  value={agentMessage}
                  onChange={(event) => setAgentMessage(event.target.value)}
                  className="min-h-24 resize-y"
                  placeholder={t("newAgentDialog.messagePlaceholder")}
                  maxLength={4000}
                />
              </label>

              <div className="rounded-lg border border-border px-4 py-3 text-sm text-muted-foreground">
                {t("newAgentDialog.inviteApprovalNote")}
              </div>
              ...
              <div>
                ...
                >
                  {createAgentInviteMutation.isPending
                    ? t("newAgentDialog.generatingPrompt")
                    : t("newAgentDialog.generatePrompt")}
                </Button>
              </div>
            </div>
          ) : (
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-3">
                    <h2 className="text-sm font-semibold">{t("newAgentDialog.promptTitle")}</h2>
                    {latestAgentPromptCopied ? (
                      <div className="inline-flex items-center gap-1 text-xs font-medium text-foreground">
                        <Check className="h-3.5 w-3.5" />
                        {t("newAgentDialog.copied")}
                      </div>
                    ) : null}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    {t("newAgentDialog.promptDescription")}
                  </p>
                </div>
              </div>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  const copied = await copyText(latestAgentPrompt, t("newAgentDialog.copyPromptUnavailableAbove"));
                  setLatestAgentPromptCopied(copied);
                }}
              >
                {latestAgentPromptCopied
                  ? t("newAgentDialog.copiedPrompt")
                  : t("newAgentDialog.copyPrompt")}
              </Button>
            </div>
          )}
        </div>
