...
const projectStatuses = [
  { value: "backlog", labelKey: "newProjectDialog.statusBacklog", label: "Backlog" },
  { value: "planned", labelKey: "newProjectDialog.statusPlanned", label: "Planned" },
  { value: "in_progress", labelKey: "newProjectDialog.statusInProgress", label: "In Progress" },
  { value: "completed", labelKey: "newProjectDialog.statusCompleted", label: "Completed" },
  { value: "cancelled", labelKey: "newProjectDialog.statusCancelled", label: "Cancelled" },
];
...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("newProjectDialog.localFolderAbsoluteError", { defaultValue: "Local folder must be a full absolute path." }));
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("newProjectDialog.repoUrlInvalidError", { defaultValue: "Repo must use a valid GitHub or GitHub Enterprise repo URL." }));
      return;
    }
      ...
      >
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t("newProjectDialog.projectNamePlaceholder", { defaultValue: "Project name" })}
            value={name}
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("newProjectDialog.repoUrlLabel", { defaultValue: "Repo URL" })}</label>
              ...
              <Tooltip delayDuration={300}>
                ...
                <TooltipContent side="top" className="max-w-(--sz-240px) text-xs">
                  {t("newProjectDialog.repoUrlTooltip", {
                    defaultValue: "Link a GitHub repository so agents can clone, read, and push code for this project.",
                  })}
                </TooltipContent>
              </Tooltip>
            </div>
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("newProjectDialog.localFolderLabel", { defaultValue: "Local folder" })}</label>
              <span className="text-xs text-muted-foreground/50">{t("newProjectDialog.optional", { defaultValue: "optional" })}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          {selectedGoals.map((goal) => (
            ...
            >
              ...
              <button
                ...
                aria-label={t("newProjectDialog.removeGoal", { defaultValue: "Remove goal {{title}}", title: goal.title })}
                type="button"
              >
                <X className="h-3 w-3" />
              </button>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={t("newProjectDialog.targetDatePlaceholder", { defaultValue: "Target date" })}
            />
          </div>
        </div>
