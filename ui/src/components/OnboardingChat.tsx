...
const CYCLING_MESSAGE_KEYS: Record<string, string> = {
  "Heartbeat triggered, waking up...": "onboardingChat.status.heartbeatTriggered",
  "Initializing...": "onboardingChat.status.initializing",
  "Getting ready...": "onboardingChat.status.gettingReady",
  "Working on a response...": "onboardingChat.status.working",
  "Reading the conversation...": "onboardingChat.status.reading",
  "Thinking through the plan...": "onboardingChat.status.thinkingPlan",
  "Drafting a response...": "onboardingChat.status.drafting",
  "Still working...": "onboardingChat.status.stillWorking",
  "Almost there...": "onboardingChat.status.almostThere",
  "Waiting to wake up...": "onboardingChat.status.waitingToWake",
  "Heartbeat pending...": "onboardingChat.status.heartbeatPending",
  "Should wake up soon...": "onboardingChat.status.wakeSoon",
};
...
function getCyclingMessage(messages: string[], elapsed: number, agentName: string): string {
  ...
  return t(CYCLING_MESSAGE_KEYS[message] ?? "onboardingChat.status.thinking", {
...
function getRunStatusMessage(status: string, agentName: string, elapsed: number): string {
  switch (status) {
    ...
    case "succeeded":
      return t("onboardingChat.status.finished", { defaultValue: "{{agent}} finished", agent: agentName });
    case "failed":
      return t("onboardingChat.status.error", { defaultValue: "{{agent}} encountered an error", agent: agentName });
    case "cancelled":
      return t("onboardingChat.status.cancelled", { defaultValue: "{{agent}}'s run was cancelled", agent: agentName });
    case "timed_out":
      return t("onboardingChat.status.timedOut", { defaultValue: "{{agent}}'s run timed out", agent: agentName });
    default:
      return t("onboardingChat.status.thinking", { defaultValue: "{{agent}} is thinking...", agent: agentName });
  }
}
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
        ...
        {t("onboardingChat.loadingConversation", { defaultValue: "Loading conversation..." })}
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full">
      ...
      >
        ...
        <WelcomeMessage
          ...
          onDiscuss={() => {
            setInput(t("onboardingChat.discussPrompt", { defaultValue: "I want to discuss the plan before you get started." }));
            inputRef.current?.focus();
          }}
          onStart={() => sendMessage(t("onboardingChat.startPrompt", { defaultValue: "Yes, get started on the hiring plan!" }))}
        />
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 mb-1">
                ...
                >
                  {isAgent ? agentName : t("onboardingChat.you", { defaultValue: "You" })}
                </span>
                {isPlan && (
                  <span className="inline-flex items-center gap-0.5 text-(length:--text-nano) text-green-600 dark:text-green-400 font-medium">
                    ...
                    {t("onboardingChat.hiringPlanDetected", { defaultValue: "Hiring plan detected" })}
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-medium">
                  {t("onboardingChat.planPrepared", { defaultValue: "{{agent}} has prepared a hiring plan", agent: agentName })}
                </p>
                <p className="text-(length:--text-micro) text-muted-foreground">
                  {t("onboardingChat.planPreparedDescription", { defaultValue: "Review it, make edits, then approve." })}
                </p>
              </div>
            </div>
            <Button size="sm" onClick={onReviewPlan}>
              {t("onboardingChat.reviewPlan", { defaultValue: "Review plan" })}
      ...
      <div className="flex items-center gap-2 border-t border-border pt-3">
        <input
          ...
          placeholder={detectedPlanCommentId ? t("onboardingChat.revisePlaceholder", { defaultValue: "Ask {{agent}} to revise the plan...", agent: agentName }) : t("onboardingChat.messagePlaceholder", { defaultValue: "Message {{agent}}...", agent: agentName })}
          value={input}
    ...
    <>
      ...
      {showMessage && (
        <div className="rounded-md px-3 py-2 text-sm bg-muted/50 border border-border mr-8 animate-in fade-in duration-300">
          ...
          <p>
            {t("onboardingChat.welcomeLead", { defaultValue: "Hi! Thanks for bringing me on to lead {{company}}.", company: companyName })}
          </p>
          <p className="mt-1">
            {t("onboardingChat.welcomeMission", { defaultValue: "Our mission is: {{mission}}", mission: companyGoal })}
          </p>
          <p className="mt-1">
            {t("onboardingChat.welcomePlanQuestion", { defaultValue: "I'm ready to put together a plan for who we should bring on. Want me to get started?" })}
          </p>
        </div>
      )}
      ...
      {showChips && (
        <div className="flex gap-2 ml-auto justify-end animate-in fade-in duration-500">
          <button
            ...
            onClick={onDiscuss}
          >
            {t("onboardingChat.discussFirst", { defaultValue: "Let's discuss first" })}
          </button>
          <button
            ...
            onClick={onStart}
          >
            {t("onboardingChat.getStarted", { defaultValue: "Yes, get started!" })}
          </button>
        </div>
      )}
      ...
      {!showMessage && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground px-3 py-2">
          ...
          {phase === "waking"
            ? t("onboardingChat.waking", { defaultValue: "{{agent}} is waking up...", agent: agentName })
            : t("onboardingChat.composing", { defaultValue: "{{agent}} is composing a message...", agent: agentName })}
        </div>
      )}
    </>
  );
