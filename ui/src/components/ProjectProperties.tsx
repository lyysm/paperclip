...
const PROJECT_STATUSES = [
  { value: "backlog", labelKey: "projectProperties.statusBacklog", label: "Backlog" },
  { value: "planned", labelKey: "projectProperties.statusPlanned", label: "Planned" },
  { value: "in_progress", labelKey: "projectProperties.statusInProgress", label: "In Progress" },
  { value: "completed", labelKey: "projectProperties.statusCompleted", label: "Completed" },
  { value: "cancelled", labelKey: "projectProperties.statusCancelled", label: "Cancelled" },
];
...
const SHARED_WORKSPACE_CONCURRENCY_OPTIONS: {
  ...
  {
    value: "auto",
    labelKey: "projectProperties.concurrencyAuto",
    label: "Auto",
    helpKey: "projectProperties.concurrencyAutoHelp",
    help: "Concurrent runs on local/SSH runners; runs take turns in cloud environments.",
  },
  {
    value: "serialize",
    labelKey: "projectProperties.concurrencySerialize",
    label: "Serialize",
    helpKey: "projectProperties.concurrencySerializeHelp",
    help: "Runs always take turns in the shared project workspace.",
  },
  {
    value: "allow",
    labelKey: "projectProperties.concurrencyAllow",
    label: "Allow",
    helpKey: "projectProperties.concurrencyAllowHelp",
  ...
  if (state === "saved") {
    return (
      <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-green-600 dark:text-green-400">
        ...
        {t("projectProperties.savedState", { defaultValue: "Saved" })}
      </span>
    );
  }
  if (state === "error") {
    return (
      <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-destructive">
        ...
        {t("projectProperties.failedState", { defaultValue: "Failed" })}
      </span>
    );
  }
  return null;
      ...
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("projectProperties.archiveHelp", {
              defaultValue: "Archive this project to hide it from the sidebar and project selectors.",
            })
          : t("projectProperties.unarchiveHelp", {
              defaultValue: "Unarchive this project to restore it in the sidebar and project selectors.",
            })}
      </p>
      {archivePending ? (
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {t("projectProperties.archiveConfirmTitle", {
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{t("projectProperties.archiveProjectAction", { defaultValue: "{{action}} project", action })}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{t("projectProperties.archiveProjectAction", { defaultValue: "{{action}} project", action })}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const sharedConcurrencyOption = SHARED_WORKSPACE_CONCURRENCY_OPTIONS.find(
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("projectProperties.localFolderAbsoluteError", { defaultValue: "Local folder must be a full absolute path." }));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("projectProperties.repoUrlInvalidError", { defaultValue: "Repo must use a valid GitHub or GitHub Enterprise repo URL." }));
      return;
    }
    setWorkspaceError(null);
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("projectProperties.nameField", { defaultValue: "Name" })} state={fieldState("name")} />}>
        ...
        <PropertyRow
          label={<FieldLabel label={t("projectProperties.descriptionField", { defaultValue: "Description" })} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            <InlineEditor
              ...
              placeholder={t("projectDetail.addDescriptionPlaceholder", { defaultValue: "Add a description..." })}
              multiline
            />
          ) : (
            <p className="text-sm text-muted-foreground">
        ...
        <PropertyRow label={<FieldLabel label={t("projectProperties.statusField", { defaultValue: "Status" })} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("projectProperties.leadField", { defaultValue: "Lead" })}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("projectProperties.goalsField", { defaultValue: "Goals" })} state={fieldState("goals")} />}
          alignStart
                ...
                >
                  ...
                  {(onUpdate || onFieldUpdate) && (
                    <button
                      ...
                      aria-label={t("projectProperties.removeGoal", { defaultValue: "Remove goal {{title}}", title: goal.title })}
                    >
                      <X className="h-3 w-3" />
                    </button>
                  )}
        ...
        <PropertyRow
          label={<FieldLabel label={t("projectProperties.envField", { defaultValue: "Env" })} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
        ...
        <PropertyRow label={<FieldLabel label={t("projectProperties.createdField", { defaultValue: "Created" })} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projectProperties.updatedField", { defaultValue: "Updated" })} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("projectDetail.targetDate", { defaultValue: "Target Date" })} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{t("projectProperties.codebaseSection", { defaultValue: "Codebase" })}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  aria-label={t("projectProperties.codebaseHelpLabel", { defaultValue: "Codebase help" })}
                >
                  ?
                </button>
              </TooltipTrigger>
          ...
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("projectProperties.repoLabel", { defaultValue: "Repo" })}</div>
              {codebase.repoUrl ? (
            ...
            <div className="space-y-1">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("projectProperties.localFolderLabel", { defaultValue: "Local folder" })}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-(length:--text-micro) text-muted-foreground">
                      {t("projectProperties.paperclipManagedFolder", { defaultValue: "Paperclip-managed folder." })}
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-(length:--text-nano) text-muted-foreground hover:text-foreground"
                      aria-label={t("projectProperties.executionWorkspacesHelpLabel", { defaultValue: "Execution workspaces help" })}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("projectProperties.enableIsolatedCheckouts", { defaultValue: "Enable isolated task checkouts" })}</span>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("projectProperties.newTasksIsolatedDefault", { defaultValue: "New tasks default to isolated checkout" })}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-(length:--text-micro) text-muted-foreground">
                          {t("projectProperties.newTasksIsolatedDefaultHelp", {
                            defaultValue:
                    ...
                    <div className="space-y-0.5">
                      <div className="mb-1 flex items-center gap-1.5">
                        <label className="flex items-center gap-2 text-sm">
                          <span>{t("projectProperties.sharedWorkspaceConcurrency", { defaultValue: "Shared workspace concurrency" })}</span>
                      ...
                      {onUpdate || onFieldUpdate ? (
                        ...
                        <div className="text-xs">
                          {sharedConcurrencyOption
                            ? t(sharedConcurrencyOption.labelKey, { defaultValue: sharedConcurrencyOption.label })
                            : null}
                        </div>
                      )}
                      <p className="text-(length:--text-micro) text-muted-foreground">
                        {sharedConcurrencyOption
                          ? t(sharedConcurrencyOption.helpKey, { defaultValue: sharedConcurrencyOption.help })
                          : null}
                      </p>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                          ...
                          <div>
                            <div className="mb-1 flex items-center gap-1.5">
                              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span>{t("projectProperties.environmentField", { defaultValue: "Environment" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.baseRefField", { defaultValue: "Base ref" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.branchTemplateField", { defaultValue: "Branch template" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.worktreeParentDirField", { defaultValue: "Worktree parent dir" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.provisionCommandField", { defaultValue: "Provision command" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.runtimeProvisionCommandField", { defaultValue: "Runtime provision command" })}</span>
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {t("projectProperties.runtimeProvisionHelp", {
                              defaultValue:
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.teardownCommandField", { defaultValue: "Teardown command" })}</span>
