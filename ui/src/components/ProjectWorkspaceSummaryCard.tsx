...
function workspaceKindLabel(kind: ProjectWorkspaceSummary["kind"]) {
  return kind === "execution_workspace"
    ? t("projectWorkspaces.executionWorkspace", { defaultValue: "Execution workspace" })
    : t("projectWorkspaces.projectWorkspace", { defaultValue: "Project workspace" });
}
          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="border-border/70 bg-background px-2.5 py-1 text-muted-foreground">
                {t("projectWorkspaces.updated", { defaultValue: "Updated {{time}}", time: timeAgo(summary.lastUpdatedAt) })}
              </Badge>
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {t("projectWorkspaces.servicesCount", { defaultValue: "{{running}}/{{total}} services", running: summary.runningServiceCount, total: summary.serviceCount })}
                </Badge>
              ) : null}
              {summary.executionWorkspaceStatus ? (
                <Badge variant="outline" className="border-border/70 bg-background px-2.5 py-1 text-muted-foreground">
                  {t(`statuses.generic.${summary.executionWorkspaceStatus}`, { defaultValue: summary.executionWorkspaceStatus.replace(/_/g, " ") })}
                </Badge>
              ) : null}
            </div>
            <Link
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices
                  ? t("projectWorkspaces.stopServices", { defaultValue: "Stop services" })
                  : t("projectWorkspaces.startServices", { defaultValue: "Start services" })}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed"
                  ? t("projectWorkspaces.retryClose", { defaultValue: "Retry close" })
                  : t("projectWorkspaces.closeWorkspace", { defaultValue: "Close workspace" })}
              </Button>
            ) : null}
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("projectWorkspaces.branch", { defaultValue: "Branch" })}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.branchName}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("projectWorkspaces.branchCopied", { defaultValue: "Branch copied" })}
                    >
                      {summary.branchName}
                    </CopyText>
                    <CopyText
                      text={summary.branchName}
                      ariaLabel={t("projectWorkspaces.copyBranch", { defaultValue: "Copy branch" })}
            ...
            {summary.cwd ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("projectWorkspaces.path", { defaultValue: "Path" })}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("projectWorkspaces.pathCopied", { defaultValue: "Path copied" })}
                    >
                      {truncatePath(summary.cwd)}
                    </CopyText>
                    <CopyText
                      text={summary.cwd}
                      ariaLabel={t("projectWorkspaces.copyPath", { defaultValue: "Copy path" })}
                      className="mt-0.5 shrink-0 text-muted-foreground hover:text-foreground"
                      copiedLabel={t("projectWorkspaces.pathCopied", { defaultValue: "Path copied" })}
                    >
                      <Copy className="h-3.5 w-3.5" />
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("projectWorkspaces.service", { defaultValue: "Service" })}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
              {t("projectWorkspaces.linkedTasks", { defaultValue: "Linked tasks" })}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                ...
                >
                  {t("projectWorkspaces.moreTasks", { defaultValue: "+{{count}} more", count: hiddenIssueCount })}
                </Link>
              ) : null}
            </div>
          </div>
