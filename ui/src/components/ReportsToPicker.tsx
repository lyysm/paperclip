        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("agentConfig.unknownManager")}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {t("agentConfig.reportsToCurrent", {
                  name: current.name,
                  suffix: terminatedManager ? ` (${t("agentConfig.terminated")})` : "",
                })}
              </span>
            </>
          ) : (
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            ...
            <span className="min-w-0 truncate">
              {t("agentConfig.currentManagerTerminated", { name: current.name })}
            </span>
          </div>
        )}
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            {t("agentConfig.savedManagerMissing")}
          </div>
        )}
        {rows.map((a) => (
          <button
