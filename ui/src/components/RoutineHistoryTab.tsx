...
export function RoutineHistoryTab({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      pushToast({
        title: t("routines.history.restoredRevision", { defaultValue: "Restored revision {{from}} as revision {{to}}", from: restoredFromNumber, to: newNumber }),
        body: data.secretMaterials.length > 0
          ? t("routines.history.restoreWithSecrets", { defaultValue: "Trigger enabled state was restored from the snapshot. New webhook secrets are available in the banner above." })
          : t("routines.history.restoreComplete", { defaultValue: "Trigger enabled state was restored from the snapshot." }),
        tone: "success",
      });
    ...
    onError: (error) => {
      pushToast({
        title: t("routines.history.restoreFailed", { defaultValue: "Failed to restore revision" }),
        body: error instanceof Error ? error.message : t("routines.history.restoreFailedBody", { defaultValue: "Paperclip could not restore the revision." }),
        tone: "error",
      });
    },
  });
  ...
  if (revisionsQuery.error) {
    return (
      <div className="rounded-md border border-l-2 border-l-destructive border-border p-4 space-y-3">
        <div>
          <p className="text-sm font-medium">{t("routines.history.loadFailed", { defaultValue: "Could not load revisions" })}</p>
          <p className="text-xs text-muted-foreground">
            {revisionsQuery.error instanceof Error
              ? revisionsQuery.error.message
              : t("routines.history.unknownLoadError", { defaultValue: "Unknown error loading revisions." })}
          </p>
        </div>
        <Button size="sm" variant="outline" onClick={() => revisionsQuery.refetch()}>
          {t("common.tryAgain", { defaultValue: "Retry" })}
        </Button>
      </div>
    );
  }
      ...
      <div className="space-y-4 min-w-0">
        ...
        {!isEditDirty && onlyBootstrapRevision ? (
          <div className="space-y-2">
            <EmptyState
              icon={HistoryIcon}
              message={t("routines.history.noEdits", { defaultValue: "No edits yet" })}
            />
            <p className="text-center text-xs text-muted-foreground">
              {t("routines.history.firstRevisionDescription", { defaultValue: "Revision 1 is the only history this routine has. Saving an edit creates the first additional revision." })}
            </p>
          </div>
        ) : (
          selectedRevision && (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
            {t("routines.history.viewingRevision", { defaultValue: "Viewing revision {{number}} (read-only)", number: revisionNumber })}
          </p>
          <p className="text-xs text-muted-foreground">
            {t("routines.history.restoreAppendOnly", { defaultValue: "Restoring this revision creates a new revision {{number}} with the same content. History stays append-only.", number: nextRevisionNumber })}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={onReturn} disabled={pending}>
            {t("routines.history.returnToCurrent", { defaultValue: "Return to current" })}
          </Button>
          <Button size="sm" onClick={onRestore} disabled={pending}>
            <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
            {t("routines.history.restoreAsNew", { defaultValue: "Restore as new revision" })}
          </Button>
        </div>
      </div>
    </div>
...
function ConflictBanner({
  ...
  const labels = dirtyFields.length > 0
    ...
    : [t("routines.history.theRoutine", { defaultValue: "the routine" })];
  const fieldsText = formatDirtyFieldList(labels);
  return (
    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">{t("routines.history.unsavedEdits", { defaultValue: "Unsaved routine edits" })}</p>
          <p className="text-xs text-muted-foreground">
            {t("routines.history.unsavedEditsDescription", { defaultValue: "You changed {{fields}} but haven't saved yet. Save or discard before previewing or restoring an older revision.", fields: fieldsText })}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={onDiscard}>
            {t("routines.history.discardChanges", { defaultValue: "Discard changes" })}
          </Button>
          <Button size="sm" onClick={onSave}>
            {t("routines.history.saveAndContinue", { defaultValue: "Save and continue" })}
          </Button>
        </div>
      </div>
    ...
    <aside className="space-y-1">
      <header className="flex items-center justify-between pb-2">
        <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("routines.history.revisions", { defaultValue: "Revisions" })}
        </p>
        <span className="text-(length:--text-micro) text-muted-foreground">{t("routines.history.totalRevisions", { defaultValue: "{{count}} total", count: totalRevisions })}</span>
      </header>
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 text-sm font-medium">
              <span>{t("routines.history.revisionShort", { defaultValue: "rev {{number}}", number: revision.revisionNumber })}</span>
              {isCurrent && (
                <Badge variant="outline" className="border-border px-1.5 text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
                  {t("routines.current", { defaultValue: "Current" })}
                </Badge>
              )}
              {revision.restoredFromRevisionId && (
                <Badge variant="outline" className="border-amber-500/40 bg-amber-500/10 px-1.5 text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow) text-amber-800 dark:text-amber-200">
                  {t("routines.history.restored", { defaultValue: "Restored" })}
                </Badge>
              )}
            </div>
            <div className="text-xs text-muted-foreground truncate">
              {t("routines.history.savedBy", { defaultValue: "Saved {{time}} by {{actor}}", time: relativeTime(revision.createdAt), actor: t(`routines.history.actor.${getActorLabel(revision)}`, { defaultValue: getActorLabel(revision) }) })}
      ...
      {totalRevisions > revisions.length && !showOlder && (
        <Button variant="ghost" size="sm" className="w-full" onClick={onShowOlder}>
          {t("routines.history.showOlder", { defaultValue: "Show {{count}} older…", count: totalRevisions - revisions.length })}
        </Button>
      )}
    </aside>
  );
...
function RevisionPreview({
  ...
  const restoreLabel = t("routines.history.restoreThisRevision", { defaultValue: "Restore this revision" });
  ...
  const fieldRows: Array<{ key: string; label: string; value: string; differs: boolean }> = [
    {
      key: "title",
      label: t("routines.history.field.title", { defaultValue: "Title" }),
      value: snapshot.title,
      differs: !!currentSnapshot && currentSnapshot.title !== snapshot.title,
    },
    {
      key: "priority",
      label: t("routines.history.field.priority", { defaultValue: "Priority" }),
      value: snapshot.priority,
      differs: !!currentSnapshot && currentSnapshot.priority !== snapshot.priority,
    },
    {
      key: "status",
      label: t("routines.history.field.status", { defaultValue: "Status" }),
      value: snapshot.status,
      differs: !!currentSnapshot && currentSnapshot.status !== snapshot.status,
    },
    {
      key: "assigneeAgentId",
      label: t("routines.history.field.defaultAgent", { defaultValue: "Default agent" }),
      value: resolveAgentName(snapshot.assigneeAgentId, agents, t("routines.unassigned", { defaultValue: "Unassigned" })),
      differs: !!currentSnapshot && currentSnapshot.assigneeAgentId !== snapshot.assigneeAgentId,
    },
    {
      key: "projectId",
      label: t("routines.history.field.project", { defaultValue: "Project" }),
      value: resolveProjectName(snapshot.projectId, projects, t("routines.noProject", { defaultValue: "No project" })),
      differs: !!currentSnapshot && currentSnapshot.projectId !== snapshot.projectId,
    },
    {
      key: "concurrencyPolicy",
      label: t("routines.history.field.concurrency", { defaultValue: "Concurrency" }),
      value: t(`routines.concurrencyOption.${snapshot.concurrencyPolicy}.title`, { defaultValue: snapshot.concurrencyPolicy.replaceAll("_", " ") }),
      differs: !!currentSnapshot && currentSnapshot.concurrencyPolicy !== snapshot.concurrencyPolicy,
    },
    {
      key: "catchUpPolicy",
      label: t("routines.history.field.catchUp", { defaultValue: "Catch-up" }),
      value: t(`routines.catchUpOption.${snapshot.catchUpPolicy}.title`, { defaultValue: snapshot.catchUpPolicy.replaceAll("_", " ") }),
      differs: !!currentSnapshot && currentSnapshot.catchUpPolicy !== snapshot.catchUpPolicy,
    },
    {
      key: "env",
      label: t("routines.history.field.env", { defaultValue: "Env" }),
      value: envSummary,
      differs: envDiffers,
    },
  ];
    ...
    <div className="space-y-4">
      <header className={`${cardWrapper} p-4 space-y-2`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1 min-w-0">
            <p className="text-sm font-medium">{t("routines.history.revisionShort", { defaultValue: "rev {{number}}", number: revision.revisionNumber })}</p>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" size="sm" onClick={onCompare}>
              ...
              {t("routines.history.compareCurrent", { defaultValue: "Compare with current" })}
            </Button>
            <Button
              size="sm"
              onClick={onRestore}
      ...
      <div className={`${cardWrapper} p-3`}>
        <p className="pb-2 text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("routines.history.structuredFields", { defaultValue: "Structured fields" })}
        </p>
              ...
              <p className="text-sm">
                ...
                {row.differs && (
                  <Badge variant="outline" className="ml-2 border-amber-500/40 bg-amber-500/10 px-1.5 text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow) text-amber-800 dark:text-amber-200">
                    {t("routines.history.differsFromCurrent", { defaultValue: "differs from current" })}
                  </Badge>
                )}
              </p>
            </div>
      ...
      <div className={`${cardWrapper} p-3 space-y-2`}>
        <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("routines.history.description", { defaultValue: "Description" })}
        </p>
        <div className="rounded-md bg-background/40 p-3 text-sm leading-7">
          {snapshot.description ? (
            <MarkdownBody>{snapshot.description}</MarkdownBody>
          ) : (
            <span className="text-muted-foreground">{t("routines.history.noDescription", { defaultValue: "No description" })}</span>
          )}
        </div>
      </div>
      ...
      <div className={`${cardWrapper} p-3 space-y-2`}>
        <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("routines.history.triggersCount", { defaultValue: "Triggers ({{count}})", count: triggers.length })}
        </p>
        {triggers.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("routines.history.noTriggers", { defaultValue: "No triggers in this revision." })}</p>
        ) : (
          <ul className="divide-y divide-border">
            {triggers.map((trigger) => (
              <li key={trigger.id} className="py-2 flex flex-wrap items-center gap-2 text-sm">
                ...
                >
                  {trigger.enabled ? t("routines.enabled", { defaultValue: "enabled" }) : t("routines.disabled", { defaultValue: "disabled" })}
                </span>
              </li>
            ))}
          </ul>
        ...
        <p className="text-xs text-muted-foreground">
          {t("routines.history.webhookSecretDescription", { defaultValue: "Webhook secrets are not stored in revisions. If a restored webhook trigger needs re-creation, Paperclip mints fresh secret material at restore time." })}
        </p>
      </div>
      ...
      {snapshot.variables.length > 0 && (
        <div className={`${cardWrapper} p-3 space-y-2`}>
          <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("routines.history.variablesCount", { defaultValue: "Variables ({{count}})", count: snapshot.variables.length })}
          </p>
          <ul className="divide-y divide-border">
            {snapshot.variables.map((variable) => (
              <li key={variable.name} className="py-2 flex items-center justify-between text-sm">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("routines.history.variableDefault", { defaultValue: "default: {{value}}", value: formatVariableDefault(variable) })}
                </span>
              </li>
            ))}
          </ul>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t("routines.history.restoreRevisionQuestion", { defaultValue: "Restore revision {{number}}?", number: target.revisionNumber })}</DialogTitle>
          <DialogDescription>
            {t("routines.history.restoreRevisionDescription", { defaultValue: "This creates a new revision {{newNumber}} with the same content as revision {{targetNumber}}. Revisions {{targetNumber}}–{{currentNumber}} stay in history and are not modified.", newNumber: newRevisionNumber, targetNumber: target.revisionNumber, currentNumber: currentRevisionNumber })}
          </DialogDescription>
        </DialogHeader>
        <ul className="space-y-2 text-sm">
          <li className="flex items-start gap-2">
            <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
              {t("routines.history.restoreFields", { defaultValue: "Routine field values, variables, and schedule cron will revert." })}
          </li>
          {envDiffCounts.total > 0 && (
            <li className="flex items-start gap-2">
              <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
              {t("routines.history.restoreSecrets", { defaultValue: "Routine secrets will revert: {{counts}}.", counts: formatEnvDiffCounts(envDiffCounts) })}
            </li>
          )}
          <li className="flex items-start gap-2">
            <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
            {t("routines.history.preserveRuns", { defaultValue: "Previous run history is preserved." })}
          </li>
          {recreatedWebhookLabels.map((label) => (
            <li key={label} className="flex items-start gap-2 text-amber-800 dark:text-amber-200">
              ...
              {t("routines.history.recreateWebhook", { defaultValue: "The webhook trigger {{label}} will be recreated with a new URL and secret. Paperclip will show the secret once after restore — copy it before closing.", label })}
            </li>
          ))}
        </ul>
        <div className="space-y-1.5">
          <Label htmlFor="restore-change-summary" className="text-xs">
            {t("routines.history.changeSummaryOptional", { defaultValue: "Change summary (optional)" })}
          </Label>
          <Input
            id="restore-change-summary"
            value={changeSummary}
            placeholder={t("routines.history.changeSummaryPlaceholder", { defaultValue: "Why are you restoring? Visible in history." })}
        ...
        <DialogFooter>
          ...
          <Button onClick={onConfirm} disabled={pending}>
            <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
            {pending ? t("routines.history.restoring", { defaultValue: "Restoring…" }) : t("routines.history.restoreAsRevision", { defaultValue: "Restore as revision {{number}}", number: newRevisionNumber })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-(--pct-90) w-full max-h-(--sz-85vh) overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>{t("routines.history.compareRevisions", { defaultValue: "Compare routine revisions" })}</DialogTitle>
        </DialogHeader>
        <div className="flex flex-wrap items-center gap-3">
          <RevisionPicker
            label={t("routines.history.old", { defaultValue: "Old" })}
            value={leftId}
            onChange={setLeftId}
            revisions={revisions}
            tone="red"
          />
          <RevisionPicker
            label={t("routines.history.new", { defaultValue: "New" })}
            value={rightId}
            onChange={setRightId}
            revisions={revisions}
            tone="green"
        ...
        <div className="overflow-auto flex-1 space-y-4">
          <section className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("routines.history.fieldChanges", { defaultValue: "Field changes" })}
            </p>
            {fieldChanges.length === 0 ? (
              <p className="text-sm text-muted-foreground">{t("routines.history.noFieldChanges", { defaultValue: "No structural field changes." })}</p>
            ) : (
              <table className="w-full text-sm border border-border rounded-md overflow-hidden">
                <thead>
                  <tr className="text-xs uppercase tracking-wide bg-muted/30 text-muted-foreground">
                    <th className="px-3 py-2 text-left">{t("routines.history.field", { defaultValue: "Field" })}</th>
                    <th className="px-3 py-2 text-left">{t("routines.history.oldValue", { defaultValue: "Old value" })}</th>
                    <th className="px-3 py-2 text-left">{t("routines.history.newValue", { defaultValue: "New value" })}</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <section className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("routines.history.descriptionDiff", { defaultValue: "Description diff" })}
            </p>
        ...
        <DialogFooter className="justify-between sm:justify-between">
          ...
          {leftIsHistorical && left && (
            <Button onClick={() => onRestore(left)}>
              <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
              {t("routines.history.restoreRevAsNew", { defaultValue: "Restore rev {{number}} as new revision", number: left.revisionNumber })}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
      ...
      >
        {revisions.map((revision) => (
          <option key={revision.id} value={revision.id}>
            {t("routines.history.revisionOption", { defaultValue: "rev {{number}} — {{time}}", number: revision.revisionNumber, time: relativeTime(revision.createdAt) })}
            {revision.changeSummary ? ` • ${revision.changeSummary}` : ""}
          </option>
        ))}
      </select>
...
function DiffTable({ rows }: { rows: DiffRow[] }) {
  const { t } = useTranslation();
  if (rows.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("routines.history.noDescriptionEither", { defaultValue: "No description on either revision." })}</p>;
  }
  if (rows.every((row) => row.kind === "context")) {
    return <p className="text-sm text-muted-foreground">{t("routines.history.descriptionsIdentical", { defaultValue: "Descriptions are identical." })}</p>;
  }
  ...
  return (
    <div className="rounded-md border border-border text-xs font-mono leading-6 overflow-hidden">
      <div className="grid grid-cols-(--gtc-1) border-b border-border/60 bg-muted/30 px-3 py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        <span>{t("routines.history.old", { defaultValue: "Old" })}</span>
        <span>{t("routines.history.new", { defaultValue: "New" })}</span>
        <span />
        <span>{t("routines.history.content", { defaultValue: "Content" })}</span>
      </div>
      {rows.map((row, index) => (
        <div
...
function resolveAgentName(agentId: string | null, lookup: AgentLookup, fallback = t("routines.unassigned", { defaultValue: "Unassigned" })) {
  if (!agentId) return fallback;
  return lookup.get(agentId)?.name ?? agentId;
}

function resolveProjectName(projectId: string | null, lookup: ProjectLookup, fallback = t("routines.noProject", { defaultValue: "No project" })) {
...
function summarizeTriggerSnapshot(trigger: RoutineRevisionSnapshotTriggerV1): string {
  ...
  if (trigger.kind === "webhook") {
    const replay = trigger.replayWindowSec != null ? t("routines.history.replayWindow", { defaultValue: "replay {{seconds}}s", seconds: trigger.replayWindowSec }) : "";
    return [trigger.signingMode, replay].filter(Boolean).join(" · ");
  }
  return t("routines.api", { defaultValue: "API" });
}
...
function formatDirtyFieldList(labels: string[]): string {
  if (labels.length === 0) return t("routines.history.theRoutine", { defaultValue: "the routine" });
  if (labels.length === 1) return labels[0];
  const and = t("common.and", { defaultValue: "and" });
...
function computeFieldChanges(
  ...
  compareScalar("title", t("routines.history.field.title", { defaultValue: "Title" }), oldRoutine.title, newRoutine.title);
  compareScalar("priority", t("routines.history.field.priority", { defaultValue: "Priority" }), oldRoutine.priority, newRoutine.priority);
  compareScalar(
    "assigneeAgentId",
    t("routines.history.field.defaultAgent", { defaultValue: "Default agent" }),
    resolveAgentName(oldRoutine.assigneeAgentId, agents),
    resolveAgentName(newRoutine.assigneeAgentId, agents),
  );
  compareScalar(
    "projectId",
    t("routines.history.field.project", { defaultValue: "Project" }),
    resolveProjectName(oldRoutine.projectId, projects),
    resolveProjectName(newRoutine.projectId, projects),
  );
  compareScalar("concurrencyPolicy", t("routines.history.field.concurrency", { defaultValue: "Concurrency" }), oldRoutine.concurrencyPolicy, newRoutine.concurrencyPolicy, (value) => t(`routines.concurrencyOption.${String(value)}.title`, { defaultValue: String(value).replaceAll("_", " ") }));
  compareScalar("catchUpPolicy", t("routines.history.field.catchUp", { defaultValue: "Catch-up" }), oldRoutine.catchUpPolicy, newRoutine.catchUpPolicy, (value) => t(`routines.catchUpOption.${String(value)}.title`, { defaultValue: String(value).replaceAll("_", " ") }));
  compareScalar("status", t("routines.history.field.status", { defaultValue: "Status" }), oldRoutine.status, newRoutine.status, (value) => t(`routines.status.${String(value)}`, { defaultValue: String(value) }));
  if (JSON.stringify(oldRoutine.variables) !== JSON.stringify(newRoutine.variables)) {
    changes.push({
      field: t("routines.history.field.variables", { defaultValue: "Variables" }),
...
function describeSecretRef(ref: EnvSecretRefBinding, secrets: SecretLookup): string {
  ...
  const name = secret?.name ?? t("routines.history.missingSecret", { defaultValue: "<missing-secret>" });
...
function describeEnvBinding(binding: EnvBinding | undefined, secrets: SecretLookup): string {
  ...
  if (ref) return t("routines.history.secretRefValue", { defaultValue: "secret_ref → {{value}}", value: describeSecretRef(ref, secrets) });
  return t("routines.history.plainSet", { defaultValue: "plain (set)" });
}
...
function summarizeEnv(env: RoutineEnvConfig | null): string {
  ...
  const keyLabel = entries.length === 1 ? "" : "s";
  if (secretCount === 0) return t("routines.history.envSummary", { defaultValue: "{{count}} key{{plural}}", count: entries.length, plural: keyLabel });
  return t("routines.history.envSummaryWithSecrets", { defaultValue: "{{count}} key{{plural}} ({{secretCount}} secret {{refPlural}})", count: entries.length, plural: keyLabel, secretCount, refPlural: secretCount === 1 ? "ref" : "refs" });
}
...
function formatEnvDiffCounts(counts: EnvDiffCounts): string {
  const parts: string[] = [];
  if (counts.added > 0) parts.push(t("routines.history.envAdded", { defaultValue: "{{count}} key{{plural}} added", count: counts.added, plural: counts.added === 1 ? "" : "s" }));
  if (counts.removed > 0) parts.push(t("routines.history.envRemoved", { defaultValue: "{{count}} key{{plural}} removed", count: counts.removed, plural: counts.removed === 1 ? "" : "s" }));
  if (counts.changed > 0) parts.push(t("routines.history.envChanged", { defaultValue: "{{count}} key{{plural}} changed", count: counts.changed, plural: counts.changed === 1 ? "" : "s" }));
  return parts.join(t("common.commaSeparator", { defaultValue: ", " }));
}
...
function compareEnv(
  ...
  for (const key of sortedKeys) {
    ...
    if (inNew && !inOld) {
      changes.push({
        field: t("routines.history.envFieldAdded", { defaultValue: "Env added ({{key}})", key }),
        oldValue: "—",
    ...
    if (!inNew && inOld) {
      changes.push({
        field: t("routines.history.envFieldRemoved", { defaultValue: "Env removed ({{key}})", key }),
    ...
    if (oldKind !== newKind) {
      changes.push({
        field: t("routines.history.envFieldBindingKind", { defaultValue: "Env {{key}} binding kind", key }),
        oldValue: describeEnvBinding(oldBinding, secrets),
        newValue: describeEnvBinding(newBinding, secrets),
      });
      continue;
    ...
    if (newKind === "secret_ref") {
      ...
      if (oldRef.secretId !== newRef.secretId) {
        changes.push({
          field: t("routines.history.envFieldSecret", { defaultValue: "Env {{key}} secret", key }),
          oldValue: describeEnvBinding(oldBinding, secrets),
          newValue: describeEnvBinding(newBinding, secrets),
        });
        continue;
      ...
      changes.push({
        field: t("routines.history.envFieldVersion", { defaultValue: "Env {{key}} version", key }),
    ...
    changes.push({
      field: t("routines.history.envFieldValue", { defaultValue: "Env {{key}} value", key }),
      oldValue: t("routines.history.plainSet", { defaultValue: "plain (set)" }),
      newValue: t("routines.history.plainChanged", { defaultValue: "plain (changed)" }),
    });
  }
}
...
function compareTriggers(
  ...
  for (const [, pair] of byId) {
    if (pair.old && !pair.next) {
      changes.push({
        field: t("routines.history.triggerRemoved", { defaultValue: "Trigger removed ({{label}})", label: pair.old.label ?? pair.old.kind }),
        oldValue: summarizeTriggerSnapshot(pair.old),
        newValue: null,
      });
    } else if (!pair.old && pair.next) {
      changes.push({
        field: t("routines.history.triggerAdded", { defaultValue: "Trigger added ({{label}})", label: pair.next.label ?? pair.next.kind }),
        oldValue: null,
      ...
      if (oldSummary !== newSummary || pair.old.enabled !== pair.next.enabled) {
        changes.push({
          field: t("routines.history.triggerField", { defaultValue: "Trigger {{label}}", label: pair.next.label ?? pair.next.kind }),
          oldValue: t("routines.history.triggerStateValue", { defaultValue: "{{summary}} ({{state}})", summary: oldSummary, state: pair.old.enabled ? t("routines.enabled", { defaultValue: "enabled" }) : t("routines.disabled", { defaultValue: "disabled" }) }),
          newValue: t("routines.history.triggerStateValue", { defaultValue: "{{summary}} ({{state}})", summary: newSummary, state: pair.next.enabled ? t("routines.enabled", { defaultValue: "enabled" }) : t("routines.disabled", { defaultValue: "disabled" }) }),
        });
      }
    }
  }
