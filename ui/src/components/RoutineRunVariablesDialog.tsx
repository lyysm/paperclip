    ...
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="flex h-(--sz-calc-18) max-h-(--sz-calc-18) max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-(--sz-calc-20)">
        <DialogHeader className="shrink-0 border-b border-border/60 px-6 pb-4 pr-12 pt-6">
          ...
          <DialogTitle>{t("routines.runRoutine", { defaultValue: "Run routine" })}</DialogTitle>
          <DialogDescription>
            {t("routines.runRoutineDescription", { defaultValue: "Choose the agent and optional project for this one run. Routine defaults are prefilled and won't be changed." })}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routines.agentRequired", { defaultValue: "Agent *" })}</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("routines.agent", { defaultValue: "Agent" })}
                noneLabel={t("routines.selectAgent", { defaultValue: "Select an agent" })}
                searchPlaceholder={t("routines.searchAgents", { defaultValue: "Search agents..." })}
                emptyMessage={t("routines.noAgentsFound", { defaultValue: "No agents found." })}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("routines.selectAgent", { defaultValue: "Select an agent" })}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routines.project", { defaultValue: "Project" })}</Label>
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("routines.noProject", { defaultValue: "No project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routines.noValue", { defaultValue: "No value" })}</SelectItem>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("routines.chooseValue", { defaultValue: "Choose a value" })} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routines.noValue", { defaultValue: "No value" })}</SelectItem>
        ...
        >
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("routines.defaultAgentRequired", { defaultValue: "Default agent required for this run." })}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routines.missingVariables", { defaultValue: "Missing: {{variables}}", variables: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routines.chooseWorkspaceBeforeRun", { defaultValue: "Choose an existing workspace before running." })}
            </p>
          ) : (
          ...
          >
            {isPending ? t("routines.running", { defaultValue: "Running..." }) : t("routines.runRoutine", { defaultValue: "Run routine" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
