  ...
  return (
    <>
      ...
      >
        {saveConflict ? (
          <div className="flex items-center gap-2 text-sm text-amber-800 dark:text-amber-200">
            ...
            <span>{t("routines.changedElsewhere", { defaultValue: "Routine changed elsewhere. Reload to merge." })}</span>
          </div>
        ) : (
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                ...
                <span className="font-medium">
                  {t("routines.unsavedChangesCount", { defaultValue: "{{count}} unsaved change{{plural}}", count: dirtyCount, plural: dirtyCount === 1 ? "" : "s" })}
                </span>
              </button>
            </PopoverTrigger>
            <PopoverContent align="start" className="w-64">
              <p className="mb-2 text-xs font-medium text-muted-foreground">
                {t("routines.pendingChanges", { defaultValue: "Pending changes" })}
              </p>
          ...
          {saveConflict ? (
            <>
              <Button variant="outline" size="sm" onClick={onReload}>
                {t("routines.reloadLatest", { defaultValue: "Reload latest" })}
              </Button>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      ...
                      {t("routines.overwriteAnyway", { defaultValue: "Overwrite anyway" })}
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    {t("routines.overwriteDescription", { defaultValue: "Replaces the newer revision with your local edits." })}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </>
            ...
            <>
              ...
              >
                {t("routines.discard", { defaultValue: "Discard" })}
              </Button>
              <Button
                size="sm"
                disabled={isSaving || disabled}
              ...
              >
                {isSaving ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : null}
                {t("routines.saveChanges", { defaultValue: "Save changes" })}
      ...
      <Dialog open={confirmDiscardOpen} onOpenChange={setConfirmDiscardOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("routines.discardChangesQuestion", { defaultValue: "Discard changes?" })}</DialogTitle>
            <DialogDescription>
              {t("routines.discardChangesDescription", { defaultValue: "This will revert {{count}} unsaved change{{plural}} in this section.", count: dirtyCount, plural: dirtyCount === 1 ? "" : "s" })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="ghost" size="sm" onClick={() => setConfirmDiscardOpen(false)}>
              {t("routines.keepEditing", { defaultValue: "Keep editing" })}
            </Button>
            <Button
              variant="destructive"
              size="sm"
            ...
            >
              {t("routines.discardChanges", { defaultValue: "Discard changes" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function RoutineReadOnlyStrip() {
  ...
  return (
    <div className="-mx-8 mt-6 border-t border-border bg-muted/20 px-8 py-3 text-xs text-muted-foreground">
      {t("routines.readOnly", { defaultValue: "Read-only — you don't own this routine." })}
    </div>
  );
}
