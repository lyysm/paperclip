...
export function RoutineTriggerCard({
  ...
  return (
    <form
      aria-label={t("routines.triggerAria", { defaultValue: "Trigger: {{label}}", label: trigger.label ?? trigger.kind })}
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            {trigger.kind === "schedule" && trigger.nextRunAt
              ? t("routines.nextTrigger", { defaultValue: "Next: {{time}}", time: new Date(trigger.nextRunAt).toLocaleString() })
              : trigger.kind === "webhook"
                ? t("routines.webhook", { defaultValue: "Webhook" })
                : t("routines.api", { defaultValue: "API" })}
          </span>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        {trigger.kind === "schedule" && (
          <div className="space-y-1.5 md:col-span-2">
            <Label className="text-xs">{t("routines.scheduleLabel", { defaultValue: "Schedule" })}</Label>
            <ScheduleEditor
      ...
      {!disabled && (
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          >
            ...
            {t("routines.delete", { defaultValue: "Delete" })}
          </Button>
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("routines.rotateSecret", { defaultValue: "Rotate secret" })}
            </Button>
          )}
          <Button
            variant="outline"
          ...
          >
            ...
            {t("routines.saveTrigger", { defaultValue: "Save trigger" })}
          </Button>
        </div>
      )}
    </form>
