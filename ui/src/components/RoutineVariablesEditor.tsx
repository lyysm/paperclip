  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("routines.variables", { defaultValue: "Variables" })}</p>
          <p className="text-xs text-muted-foreground">
            {t("routines.variablesDetected", { defaultValue: "Detected from {{placeholder}} placeholders in the title and instructions.", placeholder: "{{name}}" })}
          </p>
        </div>
          ...
          <div key={variable.name} className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("routines.promptVariableBeforeRun", { defaultValue: "Prompt the user for this value before each manual run." })}
              </span>
            </div>

            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routines.label", { defaultValue: "Label" })}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routines.type", { defaultValue: "Type" })}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{t("routines.defaultValue", { defaultValue: "Default value" })}</Label>
                  <label className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    {t("routines.required", { defaultValue: "Required" })}
                  </label>
                </div>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{t("routines.noDefault", { defaultValue: "No default" })}</SelectItem>
                      <SelectItem value="true">{t("routines.true", { defaultValue: "True" })}</SelectItem>
                      <SelectItem value="false">{t("routines.false", { defaultValue: "False" })}</SelectItem>
                    </SelectContent>
                  </Select>
                ) : variable.type === "select" ? (
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routines.options", { defaultValue: "Options" })}</Label>
                      <Input
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routines.defaultOption", { defaultValue: "Default option" })}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t("routines.noDefault", { defaultValue: "No default" })} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{t("routines.noDefault", { defaultValue: "No default" })}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : t("routines.defaultValue", { defaultValue: "Default value" })}
                  />
                )}
              </div>
            </div>
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        <span>
          {t("routines.variablesHint", { defaultValue: "Use {{placeholder}} placeholders in the title or instructions to prompt for inputs when the routine runs.", placeholder: "{{variable_name}}" })}
        </span>
        <button
          type="button"
          ...
          aria-label={t("routines.showVariableHelp", { defaultValue: "Show variable help" })}
        >
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("routines.routineVariables", { defaultValue: "Routine variables" })}</DialogTitle>
            <DialogDescription>
              {t("routines.routineVariablesDescription", { defaultValue: "How to prompt for inputs and which variables Paperclip fills in automatically." })}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
                {t("routines.customVariables", { defaultValue: "Custom variables" })}
              </h3>
              <p className="text-muted-foreground">
                {t("routines.typeIn", { defaultValue: "Type" })}{" "}
                <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs text-foreground">
                  {"{{variable_name}}"}
                </code>{" "}
                {t("routines.customVariablesDescription", { defaultValue: "anywhere in the title or instructions. Paperclip detects each placeholder, lists it under Variables, and prompts for a value before each run." })}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t("routines.variableRuleName", { defaultValue: "Names must start with a letter and may use letters, numbers, and underscores." })}</li>
                <li>{t("routines.variableRuleType", { defaultValue: "Pick a type (text, textarea, number, boolean, select, date), default value, and whether it is required." })}</li>
                <li>{t("routines.variableRuleDate", { defaultValue: "Variable names ending in capital Date, such as startDate, are created as date variables by default." })}</li>
                <li>{t("routines.variableRuleReuse", { defaultValue: "The same name reused across the title and instructions is treated as one variable." })}</li>
              </ul>
            </section>

            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
                {t("routines.builtinVariables", { defaultValue: "Built-in variables" })}
              </h3>
              <p className="text-muted-foreground">
                {t("routines.builtinVariablesDescription", { defaultValue: "These are filled in automatically — no setup needed and they will not appear in the Variables list." })}
              </p>
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t("routines.placeholder", { defaultValue: "Placeholder" })}</th>
                      <th className="px-3 py-2 font-medium">{t("routines.example", { defaultValue: "Example" })}</th>
                      <th className="px-3 py-2 font-medium">{t("routines.description", { defaultValue: "Description" })}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/70">
                    {BUILTIN_VARIABLE_DOCS.map((entry) => (
                      <tr key={entry.name} className="align-top">
                        ...
                        <td className="px-3 py-2 text-muted-foreground">{t(`routines.builtin.${entry.name}.description`, { defaultValue: entry.description })}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
