...
export function StageSecretsPanel({
  ...
  if (!hasAutomation) {
    return (
      <EmptyState
        icon={KeyRound}
        message={t("pipelineSettings.secretsNoAutomation", {
          defaultValue: "Secrets are available only to step automation. Pick an agent to run this step, then add the secrets it needs.",
        })}
        action={t("pipelineSettings.secretsSetUpAutomation", { defaultValue: "Set up automation" })}
  ...
  const displayName = agentName?.trim()
    ?? t("pipelineSettings.secretsFallbackAgent", { defaultValue: "the responsible agent" });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-2 rounded-md border border-border bg-muted/20 px-4 py-3 text-xs text-muted-foreground">
        ...
        <p>
          {t("pipelineSettings.secretsEnvHintBefore", {
            defaultValue: "These env vars are injected when",
          })}{" "}
          <span className="font-medium text-foreground">{displayName}</span>{" "}
          {t("pipelineSettings.secretsEnvHintMid", {
          ...
          {t("pipelineSettings.secretsEnvHintAfter", {
      ...
      {secretsLoading ? (
        <p className="text-sm text-muted-foreground">
          {t("pipelineSettings.secretsLoading", { defaultValue: "Loading secrets…" })}
        </p>
      ) : (
      ...
      <div className="flex items-center gap-3">
        <Button type="button" onClick={onSave} disabled={!dirty || saving}>
          ...
          {saving
            ? t("pipelineSettings.secretsSaving", { defaultValue: "Saving…" })
            : t("pipelineSettings.secretsSaveButton", { defaultValue: "Save secrets" })}
        </Button>
        {dirty && !saving ? (
          <span className="text-xs text-muted-foreground">
            {t("pipelineSettings.secretsUnsaved", { defaultValue: "Unsaved changes" })}
          </span>
        ) : null}
      </div>
    </div>
