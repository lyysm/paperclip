...
function statusMeta(entry: WorkspaceServiceControlEntry): { label: string; unhealthy: boolean } {
  switch (entry.state) {
    case "provisioning":
      return { label: translate("workspaceServices.provisioning"), unhealthy: false };
    case "starting":
      return { label: translate("workspaceServices.starting"), unhealthy: false };
    case "stopping":
      return { label: translate("workspaceServices.stopping"), unhealthy: false };
    case "restarting":
      return { label: translate("workspaceServices.restarting"), unhealthy: false };
    case "failed":
      return { label: translate("workspaceServices.failed"), unhealthy: false };
    case "running":
      return entry.healthStatus === "unhealthy"
        ? { label: translate("workspaceServices.unhealthy"), unhealthy: true }
        : { label: translate("workspaceServices.running"), unhealthy: false };
    default:
      return { label: translate("workspaceServices.stopped"), unhealthy: false };
  }
}
...
function CopyUrlButton({ url, disabled }: { url: string; disabled?: boolean }) {
  ...
  const copyLabel = copyState === "copied"
    ? t("workspaceServices.urlCopied")
    : copyState === "failed"
      ? t("workspaceServices.copyFailed")
      : t("workspaceServices.copyUrl");
  return (
    <Button
      variant="ghost"
      size="icon-xs"
...
function UrlSegment({ entry, compact }: { entry: WorkspaceServiceControlEntry; compact?: boolean }) {
  ...
  if (!displayUrl) {
    return <span className="font-mono text-xs text-muted-foreground/70">{t("workspaceServices.noUrl")}</span>;
  }
  return (
    <>
      {live ? (
      ...
      <span className={cn("flex items-center", live ? null : "invisible")} aria-hidden={live ? undefined : true}>
        ...
        <Button
          ...
          disabled={!live}
          className="text-muted-foreground hover:text-foreground"
          title={t("workspaceServices.openInNewTab")}
        >
          {live ? (
            <a href={entry.url ?? undefined} target="_blank" rel="noreferrer" aria-label={t("workspaceServices.openInNewTab")}>
  ...
  if (entry.state === "stopped") {
    return (
      <Button
        ...
        aria-label={t("workspaceServices.start")}
        title={t("workspaceServices.start")}
      >
        <Play className="size-3" />
        {t("workspaceServices.start")}
      </Button>
    );
  }
    ...
    return (
      <>
        ...
        <Button
          ...
          aria-label={t("workspaceServices.restart")}
          title={t("workspaceServices.restart")}
  ...
  return (
    <>
      <Button
        ...
        aria-label={t("workspaceServices.stop")}
        title={t("workspaceServices.stop")}
        className="border border-border text-foreground"
      >
...
function ServicePopoverRow({
  ...
  const secondary = entry.exposureDetail
    ...
    : entry.state === "starting" && entry.port
      ? t("workspaceServices.startingOnPort", { port: entry.port })
...
function MultiServiceBar({
  ...
  const aggregateEntry: WorkspaceServiceControlEntry = {
    key: "__all__",
    name: t("workspaceServices.allServices"),
        ...
        <div className="flex h-9 items-center pl-3 pr-1.5">
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <button
                ...
                aria-label={t("workspaceServices.runningOf", {
              ...
              >
                ...
                <span className="whitespace-nowrap">
                  {t("workspaceServices.runningCount", {
                    running: runningCount,
                    total: services.length,
                  })}
                </span>
            ...
            <PopoverContent align="end" className="w-96 p-0" onOpenAutoFocus={(event) => event.preventDefault()}>
              <div className="px-4 pb-1 pt-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                {t("workspaceServices.servicesCount", { count: services.length })}
              </div>
              ...
              <div className="flex items-center gap-1 border-t border-border px-4 py-2">
                <Button variant="ghost" size="xs" onClick={() => onAction("start", null)}>{t("workspaceServices.startAll")}</Button>
                <Button variant="ghost" size="xs" onClick={() => onAction("stop", null)}>{t("workspaceServices.stopAll")}</Button>
                <Button variant="ghost" size="xs" onClick={() => onAction("restart", null)}>{t("workspaceServices.restartAll")}</Button>
                {onManageServices ? (
                  <Button
                    variant="link"
                    size="xs"
                  ...
                  >
                    {t("workspaceServices.manageServices")} →
                  </Button>
                ) : null}
              </div>
            </PopoverContent>
          ...
          <div className="hidden min-w-0 items-center gap-0.5 sm:flex">
            {primary ? (
              ...
              <span className="font-mono text-xs text-muted-foreground/70">{t("workspaceServices.noUrl")}</span>
            )}
          </div>
          <div className="mx-3 hidden h-5 w-px bg-border sm:block" />
          <div className="ml-auto flex items-center gap-1 pl-3 sm:pl-0">
