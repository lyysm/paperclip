...
function ImagePreview({ artifact }: { artifact: CompanyArtifact }) {
  ...
  if (errored || !artifact.contentPath) {
    return <PlaceholderPreview label={t("artifacts.media.image", { defaultValue: "Image" })} />;
  }
  return (
    <PreviewFrame>
      <img
...
function TextPreview({ artifact }: { artifact: CompanyArtifact }) {
  ...
  if (!preview) {
    return (
      <PlaceholderPreview
        label={t(`artifacts.media.${artifact.source === "document" ? "document" : "text"}`, {
          defaultValue: artifact.source === "document" ? "Document" : "Text",
        })}
      />
    );
  }
...
export function ArtifactPreview({ artifact }: { artifact: CompanyArtifact }) {
  ...
  switch (artifact.mediaKind) {
    ...
    case "file":
      return <PlaceholderPreview label={t("artifacts.media.file", { defaultValue: "File" })} />;
    case "empty":
    default:
          ...
          <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
            {artifact.openPath ? (
              <SecondaryAction
                ...
                title={t("artifacts.openFile", { defaultValue: "Open file in new tab" })}
              >
            ...
            {artifact.downloadPath ? (
              <SecondaryAction
                ...
                download
                title={t("artifacts.downloadFile", { defaultValue: "Download file" })}
              >
