...
function buildFolderGroup(
  ...
  labels: { upOneFolder: string; browseSecrets: string },
...
export function SecretPicker({
  ...
  const groups = useMemo<SearchableSelectGroup<string, SecretOption>[]>(() => {
    ...
    if (boundMissing) {
      result.push({
        id: "current-missing",
        label: t("agentConfig.environmentEditor.current"),
        options: [
          {
            key: `missing-${secretId}`,
            value: secretId,
            label: t("agentConfig.environmentEditor.missingSecret", { id: secretId.slice(0, 8) }),
            title: t("agentConfig.environmentEditor.missingSecretFull", { id: secretId }),
            missing: true,
            disabled: true,
          },
        ],
    ...
    if (recent.length > 0) {
      result.push({
        id: "recently-used",
        label: t("agentConfig.environmentEditor.recentlyUsed"),
    ...
    result.push({
      id: "all-secrets",
      label: recent.length > 0 ? t("agentConfig.environmentEditor.allSecrets") : undefined,
  ...
  const deriveGroups = useCallback(
    (query: string, baseGroups: readonly SearchableSelectGroup<string, SecretOption>[]) => {
      ...
      const browseGroup = buildFolderGroup(secrets, currentPath, secretId, {
        upOneFolder: t("agentConfig.environmentEditor.upOneFolder"),
        browseSecrets: t("agentConfig.environmentEditor.browseSecrets"),
      });
  ...
  return (
    <SearchableSelect<string, SecretOption>
      ...
      placeholder={t("agentConfig.environmentEditor.selectSecret")}
      searchPlaceholder={t("agentConfig.environmentEditor.searchSecrets")}
      emptyMessage={t("agentConfig.environmentEditor.noMatchingSecrets")}
      triggerClassName={cn(
      ...
      renderValue={(option) => {
        if (!option) {
          return <span className="text-muted-foreground">{t("agentConfig.environmentEditor.selectSecret")}</span>;
        }
        if (option.missing) {
          return (
        ...
        ? {
            render: (query) => (
              <span className="flex items-center gap-1.5 text-sm">
                ...
                {query.trim() ? (
                  <span>
                    {t("agentConfig.environmentEditor.createSecret")} <span className="font-mono">&ldquo;{query.trim()}&rdquo;</span>…
                  </span>
                ) : (
                  <span>{t("agentConfig.environmentEditor.createNewSecret")}</span>
                )}
              </span>
            ),
