...
export const EnvironmentVariablesEditor = forwardRef<EnvironmentVariablesEditorHandle, EnvironmentVariablesEditorProps>(function EnvironmentVariablesEditor({
  ...
  const changeSummaryText = useMemo(() => {
    ...
    if (changeSummary.added.length > 0) {
      parts.push(t("agentConfig.environmentEditor.changeSummaryAdded", { names: formatChangedNames(changeSummary.added) }));
    }
    if (changeSummary.changed.length > 0) {
      parts.push(t("agentConfig.environmentEditor.changeSummaryEdited", { names: formatChangedNames(changeSummary.changed) }));
    }
    if (changeSummary.removed.length > 0) {
      parts.push(t("agentConfig.environmentEditor.changeSummaryRemoved", { names: formatChangedNames(changeSummary.removed) }));
    }
    return parts.join(" · ");
  }, [changeSummary, t]);
  ...
  function bulkImport(text: string, targetRowId: string): boolean {
    ...
    updateDraft(working);
    toast?.pushToast({ title: t("agentConfig.environmentEditor.importedVariables", { count: pairs.length }), tone: "success" });
    return true;
  }
  ...
  const hint = footerHint === undefined
    ? t("agentConfig.environmentEditor.defaultHint", { defaultValue: DEFAULT_HINT })
    : footerHint;
    ...
    <TooltipProvider>
      ...
      {attentionCount > 1 ? (
        <p className="inline-flex items-center gap-1.5 text-(length:--text-micro) font-medium text-amber-700 dark:text-amber-400">
          ...
          {t("agentConfig.environmentEditor.bindingsNeedAttention", { count: attentionCount })}
        </p>
      ) : null}
      ...
      {hasRows ? (
        <>
          ...
          <div className="hidden gap-x-1.5 @[40rem]/env:grid @[40rem]/env:grid-cols-(--gtc-14)">
            <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("agentConfig.environmentEditor.name")}</span>
            <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("agentConfig.environmentEditor.value")}</span>
            <span />
          </div>
        ...
        <p className="text-sm text-muted-foreground">{t("agentConfig.environmentEditor.noVariables")}</p>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
        ...
        >
          ...
          {t("agentConfig.environmentEditor.addVariable")}
        </button>
        ...
        {quickBind.length > 0 && !disabled ? (
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-muted-foreground/70">
              ...
              {t("agentConfig.environmentEditor.recentlyUsed")}:
            </span>
            {quickBind.map((secret) => (
              <button
                ...
                title={t("agentConfig.environmentEditor.bindSecret", { name: secret.name })}
              >
                + {secret.name}
              </button>
            ))}
        ...
        >
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              <span>{t("agentConfig.environmentEditor.unsavedChanges")}</span>
            </div>
            {changeSummaryText ? (
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {t("agentConfig.environmentEditor.revert")}
            </button>
            <button
              type="button"
              onClick={saveDraft}
            ...
            >
              ...
              {t("agentConfig.environmentEditor.save")}
            </button>
          </div>
        </div>
      ) : null}
      ...
      {rows.some((row) => row.source === "user_secret" && row.userSecretKey) ? (
        <p className="inline-flex items-start gap-1 text-(length:--text-micro) text-muted-foreground/70">
          ...
          <span>
            {t("agentConfig.environmentEditor.userSecretsHint")}
          </span>
        </p>
      ) : null}
      </div>
