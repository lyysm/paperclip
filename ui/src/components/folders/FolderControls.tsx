...
function selectionLabel({
  ...
  if (selection === "unfiled") return translate("folders.unfiled", { defaultValue: "Unfiled" });
...
export function FolderRail({
  ...
  return (
    <nav
      aria-label={t("folders.itemFolders", { defaultValue: "{{items}} folders", items: itemLabelPlural })}
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
          {t("folders.folders", { defaultValue: "Folders" })}
        </div>
        <Button
          variant="ghost"
          size="icon-sm"
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("folders.folderActionsFor", {
              defaultValue: "Folder actions for {{label}}",
              label: folder.name,
            })}
          >
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-sm">
      ...
      <Button size="sm" variant="outline" onClick={onCreateFolder}>
        {t("folders.createFirstFolder", { defaultValue: "Create your first folder" })}
      </Button>
      <Button
        size="icon-sm"
        variant="ghost"
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-(--sz-folder-sheet-max) rounded-t-lg pb-4">
        <SheetHeader className="border-b border-border px-4 py-3">
          <SheetTitle>{t("folders.itemFolders", { defaultValue: "{{items}} folders", items: itemLabelPlural })}</SheetTitle>
        </SheetHeader>
...
export function MoveToMenu({
  ...
  return (
    <DropdownMenuSub>
      <DropdownMenuSubTrigger>
        {t("folders.moveToDots", { defaultValue: "Move to..." })}
      </DropdownMenuSubTrigger>
    ...
    <>
        <div className="flex items-center gap-2 px-2 py-1.5">
          ...
          <input
            ...
            placeholder={t("folders.searchFolders", { defaultValue: "Search folders" })}
        ...
        {visibleFolders.length === 0 ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">
            {t("folders.noFoldersMatch", { defaultValue: "No folders match." })}
          </div>
        ) : null}
        <DropdownMenuSeparator />
        <DropdownMenuItem onSelect={onCreateAndMove}>
          <Plus className="h-3.5 w-3.5" />
          {t("folders.newFolderDots", { defaultValue: "New folder..." })}
        </DropdownMenuItem>
    </>
  );
}
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="folder-name">
              {t("folders.nameLabel", { defaultValue: "Name" })}
            </label>
            <Input
              id="folder-name"
              value={name}
          ...
          <div className="space-y-2">
            <div className="text-sm font-medium">{t("folders.colorLabel", { defaultValue: "Color" })}</div>
            <div className="flex flex-wrap gap-2">
              {FOLDER_COLORS.map((swatch) => (
                <button
                  key={swatch}
                  type="button"
                  aria-label={t("folders.useFolderColor", {
              ...
              >
                {t("folders.none", { defaultValue: "None" })}
              </button>
            </div>
          </div>
        </div>
        <DialogFooter>
          ...
          <Button onClick={() => onSubmit({ name: name.trim(), color })} disabled={pending || !name.trim()}>
            {pending
              ? t("folders.savingDots", { defaultValue: "Saving..." })
              : isEdit
                ? t("folders.save", { defaultValue: "Save" })
                : t("folders.createFolder", { defaultValue: "Create folder" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t("folders.deleteFolderDescription", {
              defaultValue:
                "The {{n}} {{items}} in this folder won't be deleted. They'll move to Unfiled.",
              n: folder?.itemCount ?? 0,
              items: itemLabelPlural,
            })}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          ...
          >
            {pending
              ? t("folders.deletingDots", { defaultValue: "Deleting..." })
  ...
  return (
    <div className="sticky top-2 z-10 flex flex-wrap items-center gap-2 rounded-md border border-border bg-background/95 px-3 py-2 shadow-sm backdrop-blur">
      <span className="mr-auto text-sm text-muted-foreground">
        {t("folders.selectedCount", { defaultValue: "{{n}} selected", n: selectedCount })}
      </span>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
