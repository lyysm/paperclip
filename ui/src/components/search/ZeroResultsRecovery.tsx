  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        <div className="text-base font-semibold">{t("searchPage.zeroResults.title", { defaultValue: "No results with these filters" })}</div>
        <p className="text-sm text-muted-foreground">
          {t("searchPage.zeroResults.matchCount", { defaultValue: "{{count}} results match", count: unfilteredTotal })}
          {query ? <> &ldquo;{query}&rdquo;</> : null}
          {t("searchPage.zeroResults.butYour", { defaultValue: ", but your" })}{" "}
          {t("searchPage.zeroResults.activeFiltersHide", { defaultValue: "{{count}} active filters hide", count: activeCount })}{" "}
          {t("searchPage.zeroResults.allOfThem", { defaultValue: "all of them." })}
        </p>
      </div>
      ...
      {suggestions.length > 0 ? (
        <div className="flex w-full flex-col gap-1.5">
          <div className="text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
            {t("searchPage.zeroResults.loosenTitle", { defaultValue: "Loosen a filter" })}
          </div>
          {suggestions.map((suggestion) => (
            ...
            >
              ...
              <span className="shrink-0 tabular-nums text-emerald-600 dark:text-emerald-400">
                {t("searchPage.zeroResults.additionalResults", { defaultValue: "+{{count}} results", count: suggestion.additionalCount })}
              </span>
            </button>
          ))}
        </div>
      ...
      <Button onClick={onClearAll} variant="default" size="sm">
        ...
        {t("searchPage.zeroResults.clearAllFilters", { defaultValue: "Clear all filters" })}
      </Button>
    </div>
  );
}
