    ...
    <section className="rounded-lg border border-border bg-card">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-3 py-2.5">
        <div>
          <h3 className="text-sm font-semibold text-foreground">
            {t("agentTools.installedApps.title", { defaultValue: "Installed apps" })}
          </h3>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t("agentTools.installedApps.description", {
              defaultValue: "Installed apps load tools into {{name}}'s context on every run. Permitted-only apps do not add context cost.",
              name: agentName,
            })}
          </p>
        </div>
      ...
      <div className="space-y-3 p-3">
        <InlineBanner tone="info" compact>
          {t("agentTools.installedApps.accessAndInstallInfo", {
            defaultValue: "Has access means the app is permitted. Installed means its tools are added to this agent's runtime context.",
          })}
        </InlineBanner>

        {connections.length === 0 ? (
          <p className="rounded-md border border-border bg-muted/30 px-3 py-4 text-sm text-muted-foreground">
            {t("agentTools.installedApps.noPermittedApps", {
                ...
                <div key={connection.id} className="space-y-2 px-3 py-3">
                  <label className="flex items-start gap-3">
                    <Checkbox
                      ...
                      aria-label={t("agentTools.installedApps.installOnAgent", {
                      ...
                      <span className="flex flex-wrap items-center gap-2">
                        ...
                        {rowPending ? (
                          <span className="text-xs text-muted-foreground">
                            {t("agentTools.status.saving", { defaultValue: "Saving..." })}
                          </span>
                        ) : null}
                      </span>
                      <span className="mt-0.5 block text-xs text-muted-foreground">
                        {installedForAll
                          ? t("agentTools.installedApps.installedForAllHint", {
                              defaultValue: "Installed from the app page for every agent. Remove the all-agents install there.",
                            })
                          : checked
                            ? t("agentTools.installedApps.loadedHint", {
                  ...
                  {permitted && !checked ? (
                    <InlineBanner
                      ...
                      actions={(
                        ...
                        >
                          {t("agentTools.installedApps.openPermissions", { defaultValue: "Open permissions" })}
                        </Link>
                      )}
                    >
                      {t("agentTools.installedApps.permittedNotInstalled", {
...
function InstallBadge({
  ...
  const label = installed
    ? installedForAll
      ? t("agentTools.badges.installedForAll", { defaultValue: "Installed for all" })
      : t("agentTools.badges.installed", { defaultValue: "Installed" })
    : permitted
      ? t("agentTools.badges.permittedOnly", { defaultValue: "Permitted only" })
      : t("agentTools.badges.notPermitted", { defaultValue: "Not permitted" });
  return (
    <span
      className={cn(
...
function InstallSaveStatusChip({
  ...
  const { t } = useTranslation();
  if (pending) {
    return <span className="text-xs text-muted-foreground">{t("agentTools.status.saving", { defaultValue: "Saving..." })}</span>;
  }
  if (error) {
    return <span className="text-xs text-destructive">{t("agentTools.status.couldNotSave", { defaultValue: "Could not save" })}</span>;
  }
  if (unsaved) {
    return <span className="text-xs text-muted-foreground">{t("agentTools.status.unsavedChanges", { defaultValue: "Unsaved changes" })}</span>;
  }
  return <span className="text-xs text-muted-foreground">{t("agentTools.status.saved", { defaultValue: "Saved" })}</span>;
}

const POLICY_EFFECT_DEFAULT: Record<string, string> = {
  allow: "allow",
  block: "block",
  deny: "deny",
...
function policyEffectLabel(policyType: string): string {
  return t(`agentTools.policyEffects.${policyType}`, {
    defaultValue: POLICY_EFFECT_DEFAULT[policyType] ?? policyType,
  });
}
...
export function AgentToolsTab({ agent, companyId }: { agent: AgentDetailRecord; companyId: string }) {
  ...
  if (effective.isLoading) {
    return (
      <ToolsLoadingState
        label={t("agentTools.loading.resolvingAccess", { defaultValue: "Resolving effective access…" })}
      />
    );
  }
  if (effective.error) {
  ...
  return (
    <div className="space-y-4">
      <EnforcementBanner
        tone="info"
        title={t("agentTools.effectiveAccess.title", { defaultValue: "Effective access" })}
        body={t("agentTools.effectiveAccess.body", {
          defaultValue: "This is exactly the tool set Paperclip will accept for {{name}}. Profile and policy edits are reflected within ~5 seconds. The agent's prompt can narrow this list but cannot expand it — everything else is blocked by default.",
          name: agent.name,
        })}
      />
        ...
        <div className="lg:col-span-2">
          <div className="rounded-lg border border-border">
            <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2.5">
              <h3 className="text-sm font-semibold text-foreground">
                {t("agentTools.allowedTools.title", { defaultValue: "Allowed tools" })}
              </h3>
              <span className="text-xs text-muted-foreground tabular-nums">
                {t("agentTools.allowedTools.count", {
                  defaultValue: "{{count}} {{unit}}",
                  count: allowedTools.length,
                  unit: t(allowedTools.length === 1 ? "agentTools.allowedTools.tool" : "agentTools.allowedTools.tools", {
                    defaultValue: allowedTools.length === 1 ? "tool" : "tools",
                  }),
                })}
              </span>
            </div>
            {allowedTools.length === 0 ? (
              <p className="px-3 py-6 text-sm text-muted-foreground">
                {t("agentTools.allowedTools.empty", {
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-xs text-muted-foreground">
                    <th className="px-3 py-2 font-medium">{t("agentTools.allowedTools.toolHeader", { defaultValue: "Tool" })}</th>
                    <th className="px-3 py-2 font-medium">{t("agentTools.allowedTools.capabilityHeader", { defaultValue: "Capability" })}</th>
                    <th className="px-3 py-2 font-medium">{t("agentTools.allowedTools.riskHeader", { defaultValue: "Risk" })}</th>
                    <th className="px-3 py-2 font-medium">{t("agentTools.allowedTools.sourceHeader", { defaultValue: "Source" })}</th>
                  </tr>
                </thead>
        ...
        <aside className="space-y-4">
          <div className="rounded-lg border border-border bg-background/60 p-3">
            <h3 className="flex items-center gap-1.5 text-sm font-semibold text-foreground">
              ...
              {t("agentTools.whyTheseTools", { defaultValue: "Why these tools?" })}
            </h3>
            ...
            <div className="mt-3 space-y-1.5">
              <div className="flex items-center justify-between gap-2">
                <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                  {t("agentTools.accessProfiles", { defaultValue: "Access profiles" })}
                </div>
                <Link
                  to={`${profilesHref}?check=1`}
                  className="text-(length:--text-micro) font-medium text-primary hover:underline"
                >
                  {t("agentTools.checkAccess", { defaultValue: "Check access" })}
                </Link>
              </div>
              {profiles.length === 0 ? (
                      ...
                      {profile.summary.isCompanyDefault ? (
                        <div className="mt-1">
                          <span className="rounded border border-border px-1.5 py-0.5 text-(length:--text-nano) uppercase text-muted-foreground">
                            {t("agentTools.companyDefault", { defaultValue: "Company default" })}
                          </span>
                        </div>
                      ) : null}
                    </div>
            ...
            <div className="mt-3 space-y-1.5">
              <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                {t("agentTools.activePolicies", { defaultValue: "Active policies" })}
              </div>
              {policiesQuery.isLoading ? (
                <p className="text-xs text-muted-foreground">
                  {t("agentTools.loading.policies", { defaultValue: "Loading policies…" })}
                </p>
                  ...
                  <div key={policy.id} className="rounded-md border border-border/70 px-2.5 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <Link
                        ...
                        title={t("agentTools.policyTitle", {
                      ...
                      <span className="shrink-0 rounded border border-border px-1.5 py-0.5 text-(length:--text-nano) uppercase text-muted-foreground">
                        {policyEffectLabel(policy.policyType)}
                      </span>
                    </div>
                    {policy.description ? (
            ...
            <div className="mt-3 space-y-1.5">
              <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                {t("agentTools.unavailableTools", { defaultValue: "Unavailable tools" })}
              </div>
              {catalogLoading ? (
                <p className="text-xs text-muted-foreground">
                  {t("agentTools.loading.tools", { defaultValue: "Checking tools…" })}
                </p>
              ) : deniedTools.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {t("agentTools.allKnownToolsAllowed", {
