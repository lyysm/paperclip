...
function agentFilterTabItems(): { value: FilterTab; label: string }[] {
  return [
    { value: "all", label: t("searchPage.all", { defaultValue: "All" }) },
    { value: "active", label: t("issuesList.quickFilter.active", { defaultValue: "Active" }) },
    { value: "paused", label: t("issuesList.paused", { defaultValue: "Paused" }) },
    { value: "error", label: t("agentsPage.errorTab", { defaultValue: "Error" }) },
    { value: "builtin", label: t("agentsPage.builtinTab", { defaultValue: "Built-in" }) },
  ];
}
...
function localEnvironmentDescriptor(): EnvironmentDescriptor {
  const label = t("accountMenu.localBadge", { defaultValue: "Local" });
  const detail = t("agentsPage.paperclipHost", { defaultValue: "Paperclip host" });
  return {
    label,
    detail,
    title: t("agentsPage.environmentTitle", { defaultValue: "{{label}} - {{detail}}", label, detail }),
  };
}

function loadingEnvironmentDescriptor(): EnvironmentDescriptor {
  const detail = t("agentsPage.loadingEnvironment", { defaultValue: "Loading environment" });
  return {
    label: "—",
    detail,
    title: detail,
...
function describeEnvironment(
  ...
  const detail = isPlatformManagedEnvironment(environment)
    ? t("agentsPage.managedByPaperclip", { defaultValue: "Managed by Paperclip" })
    : environment.driver === "sandbox"
      ? t("agentsPage.sandboxProvider", {
        defaultValue: "{{provider}} sandbox provider",
        provider: getSandboxProviderLabel(environment, capabilities),
      })
      : environment.driver === "local"
        ? t("agentsPage.paperclipHost", { defaultValue: "Paperclip host" })
        : formatEnvironmentDriver(environment.driver);

  return {
    label: environment.name,
    detail,
    title: t("agentsPage.environmentTitle", {
...
function describeMissingEnvironment(environmentId: string): EnvironmentDescriptor {
  const label = t("agentsPage.unknownEnvironment", { defaultValue: "Unknown environment" });
  return {
    label,
...
export function Agents() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message={t("agentsPage.selectCompany", { defaultValue: "Select a company to view agents." })} />;
  }
      ...
      <>
        ...
        {builtInState.status === "needs_setup" && (
          ...
          >
            ...
            >
              {t("agentsPage.setUp", { defaultValue: "Set up" })}
            </Button>
          </span>
        )}
      </>
            ...
            <div className="hidden sm:flex items-center gap-3">
              ...
              >
                <AgentActionButtons
                  agent={agent}
                  companyId={selectedCompanyId}
                  runLabel={t("agentsPage.runHeartbeat", { defaultValue: "Run Heartbeat" })}
                  showStatus={false}
                />
              </div>
              <StarToggle
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          {/* View toggle */}
          {!forceListView && (
            <div className="flex items-center border border-border" role="group" aria-label={t("issuesList.viewMode", { defaultValue: "View mode" })}>
              <button
                className={cn(
              ...
              <button
                ...
                onClick={() => setView("org")}
                title={t("agentsPage.orgChartView", { defaultValue: "Org chart view" })}
                aria-label={t("agentsPage.orgChartView", { defaultValue: "Org chart view" })}
          ...
          <Button size="sm" variant="outline" onClick={openNewAgent}>
            ...
            {t("agentsPage.newAgent", { defaultValue: "New Agent" })}
          </Button>
        </div>
      </div>

      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{t("agentsPage.agentCount", { defaultValue: "{{count}} agents", count: filtered.length })}</p>
      )}
      ...
      {agents && agents.length === 0 && (
        <EmptyState
          icon={Bot}
          message={t("agentsPage.createFirstAgent", { defaultValue: "Create your first agent to get started." })}
          action={t("agentsPage.newAgent", { defaultValue: "New Agent" })}
      ...
      {effectiveView === "list" && agents && agents.length > 0 && filtered.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          {t("agentsPage.noAgentsMatch", { defaultValue: "No agents match the selected status." })}
        </p>
      )}
      ...
      {effectiveView === "org" && orgTree && orgTree.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          {t("agentsPage.noOrgHierarchy", { defaultValue: "No organizational hierarchy defined." })}
        </p>
      )}
  ...
  return (
    ...
    >
      ...
      <span className="text-(length:--text-micro) font-medium text-blue-600 dark:text-blue-400">
        {liveCount > 1
          ? t("agentsPage.liveRunsCount", { defaultValue: "Live ({{count}})", count: liveCount })
