...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approvalDetail.breadcrumbFallback", { defaultValue: "Approval" }) },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.approveFailed", { defaultValue: "Approve failed" })),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.rejectFailed", { defaultValue: "Reject failed" })),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.revisionRequestFailed", { defaultValue: "Revision request failed" })),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.resubmitFailed", { defaultValue: "Resubmit failed" })),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.commentFailed", { defaultValue: "Comment failed" })),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.deleteFailed", { defaultValue: "Delete failed" })),
  });
  ...
  const resolvedCta =
    primaryLinkedIssue
      ? {
          label: t("approvalDetail.reviewLinkedTask", {
            defaultValue: (linkedIssues?.length ?? 0) > 1 ? "Review linked tasks" : "Review linked task",
            count: linkedIssues?.length ?? 1,
          }),
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("approvalDetail.confirmedTitle", { defaultValue: "Approval confirmed" })}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvalDetail.confirmedBody", { defaultValue: "Requesting agent was notified to review this approval and linked tasks." })}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("approvalDetail.linkedTasks", { defaultValue: "Linked Tasks" })}</p>
            ...
            <p className="text-(length:--text-micro) text-muted-foreground mt-2">
              {t("approvalDetail.linkedTasksNote", { defaultValue: "Linked tasks remain open until the requesting agent follows up and closes them." })}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approvalDetail.budgetStopResolve", { defaultValue: "Resolve this budget stop from the budget controls on" })} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("approvalDetail.commentsTitle", { defaultValue: "Comments ({{count}})", count: comments?.length ?? 0 })}</h3>
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={t("issuesList.boardUser", { defaultValue: "Board" })} size="sm" />
                )}
        ...
        <Textarea
          ...
          placeholder={t("approvalDetail.commentPlaceholder", { defaultValue: "Add a comment..." })}
          rows={3}
        />
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("approvalDetail.posting", { defaultValue: "Posting…" }) : t("approvalDetail.postComment", { defaultValue: "Post comment" })}
          </Button>
        </div>
      </div>
    </div>
