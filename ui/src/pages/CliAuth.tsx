...
export function CliAuthPage() {
      ...
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          <h1 className="text-lg font-semibold">
            {t("cliAuth.unavailableTitle", { defaultValue: "CLI auth challenge unavailable" })}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {challengeQuery.error instanceof Error
              ? challengeQuery.error.message
              : t("cliAuth.invalidOrExpired", { defaultValue: "Challenge is invalid or expired." })}
          </p>
        </Card>
      </div>
    );
  ...
  if (!challenge) {
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-destructive">
        {t("cliAuth.unavailableFallback", { defaultValue: "CLI auth challenge unavailable." })}
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          <h1 className="text-xl font-semibold">
            {t("cliAuth.approvedTitle", { defaultValue: "CLI access approved" })}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            {t("cliAuth.command", { defaultValue: "Command" })}: {" "}
        ...
        <Card className="block p-6">
          <h1 className="text-xl font-semibold">
            {challenge.status === "expired"
              ? t("cliAuth.expiredTitle", { defaultValue: "CLI auth challenge expired" })
              : t("cliAuth.cancelledTitle", { defaultValue: "CLI auth challenge cancelled" })}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
  ...
  if (challenge.requiresSignIn || !sessionQuery.data) {
    return (
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          <h1 className="text-xl font-semibold">
            {t("cliAuth.signInRequiredTitle", { defaultValue: "Sign in required" })}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
          ...
          <Button asChild className="mt-4">
            <Link to={`/auth?next=${encodeURIComponent(currentPath)}`}>
              {t("cliAuth.signInCreateAccount", { defaultValue: "Sign in / Create account" })}
            </Link>
          </Button>
        </Card>
      </div>
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <Card className="block p-6">
        <h1 className="text-xl font-semibold">
          {t("cliAuth.approveTitle", { defaultValue: "Approve Paperclip CLI access" })}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
        ...
        <div className="mt-5 space-y-3 text-sm">
          ...
          <div>
            ...
            <div className="text-foreground">
              {challenge.clientName ?? t("cliAuth.defaultClientName", { defaultValue: "paperclipai cli" })}
            </div>
          </div>
          <div>
            <div className="text-muted-foreground">
              {t("cliAuth.requestedAccess", { defaultValue: "Requested access" })}
            </div>
            <div className="text-foreground">
          ...
          {challenge.requestedCompanyName && (
            <div>
              <div className="text-muted-foreground">
                {t("cliAuth.requestedCompany", { defaultValue: "Requested company" })}
              </div>
        ...
        <div className="mt-5 flex gap-3">
          ...
          >
            {cancelMutation.isPending
              ...
              : t("cliAuth.cancel", { defaultValue: "Cancel" })}
          </Button>
        </div>
      </Card>
    </div>
