...
const EXPORT_CATEGORY_LABEL_KEYS: Record<ExportCategoryKey, string> = {
  agents: "companyExport.category.agents",
  projects: "companyExport.category.projects",
  skills: "companyExport.category.skills",
  routines: "companyExport.category.routines",
  tasks: "companyExport.category.tasks",
  attachments: "companyExport.category.attachments",
};
...
function ExportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState
        icon={Package}
        message={t("companyExport.selectFileToPreview", {
          defaultValue: "Select a file to preview its contents.",
        })}
      />
    );
  }
...
function previewErrorMessage(error: unknown): string {
  return error instanceof Error
    ? error.message
    : t("companyExport.previewLoadFailed", { defaultValue: "Failed to load export data." });
}
...
export function CompanyExport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("companyExport.breadcrumb", { defaultValue: "Export" }) },
    ]);
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err, request) => {
      ...
      pushToast({
        tone: "error",
        title: t("companyExport.exportFailedTitle", { defaultValue: "Export failed" }),
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyExport.exportDownloadedTitle", { defaultValue: "Export downloaded" }),
        body: t("companyExport.exportDownloadedBody", {
          defaultValue: "{{count}} files exported as {{name}}.",
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error
          ? err.message
          : t("companyExport.exportBuildFailed", { defaultValue: "Failed to build export package." }),
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return (
      <EmptyState
        icon={Package}
        message={t("companyExport.selectCompany", { defaultValue: "Select a company to export." })}
      />
    );
  }
  ...
  if (previewCancelled && !exportData) {
    return (
      <EmptyState
        icon={Package}
        title={t("companyExport.previewCancelledTitle", { defaultValue: "Export preview cancelled" })}
        message={t("companyExport.previewCancelledMessage", {
          defaultValue: "The preview request was cancelled. Your export settings are unchanged.",
        })}
        action={t("companyExport.retryPreview", { defaultValue: "Retry preview" })}
  ...
  if (exportPreviewMutation.isError && !exportData) {
    return (
      <EmptyState
        icon={Package}
        title={t("companyExport.previewFailedTitle", { defaultValue: "Export preview failed" })}
        message={previewErrorMessage(exportPreviewMutation.error)}
        description={t("companyExport.previewFailedDescription", {
          defaultValue: "Retry the preview. You do not need to reload this page.",
        })}
        action={t("companyExport.retryPreview", { defaultValue: "Retry preview" })}
        onAction={startPreviewRequest}
        hideActionIcon
  ...
  if (!exportData) {
    return (
      <EmptyState
        icon={Package}
        title={t("companyExport.previewUnavailableTitle", { defaultValue: "Export preview unavailable" })}
        message={t("companyExport.previewUnavailableMessage", { defaultValue: "No export preview is loaded." })}
        action={t("companyExport.loadPreview", { defaultValue: "Load preview" })}
        onAction={startPreviewRequest}
        hideActionIcon
      />
    );
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="font-medium">
              {t("companyExport.exportTitle", {
                defaultValue: "{{name}} export",
                name: selectedCompany?.name ?? t("nav.company", { defaultValue: "Company" }),
              })}
            </span>
            <span className="text-muted-foreground">
              {t("companyExport.exportingCount", {
                defaultValue: "Exporting {{selected}} of {{total}} files",
            ...
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {t("companyExport.warningsCount", { defaultValue: "{{count}} warnings", count: warnings.length })}
              </span>
            )}
          </div>
          <Button
          ...
          >
            ...
            {downloadMutation.isPending
              ...
              : t("companyExport.exportButton", { defaultValue: "Export {{count}} files", count: selectedCount })}
          </Button>
        </div>
      </div>
      ...
      {fidelityReport && fidelityReport.warnings.length > 0 && (
        <div className="mx-5 mt-3 rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3">
          <h3 className="mb-1.5 text-xs font-medium">
            {t("companyExport.notIncludedHeading", { defaultValue: "Not included in this export" })}
          </h3>
        ...
        <aside className="flex max-h-(--sz-24rem) flex-col overflow-hidden border-b border-border xl:max-h-none xl:border-b-0 xl:border-r">
          <div className="border-b border-border px-4 py-3 shrink-0">
            <h2 className="text-base font-semibold">
              {t("companyExport.packageFiles", { defaultValue: "Package files" })}
            </h2>
          </div>
          <div className="border-b border-border px-4 py-3 shrink-0">
            <h3 className="mb-2 text-xs font-medium text-muted-foreground">
              {t("companyExport.whatToInclude", { defaultValue: "What to include" })}
            </h3>
            <div
              className="grid grid-cols-2 gap-x-4 gap-y-1.5"
              role="group"
              aria-label={t("companyExport.whatToInclude", { defaultValue: "What to include" })}
            >
              {EXPORT_CATEGORY_ORDER.map((key) => {
                ...
                return (
                  <label
                    ...
                    title={
                      disabled
                        ? t("companyExport.attachmentsDisabledHint", {
                            defaultValue:
                  ...
                  >
                    ...
                    <span className="min-w-0 truncate">
                      {t(EXPORT_CATEGORY_LABEL_KEYS[key], { defaultValue: EXPORT_CATEGORY_LABELS[key] })}
                    </span>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              {t("companyExport.historyOptInHint", {
                defaultValue: "Task and routine history is opt-in because it can be large.",
              })}
            </p>
          </div>
          <div className="border-b border-border px-3 py-2 shrink-0">
            <div className="flex items-center gap-2 rounded-md border border-border px-2 py-1">
              ...
              <input
                ...
                placeholder={t("companyExport.searchFilesPlaceholder", { defaultValue: "Search files..." })}
          ...
          {exportPreviewMutation.isPending ? (
              ...
              <div className="flex max-w-md flex-col items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium">
                    {t("companyExport.updatingPreviewTitle", { defaultValue: "Updating export preview…" })}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t("companyExport.updatingPreviewHint", {
                      defaultValue: "Large task histories can take a minute. You can untick Tasks or cancel this update.",
                    })}
                  </p>
                </div>
                <Button type="button" variant="outline" size="sm" onClick={handleCancelPreview}>
                  <X />
                  {t("companyExport.cancelUpdate", { defaultValue: "Cancel update" })}
                </Button>
              </div>
            </div>
          ) : previewCancelled ? (
            ...
            >
              <div className="flex max-w-md flex-col items-center gap-3">
                <div>
                  <p className="text-sm font-medium">
                    {t("companyExport.previewUpdateCancelledTitle", { defaultValue: "Preview update cancelled" })}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t("companyExport.previewUpdateCancelledHint", {
                      defaultValue: "The previous preview remains available. Retry when you are ready.",
                    })}
                  </p>
                </div>
                <Button type="button" variant="outline" size="sm" onClick={startPreviewRequest}>
                  <RotateCcw />
                  {t("companyExport.retryPreview", { defaultValue: "Retry preview" })}
                </Button>
              </div>
            </div>
          ) : exportPreviewMutation.isError ? (
              ...
              <div className="flex max-w-md flex-col items-center gap-3">
                <div>
                  <p className="text-sm font-medium text-destructive">
                    {t("companyExport.previewFailedTitle", { defaultValue: "Export preview failed" })}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
