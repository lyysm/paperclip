...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    case "skills_sh":
      return { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: t("companySkills.managedSkillsSh", { defaultValue: "skills.sh managed" }) };
    case "github":
      return isSkillsShManaged
        ? { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: t("companySkills.managedSkillsSh", { defaultValue: "skills.sh managed" }) }
        : { icon: Github, label: sourceLabel ?? "GitHub", managedLabel: t("companySkills.managedGithub", { defaultValue: "GitHub managed" }) };
    case "url":
      return { icon: Link2, label: sourceLabel ?? "URL", managedLabel: t("companySkills.managedUrl", { defaultValue: "URL managed" }) };
    case "local":
      return { icon: Folder, label: sourceLabel ?? t("companySkills.sourceFolder", { defaultValue: "Folder" }), managedLabel: t("companySkills.managedFolder", { defaultValue: "Folder managed" }) };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Paperclip", managedLabel: t("companySkills.managedPaperclip", { defaultValue: "Paperclip managed" }) };
    default:
      return { icon: Boxes, label: sourceLabel ?? t("companySkills.sourceCatalog", { defaultValue: "Catalog" }), managedLabel: t("companySkills.managedCatalog", { defaultValue: "Catalog managed" }) };
  }
}
...
function formatProjectScanSummary(result: CompanySkillProjectScanResult) {
  const parts = [
    t("companySkills.scanFound", { defaultValue: "{{count}} found", count: result.discovered }),
    t("companySkills.scanImported", { defaultValue: "{{count}} imported", count: result.imported.length }),
    t("companySkills.scanUpdated", { defaultValue: "{{count}} updated", count: result.updated.length }),
  ];
  if (result.conflicts.length > 0) parts.push(t("companySkills.scanConflicts", { defaultValue: "{{count}} conflicts", count: result.conflicts.length }));
  if (result.skipped.length > 0) parts.push(t("companySkills.scanSkipped", { defaultValue: "{{count}} skipped", count: result.skipped.length }));
  return `${parts.join(", ")} ${t("companySkills.scanAcrossWorkspace", { defaultValue: "across {{count}} workspace", count: result.scannedWorkspaces })}.`;
}
...
function sourceFilterLabels(): Record<SourceFilter, string> {
  return {
    all: t("companySkills.filterAll", { defaultValue: "All" }),
    company: t("companySkills.filterCompany", { defaultValue: "Company" }),
    bundled: t("companySkills.filterBundled", { defaultValue: "Bundled" }),
    optional: t("companySkills.filterOptional", { defaultValue: "Optional" }),
    external: t("companySkills.filterExternal", { defaultValue: "External" }),
  };
}
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={activeFilterCount > 0 ? t("companySkills.filtersTitle", { defaultValue: "Filters: {{count}}", count: activeFilterCount }) : t("companySkills.filterTitle", { defaultValue: "Filter" })}
        >
      ...
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuLabel>{t("companySkills.source", { defaultValue: "Source" })}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={value} onValueChange={(next) => onChange(next as SourceFilter)}>
          {filters.map((filter) => (
            <DropdownMenuRadioItem key={filter} value={filter}>
              <span>{sourceFilterLabels()[filter]}</span>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="max-h-(--sz-calc-32) w-56 overflow-y-auto">
        <DropdownMenuLabel>{t("companySkills.filterTypeLabel", { defaultValue: "Type" })}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={kindFilter} onValueChange={(next) => onKindChange(next as "all" | "bundled" | "optional")}>
          <DropdownMenuRadioItem value="all">{t("companySkills.filterAll", { defaultValue: "All" })}</DropdownMenuRadioItem>
          <DropdownMenuRadioItem value="bundled">{t("companySkills.filterBundled", { defaultValue: "Bundled" })}</DropdownMenuRadioItem>
          <DropdownMenuRadioItem value="optional">{t("companySkills.filterOptional", { defaultValue: "Optional" })}</DropdownMenuRadioItem>
        </DropdownMenuRadioGroup>
        <DropdownMenuSeparator />
        <DropdownMenuLabel>{t("companySkills.filterCategoryLabel", { defaultValue: "Category" })}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={categoryFilter || "__all__"} onValueChange={(next) => onCategoryChange(next === "__all__" ? "" : next)}>
          <DropdownMenuRadioItem value="__all__">{t("companySkills.filterAllCategories", { defaultValue: "All categories" })}</DropdownMenuRadioItem>
...
function TrustChip({ level }: { level: CompanySkillTrustLevel }) {
  ...
  const map = {
    markdown_only: {
      icon: ShieldCheck,
      label: t("companySkills.trustMarkdownOnly", { defaultValue: "Markdown only" }),
      tooltip: t("companySkills.trustMarkdownOnlyTooltip", { defaultValue: "Text only — no scripts, no binaries, no assets." }),
      className: "border-border bg-muted/40 text-muted-foreground",
    },
    assets: {
      icon: Folder,
      label: t("companySkills.trustAssets", { defaultValue: "Includes assets" }),
      tooltip: t("companySkills.trustAssetsTooltip", { defaultValue: "Ships images, fonts, or other non-script files." }),
      className: "border-cyan-500/30 bg-cyan-500/10 text-cyan-800 dark:text-cyan-200",
    },
    scripts_executables: {
      icon: AlertTriangle,
      label: t("companySkills.trustScripts", { defaultValue: "Includes scripts" }),
      tooltip: t("companySkills.trustScriptsTooltip", { defaultValue: "Ships executable scripts. Review before installing." }),
...
function CompatChip({ compatibility }: { compatibility: CompanySkillCompatibility }) {
  ...
  const map = {
    unknown: {
      icon: HelpCircle,
      label: t("companySkills.compatUnknown", { defaultValue: "Unknown format" }),
      tooltip: t("companySkills.compatUnknownTooltip", { defaultValue: "Paperclip could not validate this skill as Agent Skills markdown. Install at your own risk." }),
      className: "border-yellow-500/40 bg-yellow-500/10 text-yellow-800 dark:text-yellow-200",
    },
    invalid: {
      icon: XOctagon,
      label: t("companySkills.compatInvalid", { defaultValue: "Invalid" }),
      tooltip: t("companySkills.compatInvalidTooltip", { defaultValue: "This skill cannot be installed — content is not valid Agent Skills markdown." }),
...
function ProvenanceBadge({ packageName, packageVersion }: { packageName: string | null; packageVersion: string | null }) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>{t("companySkills.provenanceTooltip", { defaultValue: "Installed from the app-shipped skills catalog. Provenance is signed by package version and content hash." })}</TooltipContent>
    </Tooltip>
  );
}
...
export function skillDetailBreadcrumbs(
  ...
  const trail = detail.folderId
    ...
    : [];
  return [
    { label: t("nav.skills", { defaultValue: "Skills" }), href: "/skills" },
...
function discoverySortLabels(): Record<DiscoverySort, string> {
  return {
    agents: t("companySkills.sortMostAgents", { defaultValue: "Most agents" }),
    stars: t("companySkills.sortMostStars", { defaultValue: "Most stars" }),
    forks: t("companySkills.sortMostForks", { defaultValue: "Most forks" }),
    recent: t("companySkills.sortRecent", { defaultValue: "Recently updated" }),
    alphabetical: t("companySkills.sortAlphabetical", { defaultValue: "Alphabetical" }),
  };
}
...
function discoveryVersionLabel(skill: {
  ...
  if (required) return t("companySkills.core", { defaultValue: "core" });
...
function skillSettingsToastBody(skill: Pick<CompanySkillDetail, "categories" | "sharingScope">) {
  const sharing = skill.sharingScope === "private"
    ? t("companySkills.sharingPrivateLabel", { defaultValue: "Sharing: private" })
    : t("companySkills.sharingCompanyLabel", { defaultValue: "Sharing: company" });
  const categories = skill.categories.length
    ? t("companySkills.categoriesWithValue", { defaultValue: "Categories: {{value}}", value: skill.categories.join(", ") })
    : t("companySkills.categoriesNone", { defaultValue: "Categories: none" });
...
function buildDiscoveryCards(
  ...
  for (const skill of installed) {
    ...
    cards.push({
      ...
      name: skill.name,
      slug: skill.slug,
      author: skill.authorName ?? skill.sourceLabel ?? t("companySkills.authorYou", { defaultValue: "you" }),
  ...
  for (const entry of catalog) {
    ...
    cards.push({
      ...
      updatedAt: 0,
      sourceBadge: "catalog",
      sourceLabel: entry.packageName ?? t("companySkills.sourceCatalog", { defaultValue: "Catalog" }),
    });
  }
    ...
    >
      <div className="flex items-start gap-3">
        {selectMode && canMove ? (
          <input
            ...
            checked={selected}
            aria-label={t("companySkills.selectSkillAria", { defaultValue: "Select {{name}}", name: card.name })}
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-muted-foreground">
            {t("companySkills.byAuthor", {
              defaultValue: "by {{author}}{{version}}",
              author: card.author,
              version: card.version ? ` · ${card.version}` : "",
            })}
          </div>
          {badgeFolder !== undefined ? (
            <div className="mt-1 flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
              <FolderSwatch color={badgeFolder?.color} className="h-2 w-2" />
              <span className="truncate">{badgeFolder ? badgeFolder.name : t("companySkills.unfiled", { defaultValue: "Unfiled" })}</span>
            </div>
          ) : null}
        </div>
        ...
        {(() => {
          ...
          return (
            <span className="shrink-0 text-muted-foreground" title={t("companySkills.fromSource", { defaultValue: "From {{label}}", label: meta.label })} aria-label={t("companySkills.fromSource", { defaultValue: "From {{label}}", label: meta.label })}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("companySkills.moreActionsFor", { defaultValue: "More actions for {{name}}", name: card.name })}
                onClick={(event) => event.stopPropagation()}
              >
              ...
              {onOpenMove ? (
                <>
                  <DropdownMenuItem onSelect={() => onOpenMove(card)}>
                    ...
                    {t("companySkills.moveToFolderDots", { defaultValue: "Move to folder…" })}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                </>
              ) : null}
      ...
      {card.forkedFrom ? (
        <div className="mt-2 inline-flex items-center gap-1 text-(length:--text-micro) text-muted-foreground">
          ...
          {t("companySkills.forkedBadge", { defaultValue: "Forked" })}
        </div>
      ) : null}
      ...
      <div className="mt-auto pt-3">
        ...
        <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
          <span>{t("companySkills.agentCount", { defaultValue: "{{count}} agent", count: card.agentCount })}</span>
          {card.starCount > 0 ? (
            <>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1">
          {card.installed ? (
            <Badge variant="outline" className="border-emerald-500/30 bg-emerald-500/10 text-(length:--text-nano) text-emerald-700 dark:text-emerald-300">
              {t("companySkills.installedBadge", { defaultValue: "Installed" })}
            </Badge>
          ) : null}
          ...
          {card.required ? (
            <Badge variant="outline" className="ml-auto border-border bg-muted/60 text-(length:--text-nano) text-muted-foreground">
              ...
              {t("companySkills.bundledBadge", { defaultValue: "Bundled" })}
            </Badge>
          ) : null}
        </div>
      </div>
...
function CategoryNav({
  ...
  return (
    <nav className="flex flex-col gap-0.5 px-2">
      ...
      >
        <span>{t("companySkills.filterAll", { defaultValue: "All" })}</span>
    ...
    <div className="flex min-h-(--sz-calc-30) md:h-(--sz-calc-33) md:min-h-0 md:overflow-hidden">
      ...
      <aside className={cn("hidden w-60 shrink-0 flex-col overflow-hidden border-r border-border md:flex", showFolderRail && "md:hidden")}>
        <div className="border-b border-border px-4 py-4">
          <h2 className="text-sm font-semibold text-foreground">{t("companySkills.storeTitle", { defaultValue: "Skills Store" })}</h2>
          <p className="text-xs text-muted-foreground">{t("companySkills.storeTagline", { defaultValue: "Discover, install, fork, share" })}</p>
        </div>
        <div className="px-4 pb-1 pt-3 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
          {t("companySkills.categoriesLabel", { defaultValue: "Categories" })}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm">
                <span className="text-muted-foreground">{t("companySkills.sortButton", { defaultValue: "Sort" })}</span>
                <span className="ml-1.5">{discoverySortLabels()[sort]}</span>
          ...
          {availableSources.length > 1 ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm">
                  <span className="text-muted-foreground">{t("companySkills.source", { defaultValue: "Source" })}</span>
                  <span className="ml-1.5 capitalize">
                    {sourceBadgeFilter === "all" ? t("companySkills.filterAll", { defaultValue: "All" }) : sourceMeta(sourceBadgeFilter as CompanySkillSourceBadge, null).label}
                  </span>
                  <ChevronDown className="ml-1 h-3.5 w-3.5" />
                </Button>
              </DropdownMenuTrigger>
          ...
          <Button
            ...
            aria-label={t("companySkills.scanAria", { defaultValue: "Scan project workspaces for skills" })}
            title={t("companySkills.scanAria", { defaultValue: "Scan project workspaces for skills" })}
          >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onSelect={onImport}>
                ...
                {t("companySkills.importFromPathOrUrl", { defaultValue: "Import from path or URL" })}
              </DropdownMenuItem>
          ...
          {folderResult && onFolderSelect ? (
            <div className="w-full md:hidden">
              <FolderChip
                result={folderResult}
                selection={folderSelection}
                allLabel={t("companySkills.allSkills", { defaultValue: "All skills" })}
          ...
          {onCreateFolder && !showFolderRail ? (
            <Button variant="outline" size="sm" onClick={onCreateFolder}>
              <Plus className="mr-1 h-3.5 w-3.5" />
              {t("companySkills.newFolder", { defaultValue: "New folder" })}
            </Button>
          ) : null}
          {onToggleSelectMode ? (
            <Button variant="ghost" size="sm" onClick={onToggleSelectMode}>
              {selectMode ? t("common.done", { defaultValue: "Done" }) : t("companySkills.selectMode", { defaultValue: "Select" })}
            </Button>
          ) : null}
        </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="w-full justify-between">
                  <span className="capitalize">{activeCategory ?? t("companySkills.filterAllCategories", { defaultValue: "All categories" })}</span>
              ...
              <DropdownMenuContent align="start" className="max-h-72 overflow-auto">
                ...
                >
                  <DropdownMenuRadioItem value="__all__">{t("companySkills.allCount", { defaultValue: "All ({{count}})", count: categoryTotal })}</DropdownMenuRadioItem>
                  {categories.map((category) => (
          ...
          <Tabs value={tab} onValueChange={(value) => onTabChange(value as DiscoveryTab)}>
            <TabsList variant="line" className="p-0">
              ...
              <TabsTrigger value="bundled" className="px-3">
                <span>{t("companySkills.filterBundled", { defaultValue: "Bundled" })}</span>
        ...
        <div className="min-h-0 flex-1 overflow-auto p-4">
            ...
            <div className="mb-4 flex items-center justify-between gap-2">
              ...
              {activeProjectFolder && activeProjectId ? (
                <Button
                  ...
                  disabled={scanPending}
                  aria-label={t("companySkills.refreshProjectSkillsAria", { defaultValue: "Refresh {{name}} project skills", name: activeProjectFolder.name })}
                  title={t("companySkills.refreshProjectSkillsTitle", { defaultValue: "Refresh skills from {{name}}", name: activeProjectFolder.name })}
                >
                  <RefreshCw className={cn("h-3.5 w-3.5", scanPending && "animate-spin")} />
                  {t("companySkills.refresh", { defaultValue: "Refresh" })}
                </Button>
              ) : null}
            </div>
          ) : null}
          {folderNudgeStorageKey && onCreateFolder && folderResult && folderResult.folders.length === 0 && !loading && cards.length > 0 ? (
            <AllUnfiledBanner
              ...
              itemLabelPlural={t("companySkills.skillsNounPlural", { defaultValue: "skills" })}
          ...
          {loading ? (
            ...
            <div className="py-12">
              <EmptyState
                ...
                message={
                  totalCount === 0
                    ? t("companySkills.emptyNoSkills", { defaultValue: "No skills yet. Create one or install from the catalog." })
                    : search || activeCategory || sourceFilterActive
                      ? t("companySkills.emptyNoMatch", { defaultValue: "No skills match your filters." })
                      : t("companySkills.emptyTab", { defaultValue: "No skills in this tab yet." })
                }
              />
              {totalCount === 0 ? (
                <div className="mt-3 flex flex-col items-center gap-2">
                  ...
                  <Button size="sm" variant="ghost" onClick={onCreate}>
                    {t("companySkills.createASkill", { defaultValue: "Create a skill" })}
                  </Button>
                </div>
              ) : (search || activeCategory || sourceFilterActive) ? (
                <div className="mt-3 flex justify-center">
                  ...
                  >
                    {t("companySkills.clearFilters", { defaultValue: "Clear filters" })}
                  </Button>
                </div>
              ) : null}
            </div>
            ...
            <>
              <p className="mb-3 text-xs text-muted-foreground">
                {t("companySkills.skillCount", { defaultValue: "{{count}} skill", count: sourceFilteredCards.length })}
...
function NewSkillWizard({
  ...
  const steps = [
    t("companySkills.wizardBasics", { defaultValue: "Basics" }),
    t("companySkills.wizardDesign", { defaultValue: "Design" }),
    t("companySkills.wizardContent", { defaultValue: "Content" }),
    t("companySkills.wizardReview", { defaultValue: "Review" }),
  ];
    ...
    <div className="space-y-4">
      ...
      {draft.forkedFromName ? (
        <div className="flex items-center gap-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          ...
          {t("companySkills.forkingName", { defaultValue: "Forking {{name}}", name: draft.forkedFromName })}
        </div>
      ) : null}
      ...
      {step === 0 ? (
        <div className="space-y-3">
          ...
          <Textarea
            ...
            placeholder={t("companySkills.taglinePlaceholder", { defaultValue: "One-line promise for the skill" })}
            className="min-h-20"
          />
        </div>
      ) : step === 1 ? (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <SkillCardIcon
              ...
              card={{
                ...
                name: draft.name || t("companySkills.newSkillFallback", { defaultValue: "New Skill" }),
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-medium">{draft.name || t("companySkills.newSkillFallback", { defaultValue: "New Skill" })}</div>
              <div className="truncate text-xs text-muted-foreground">{draft.tagline || t("companySkills.noTaglineYet", { defaultValue: "No tagline yet." })}</div>
            </div>
          </div>
          <div>
            <label className="mb-2 block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.colorLabel", { defaultValue: "Color" })}</label>
            <div className="flex flex-wrap gap-2">
              {SKILL_CREATE_ACCENTS.map((color) => (
                <button
                  ...
                  aria-label={t("companySkills.useColorAria", { defaultValue: "Use {{color}}", color: color })}
                />
              ))}
              <Input
                value={draft.color}
          ...
          <div>
            <label className="mb-2 block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.categoriesLabel", { defaultValue: "Categories" })}</label>
            <Input
              value={categoryDraft}
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-(--gtc-26) gap-y-2">
            <span className="text-muted-foreground">{t("companySkills.nameLabel", { defaultValue: "Name" })}</span>
            <span>{draft.name || t("companySkills.untitled", { defaultValue: "Untitled" })}</span>
            <span className="text-muted-foreground">{t("companySkills.slugLabel", { defaultValue: "Slug" })}</span>
            <span className="font-mono">{effectiveSlug || "skill"}</span>
            <span className="text-muted-foreground">{t("companySkills.scopeLabel", { defaultValue: "Scope" })}</span>
            <span>{draft.sharingScope === "private" ? t("companySkills.sharingPrivate", { defaultValue: "Private" }) : t("companySkills.sharingCompany", { defaultValue: "Company" })}</span>
            <span className="text-muted-foreground">{t("companySkills.categoriesLabel", { defaultValue: "Categories" })}</span>
            <span>{draft.categories.length ? draft.categories.join(", ") : t("companySkills.none", { defaultValue: "none" })}</span>
          </div>
          <div className="space-y-2">
            <label className="block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.sharingLabel", { defaultValue: "Sharing" })}</label>
            <div className="grid gap-2 sm:grid-cols-3">
              {(["company", "private"] as const).map((scope) => (
                ...
                >
                  <span className="block font-medium">{scope === "company" ? t("companySkills.sharingCompany", { defaultValue: "Company" }) : t("companySkills.sharingPrivate", { defaultValue: "Private" })}</span>
                  <span className="mt-1 block text-xs text-muted-foreground">
                    {scope === "company" ? t("companySkills.sharingCompanyHint", { defaultValue: "Visible inside this company." }) : t("companySkills.sharingPrivateHint", { defaultValue: "Only visible in your library." })}
                  </span>
                </button>
              ))}
              <button
              ...
              >
                <span className="block font-medium">{t("companySkills.sharingPublic", { defaultValue: "Public link" })}</span>
                <span className="mt-1 block text-xs">{t("companySkills.comingLater", { defaultValue: "Coming later." })}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {step < steps.length - 1 ? (
            ...
            <Button size="sm" onClick={submit} disabled={isPending || !nameValid}>
              {isPending ? t("companySkills.creating", { defaultValue: "Creating..." }) : draft.forkedFromSkillId ? t("companySkills.createFork", { defaultValue: "Create fork" }) : t("companySkills.createSkill", { defaultValue: "Create skill" })}
            </Button>
          )}
        </div>
      </div>
  ...
  if (filtered.length === 0) {
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        {t("companySkills.noCatalogMatch", { defaultValue: "No catalog skills match this filter." })}
      </div>
    );
  }
      ...
      <div key={skill.id} className="border-b border-border">
        ...
        >
          ...
          <button
            ...
            aria-label={expanded ? t("companySkills.collapseSkillAria", { defaultValue: "Collapse {{name}}", name: skill.name }) : t("companySkills.expandSkillAria", { defaultValue: "Expand {{name}}", name: skill.name })}
          >
  ...
  return (
    <div>
      {bundled.length > 0 && kindFilter !== "optional" ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("companySkills.bundledCount", { defaultValue: "Bundled · {{count}}", count: bundled.length })}
          </div>
      ...
      {optional.length > 0 && kindFilter !== "bundled" ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("companySkills.optionalCount", { defaultValue: "Optional · {{count}}", count: optional.length })}
          </div>
      ...
      {installed.length > 0 ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("companySkills.installedCount", { defaultValue: "Installed · {{count}}", count: installed.length })}
          </div>
  ...
  if (skill.compatibility === "invalid") {
    cta = (
      <Tooltip>
        ...
        <TooltipContent>{t("companySkills.cannotInstallTooltip", { defaultValue: "This skill cannot be installed — its content is not valid Agent Skills markdown." })}</TooltipContent>
      </Tooltip>
    );
  } else if (!isInstalled) {
    cta = (
      <Button onClick={onInstall} disabled={loadingPrimaryAction}>
        ...
        {loadingPrimaryAction ? t("companySkills.preparing", { defaultValue: "Preparing..." }) : t("companySkills.installInOrg", { defaultValue: "Install skill in this organization" })}
      </Button>
    );
    ...
    <div className="min-w-0">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-4">
          ...
          {hashOutOfSync ? (
            <Tooltip>
              ...
              <TooltipContent>{t("companySkills.updateAvailableTooltip", { defaultValue: "Catalog content hash has changed since this skill was installed." })}</TooltipContent>
            </Tooltip>
          ) : null}
          {skill.requires.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("companySkills.requiresWith", { defaultValue: "Requires: {{value}}", value: skill.requires.join(", ") })}
            </Badge>
          ) : null}
          {skill.recommendedForRoles.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("companySkills.rolesWith", { defaultValue: "Roles: {{value}}", value: skill.recommendedForRoles.join(" · ") })}
            </Badge>
          ) : null}
          {skill.tags.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("companySkills.tagsWith", { defaultValue: "Tags: {{value}}", value: skill.tags.join(" · ") })}
            </Badge>
          ) : null}
        </div>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          <span className="uppercase tracking-(--tracking-caps)">{t("companySkills.keyLabel", { defaultValue: "Key" })}</span>
          <span className="font-mono">{skill.key}</span>
          <span className="uppercase tracking-(--tracking-caps)">·</span>
          <span className="uppercase tracking-(--tracking-caps)">{t("companySkills.hashLabel", { defaultValue: "Hash" })}</span>
          ...
          <CopyText
            ...
            ariaLabel={t("companySkills.copyHashAria", { defaultValue: "Copy content hash" })}
            title={t("companySkills.copyHashAria", { defaultValue: "Copy content hash" })}
      ...
      <div className="min-h-(--sz-400px) px-5 py-5">
        {fileQuery.isLoading ? (
          ...
          <div className="text-sm text-destructive">{fileQuery.error instanceof Error ? fileQuery.error.message : t("companySkills.failedToLoadFile", { defaultValue: "Failed to load file" })}</div>
        ) : !fileQuery.data ? (
...
function InstallPreviewDialog({
  ...
  if (defaultAction === "update") {
    confirmLabel = t("companySkills.confirmInstallUpdate", { defaultValue: "Install update" });
  } else if (defaultAction === "replace") {
    confirmLabel = t("companySkills.confirmReplace", { defaultValue: "Replace existing skill" });
    confirmVariant = "destructive";
  }
  if (isPending) confirmLabel = t("companySkills.installingEllipsis", { defaultValue: "Installing…" });
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => (!isPending ? onOpenChange(value) : null)}>
      <DialogContent className="sm:max-w-2xl" showCloseButton={!isPending}>
        <DialogHeader>
          <DialogTitle>
            {t("companySkills.installDialogTitle", {
              defaultValue: "{{action}} · {{name}}",
              action: defaultAction === "update"
                ? t("companySkills.installDialogActionUpdate", { defaultValue: "Update" })
                : defaultAction === "replace"
                  ? t("companySkills.installDialogActionReplace", { defaultValue: "Replace" })
                  : t("companySkills.installDialogActionInstall", { defaultValue: "Install" }),
              name: skill.name,
            })}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4 text-sm">
            ...
            <div className="grid grid-cols-(--gtc-26) gap-y-2 text-xs">
              ...
              <div className="flex items-center gap-2">
                ...
                {skill.trustLevel === "markdown_only" ? (
                  <span className="text-muted-foreground">{t("companySkills.safeLabel", { defaultValue: "Safe" })}</span>
                ) : skill.trustLevel === "scripts_executables" ? (
                  <span className="text-amber-800 dark:text-amber-200">{t("companySkills.reviewRequired", { defaultValue: "Review required" })}</span>
                ) : (
                  <span className="text-muted-foreground">{t("companySkills.nonScriptAssets", { defaultValue: "Non-script assets" })}</span>
                )}
              </div>
              <div className="text-muted-foreground">{t("companySkills.compatibilityLabel", { defaultValue: "Compatibility" })}</div>
              <div className="flex items-center gap-2">
                {skill.compatibility === "compatible" ? (
                  <span className="inline-flex items-center gap-1 text-muted-foreground">
                    <Check className="h-3 w-3" aria-hidden="true" />
                    {t("companySkills.compatibleLabel", { defaultValue: "Compatible" })}
                  </span>
                ) : (
                  <CompatChip compatibility={skill.compatibility} />
                )}
              </div>
              <div className="text-muted-foreground">{t("companySkills.requiresLabel", { defaultValue: "Requires" })}</div>
              <div className="text-foreground">{skill.requires.length === 0 ? t("companySkills.none", { defaultValue: "none" }) : skill.requires.join(", ")}</div>
              <div className="text-muted-foreground">{t("companySkills.rolesLabel", { defaultValue: "Roles" })}</div>
              <div className="text-foreground">{skill.recommendedForRoles.length === 0 ? t("companySkills.anyLabel", { defaultValue: "any" }) : skill.recommendedForRoles.join(" · ")}</div>
              <div className="text-muted-foreground">{t("companySkills.provenanceLabel", { defaultValue: "Provenance" })}</div>
              <div className="min-w-0">
          ...
          {conflict ? (
            <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-200">
              {t("companySkills.installConflictBody", {
                defaultValue: "An existing skill with key {{key}} is installed ({{source}}). Installing will {{action}}.",
                key: conflict.key,
                source: conflict.sourceLabel ?? conflict.sourceType,
                action: defaultAction === "update"
                  ? t("companySkills.installConflictOverwrite", { defaultValue: "overwrite the catalog content" })
                  : t("companySkills.installConflictReplace", { defaultValue: "replace the existing skill" }),
              })}
            </div>
          ) : null}
          ...
          >
            ...
            {t("companySkills.advancedLabel", { defaultValue: "Advanced" })}
          </button>
          {advancedOpen ? (
            <div className="space-y-3 rounded-md border border-border p-3 text-xs">
              <div>
                <label className="mb-1 block uppercase tracking-wide text-muted-foreground">{t("companySkills.slugOverrideLabel", { defaultValue: "Slug override" })}</label>
              ...
              <label className="flex items-center gap-2">
                ...
                <span>{t("companySkills.forceReplaceLabel", { defaultValue: "Force replace existing same-key skill" })}</span>
              </label>
            </div>
          ) : null}
    ...
    <AgentMultiSelect
      ...
      headerContent={sortedVersions.length > 0 ? (
        <div className="mt-2 flex items-center gap-2 text-xs">
          ...
          >
            <option value="__latest__">{t("companySkills.latest", { defaultValue: "Latest" })}</option>
      ...
      emptyMessage={eligible.length === 0 ? t("companySkills.noAgentsSupport", { defaultValue: "No agents in this company support skills yet." }) : t("companySkills.noAgentsYet", { defaultValue: "No agents yet." })}
      ...
      getDescription={(agent) => {
        const option = agent as AttachAgentOption;
        return t("companySkills.agentDescription", {
          defaultValue: "{{adapter}}{{required}}{{unsupported}}",
          adapter: option.adapterType,
          required: option.required ? t("companySkills.requiredSuffix", { defaultValue: " · required" }) : "",
          unsupported: !option.supportsSkills ? t("companySkills.skillsNotSupportedSuffix", { defaultValue: " · skills not supported" }) : "",
        });
      }}
      renderNameSuffix={(agent) => (agent as AttachAgentOption).paused ? (
        <Badge variant="outline" className="[&>svg]:size-2.5 border-amber-500/30 bg-amber-500/10 px-1.5 text-(length:--text-nano) uppercase tracking-wide text-amber-500">
          ...
          {t("companySkills.paused", { defaultValue: "Paused" })}
        </Badge>
      ) : null}
    />
  );
...
function SkillList({
  ...
  if (filteredSkills.length === 0) {
    if (sourceFilter !== "all" && skills.length > 0) {
      return (
        <div className="px-4 py-6 text-sm text-muted-foreground">
          {t("companySkills.noSourceSkillsInstalled", { defaultValue: "No {{label}} skills installed.", label: sourceFilterLabels()[sourceFilter].toLowerCase() })}{" "}
          <button type="button" className="text-foreground underline" onClick={onClearFilters}>
            {t("companySkills.clearFilter", { defaultValue: "Clear filter" })}
          </button>
        </div>
      );
    }
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        {t("companySkills.noSkillsMatchFilter", { defaultValue: "No skills match this filter." })}
      </div>
    );
  }
...
function skillDetailTabs(): Array<{ value: SkillDetailTab; label: string; icon: typeof FileText }> {
  return [
    { value: "overview", label: t("companySkills.tabOverview", { defaultValue: "Overview" }), icon: FileText },
    { value: "files", label: t("companySkills.tabFiles", { defaultValue: "Files" }), icon: FolderOpen },
    { value: "versions", label: t("companySkills.tabVersions", { defaultValue: "Versions" }), icon: History },
    { value: "agents", label: t("companySkills.tabAgents", { defaultValue: "Agents" }), icon: Users },
  ];
}
...
function versionLabel(version: CompanySkillVersion | null | undefined) {
  if (!version) return t("companySkills.latest", { defaultValue: "Latest" });
      ...
      <DialogContent className="flex max-h-(--sz-85vh) w-full !max-w-(--pct-90) flex-col overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <DialogHeader className="shrink-0">
            <DialogTitle>{t("companySkills.diffTitle", { defaultValue: "Diff · skill files" })}</DialogTitle>
          </DialogHeader>
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <label className="flex items-center gap-2">
              <Badge variant="outline" className="border-red-500/30 bg-red-500/10 uppercase tracking-wider text-red-400">{t("companySkills.diffOldBadge", { defaultValue: "Old" })}</Badge>
              <select
                value={leftVersionId ?? ""}
              ...
              >
                <option value="">{t("companySkills.initialOption", { defaultValue: "Initial" })}</option>
                {sorted.map((version) => (
            ...
            <label className="flex items-center gap-2">
              <Badge variant="outline" className="border-green-500/30 bg-green-500/10 uppercase tracking-wider text-green-400">{t("companySkills.diffNewBadge", { defaultValue: "New" })}</Badge>
              <select
                value={right?.id ?? ""}
        ...
        <div className="flex min-h-0 flex-1 gap-3">
          ...
          <div className="min-w-0 flex-1 overflow-auto rounded-md border border-border text-xs">
            {!right ? (
              ...
              <div className="p-6 text-center text-sm text-muted-foreground">{t("companySkills.sameVersion", { defaultValue: "Both sides are the same version." })}</div>
            ) : (
              <div className="font-mono text-xs leading-6">
                <div className="grid grid-cols-(--gtc-1) border-b border-border/60 bg-muted/30 px-3 py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                  <span>{t("companySkills.diffOldBadge", { defaultValue: "Old" })}</span>
                  <span>{t("companySkills.diffNewBadge", { defaultValue: "New" })}</span>
                  <span />
...
function SkillLocationCard({
  ...
  const canonical = folderPath && folderPath.length > 0 ? folderPath : t("companySkills.unfiled", { defaultValue: "Unfiled" });
  return (
    <section>
      <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.locationLabel", { defaultValue: "Location" })}</div>
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        >
          ...
          {copied ? t("companySkills.copiedLabel", { defaultValue: "Copied" }) : t("companySkills.copyPath", { defaultValue: "Copy path" })}
        </Button>
        {onMove ? (
          <Button size="sm" variant="outline" onClick={onMove}>
            <FolderInput className="mr-1.5 h-3.5 w-3.5" />
            {t("companySkills.moveButton", { defaultValue: "Move" })}
          </Button>
        ) : null}
      </div>
    </section>
  ...
  return (
    <section>
      <div className="mb-2 flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted-foreground">
        ...
        {t("companySkills.tagsLabel", { defaultValue: "Tags" })}
      </div>
      <div className="flex flex-wrap gap-1.5">
        {categories.map((tag) => (
          ...
          >
            ...
            <button
              type="button"
              aria-label={t("companySkills.removeTagAria", { defaultValue: "Remove tag {{name}}", name: tag })}
              disabled={pending}
...
export function SkillDetailPage({
  ...
  if (!detail) {
    return loading ? <PageSkeleton variant="detail" /> : <EmptyState icon={Boxes} message={t("companySkills.skillNotFound", { defaultValue: "Skill not found." })} />;
  }
    ...
    return (
      <div className="grid min-h-(--sz-560px) gap-0 lg:grid-cols-(--gtc-28)">
        <aside className="border-b border-border pb-3 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-3">
          <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.tabFiles", { defaultValue: "Files" })}</div>
          <SkillTree
            ...
            <div className="flex items-center gap-2">
              {file?.markdown && !editMode ? (
                <div className="flex items-center border border-border">
                  ...
                  >
                    <span className="flex items-center gap-1.5"><Eye className="h-3.5 w-3.5" /> {t("companySkills.viewToggle", { defaultValue: "View" })}</span>
                  </button>
                  <button
                    className={cn("border-l border-border px-3 py-1.5 text-sm", viewMode === "code" ? "text-foreground" : "text-muted-foreground")}
                    onClick={() => setViewMode("code")}
                  >
                    <span className="flex items-center gap-1.5"><Code2 className="h-3.5 w-3.5" /> {t("companySkills.codeToggle", { defaultValue: "Code" })}</span>
                  </button>
                </div>
              ) : null}
              {skill.editable && file?.editable ? (
                editMode ? (
                  <>
                    <Button variant="ghost" size="sm" onClick={() => setEditMode(false)} disabled={savePending}>{t("companySkills.cancel", { defaultValue: "Cancel" })}</Button>
                    <Button size="sm" onClick={onSave} disabled={savePending}>
                      <Save className="mr-1.5 h-3.5 w-3.5" />
                      {savePending ? t("companySkills.savingDots", { defaultValue: "Saving..." }) : t("companySkills.save", { defaultValue: "Save" })}
                    </Button>
                  </>
                ) : (
                ...
                <Button
                  ...
                  size="sm"
                  onClick={onFork}
                  title={skill.editableReason ?? t("companySkills.forkToEditTitle", { defaultValue: "Fork this skill to edit it." })}
                >
  ...
  function renderOverviewBody() {
    return (
      <div className="space-y-6">
        <section>
          <h2 className="mb-2 text-sm font-medium">{t("companySkills.aboutLabel", { defaultValue: "About" })}</h2>
          {fileLoading ? (
            <PageSkeleton variant="detail" />
          ) : file?.markdown ? (
        ...
        <section className="grid min-w-0 gap-3 text-sm sm:grid-cols-2">
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("companySkills.keyLabel", { defaultValue: "Key" })}</div>
          ...
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("companySkills.modeLabel", { defaultValue: "Mode" })}</div>
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {skill.editable ? (
                t("companySkills.editableLabel", { defaultValue: "Editable" })
              ) : (
                <>
                  <span>{t("companySkills.readOnly", { defaultValue: "Read only" })}</span>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm text-muted-foreground">
            {versionsLoading ? t("companySkills.loadingVersions", { defaultValue: "Loading versions..." }) : t("companySkills.versionCount", { defaultValue: "{{count}} version", count: versions.length })}
          </div>
          <Button
            type="button"
            variant="outline"
          ...
          >
            <History className="mr-1.5 h-3.5 w-3.5" /> {t("companySkills.compareButton", { defaultValue: "Compare" })}
          </Button>
        </div>
              ...
              <div key={version.id} className="grid gap-2 border-b border-border px-0 py-3 text-sm last:border-b-0 sm:grid-cols-(--gtc-13)">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    {t("companySkills.versionFilesCount", { defaultValue: "{{time}} · {{count}} files", time: relativeTime(version.createdAt), count: version.fileInventory.length })}
                  </div>
                </div>
                <Button
                  type="button"
                ...
                >
                  {t("companySkills.viewDiff", { defaultValue: "View diff" })}
                </Button>
              </div>
            ))
          )}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            {t("companySkills.agentsAttached", { defaultValue: "{{count}} agent attached", count: attached.length })}
            {selectedVersion
              ? t("companySkills.versionSuffix", { defaultValue: " · {{version}}", version: versionLabel(selectedVersion) })
              : t("companySkills.latestSuffix", { defaultValue: " · Latest" })}
          </p>
          <AttachAgentsPopover
            agents={attachAgents}
        ...
        {attached.length === 0 ? (
          <div className="rounded-md border border-dashed border-border py-8 text-center text-sm text-muted-foreground">
            {t("companySkills.noAgentsUsing", { defaultValue: "No agents are using this skill yet. Use “Add to agent” to attach it." })}
          </div>
        ) : (
          <div className="border-y border-border">
            {attached.map((agent) => {
              ...
              return (
                <div key={agent.id} className="flex items-center gap-3 border-b border-border py-3 text-sm last:border-b-0">
                  ...
                  >
                    {t("companySkills.viewAgentLink", { defaultValue: "View" })}
                  </Link>
                </div>
              );
            })}
    ...
    <div className="min-h-(--sz-calc-30)">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span
                        ...
                        aria-label={t("companySkills.installedFrom", { defaultValue: "Installed from {{label}}", label: source.label })}
                      >
                        <SourceIcon className="h-4 w-4" />
                      </span>
                    </TooltipTrigger>
                    <TooltipContent>{t("companySkills.installedFrom", { defaultValue: "Installed from {{label}}", label: source.label })}</TooltipContent>
                  </Tooltip>
                </div>
                {/* GitHub-style "by" attribution sits directly under the title. */}
                {detail.authorName ? (
                  <p className="mt-0.5 text-sm text-muted-foreground">
                    {t("companySkills.byAuthorName", { defaultValue: "by " })}
                ...
                {subtitleText ? (
                  <div className="mt-1 max-w-2xl">
                    ...
                    {descClamped ? (
                      ...
                      >
                        {descExpanded ? t("companySkills.showLess", { defaultValue: "Show less" }) : t("companySkills.viewAll", { defaultValue: "View all" })}
                      </button>
                    ) : null}
                  </div>
                ) : null}
          ...
          <div className="flex flex-wrap items-center justify-end gap-1">
            ...
            <div className="flex items-center overflow-hidden rounded-md border border-border">
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-muted-foreground">
                    ...
                    <span className="hidden sm:inline">{t("companySkills.installsLabel", { defaultValue: "install", count: detail.attachedAgentCount })}</span>
                  </span>
                </TooltipTrigger>
                <TooltipContent>{t("companySkills.installsTooltip", { defaultValue: "Agents in this company that currently have this skill installed." })}</TooltipContent>
              </Tooltip>
              <button
                type="button"
                ...
                title={detail.starredByCurrentActor ? t("companySkills.unstarTitle", { defaultValue: "Unstar this skill" }) : t("companySkills.starTitle", { defaultValue: "Star this skill" })}
              >
                <Star className={cn("h-3.5 w-3.5", detail.starredByCurrentActor && "fill-current text-yellow-400")} />
                <span className="hidden sm:inline">{detail.starredByCurrentActor ? t("companySkills.starred", { defaultValue: "Starred" }) : t("companySkills.star", { defaultValue: "Star" })}</span>
              ...
              <button
                ...
                title={t("companySkills.forkTitle", { defaultValue: "Fork this skill" })}
              >
                <GitFork className="h-3.5 w-3.5" />
      ...
      <div className="grid gap-6 px-4 py-4 xl:grid-cols-(--gtc-29)">
        ...
        <aside className="min-w-0 space-y-6 border-t border-border pt-4 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
          ...
          />
          <section>
            <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.tabAgents", { defaultValue: "Agents" })}</div>
            <div className="space-y-3">
                ...
                <div className="space-y-0.5">
                  ...
                  {detail.usedByAgents.length > 3 ? (
                    <p className="px-1.5 pt-0.5 text-xs text-muted-foreground">
                      {t("companySkills.andMore", { defaultValue: "and {{count}} more", count: detail.usedByAgents.length - 3 })}
                    </p>
                  ) : null}
                </div>
              )}
          ...
          {detail.sourceType === "github" ? (
            <section>
              <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.updatesLabel", { defaultValue: "Updates" })}</div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Pin className="h-3.5 w-3.5 shrink-0" aria-label={t("companySkills.pinnedRevision", { defaultValue: "Pinned source revision" })} />
                    </TooltipTrigger>
                    <TooltipContent>{t("companySkills.pinnedRevision", { defaultValue: "Pinned source revision" })}</TooltipContent>
                  </Tooltip>
                  <span className="truncate font-mono text-foreground">{currentPin ?? t("companySkills.untracked", { defaultValue: "untracked" })}</span>
                </div>
                ...
                {updateStatus?.supported && updateStatus.hasUpdate ? (
                  <Button size="sm" className="w-full" onClick={onInstallUpdate} disabled={installUpdatePending}>
                    ...
                    {t("companySkills.installUpdateWithPin", { defaultValue: "Install update{{pin}}", pin: latestPin ? ` ${latestPin}` : "" })}
                  </Button>
                ) : updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading ? (
                  <p className="text-xs text-muted-foreground">{t("companySkills.upToDate", { defaultValue: "Up to date." })}</p>
                ) : null}
              </div>
            </section>
          ) : null}
      ...
      {isDirty ? (
        <div className="fixed bottom-6 left-1/2 z-40 flex -translate-x-1/2 items-center gap-3 rounded-full border border-border bg-background/95 px-4 py-2 shadow-lg backdrop-blur">
          ...
          <Button size="sm" onClick={onSave} disabled={savePending}>
            <Save className="mr-1.5 h-3.5 w-3.5" />
            {savePending ? t("companySkills.savingEllipsis", { defaultValue: "Saving…" }) : t("companySkills.saveChanges", { defaultValue: "Save changes" })}
          </Button>
        </div>
      ) : null}
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.skillSettingsTitle", { defaultValue: "Skill settings" })}</DialogTitle>
            <DialogDescription>{t("companySkills.settingsDescription", { defaultValue: "Manage how {{name}} is grouped and shared.", name: detail.name })}</DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.categoriesLabel", { defaultValue: "Categories" })}</label>
              <Input
              ...
              />
              <p className="text-xs text-muted-foreground">{t("companySkills.categoriesHint", { defaultValue: "Separate categories with commas. Leave empty to clear categories." })}</p>
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.sharingLabel", { defaultValue: "Sharing" })}</label>
              <select
              ...
              >
                <option value="company">{t("companySkills.sharingCompanyOption", { defaultValue: "Company — visible inside this company" })}</option>
                <option value="private">{t("companySkills.sharingPrivateOption", { defaultValue: "Private — only visible in your library" })}</option>
              </select>
            ...
            <div className="flex justify-end gap-2 border-t border-border pt-4">
              ...
              >
                {t("companySkills.reset", { defaultValue: "Reset" })}
              </Button>
              <Button
                type="button"
                size="sm"
              ...
              >
                <Save className="mr-1.5 h-3.5 w-3.5" />
                {updateSettingsPending ? t("companySkills.savingEllipsis", { defaultValue: "Saving…" }) : t("companySkills.saveSettings", { defaultValue: "Save settings" })}
              </Button>
            </div>
            {detail.editable ? (
              <div className="rounded-md border border-destructive/40 p-3">
                ...
                <div className="mt-2 flex items-center justify-between gap-3">
                  ...
                  <Button
                    ...
                    title={detail.usedByAgents.length > 0 ? t("companySkills.detachBeforeRemove", { defaultValue: "Detach this skill from all agents before removing it." }) : undefined}
                  >
                    <Trash2 className="mr-1.5 h-3.5 w-3.5" />
                    {deletePending ? t("companySkills.removingEllipsis", { defaultValue: "Removing…" }) : t("common.remove", { defaultValue: "Remove" })}
                  </Button>
                </div>
              </div>
            ) : null}
...
function SkillPane({
  ...
  const removeDisabledReason = removeBlocked
    ? t("companySkills.detachBeforeRemove", { defaultValue: "Detach this skill from all agents before removing it." })
    : null;
      ...
      <div className="border-b border-border px-5 py-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            >
              <Trash2 className="mr-1.5 h-3.5 w-3.5" />
              {deletePending ? t("companySkills.removingDots", { defaultValue: "Removing..." }) : t("common.remove", { defaultValue: "Remove" })}
            </Button>
            {detail.editable ? (
              <button
              ...
              >
                ...
                {editMode ? t("companySkills.stopEditing", { defaultValue: "Stop editing" }) : t("common.edit", { defaultValue: "Edit" })}
              </button>
            ) : (
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex min-w-0 items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.source", { defaultValue: "Source" })}</span>
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.pinLabel", { defaultValue: "Pin" })}</span>
                <span className="font-mono text-xs">{currentPin ?? t("companySkills.untracked", { defaultValue: "untracked" })}</span>
                {updateStatus?.trackingRef && (
                  <span className="text-xs text-muted-foreground">{t("companySkills.trackingRef", { defaultValue: "tracking {{ref}}", ref: updateStatus.trackingRef })}</span>
                )}
                <Button
                  variant="ghost"
                  size="sm"
                ...
                {updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading && (
                  <span className="text-xs text-muted-foreground">{t("companySkills.upToDateShort", { defaultValue: "Up to date" })}</span>
                )}
            ...
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.keyLabel", { defaultValue: "Key" })}</span>
              <span className="font-mono text-xs">{detail.key}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.modeLabel", { defaultValue: "Mode" })}</span>
              <span>{detail.editable ? t("companySkills.editableLabel", { defaultValue: "Editable" }) : t("companySkills.readOnly", { defaultValue: "Read only" })}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.trust", { defaultValue: "Trust" })}</span>
          ...
          <div className="flex flex-wrap items-start gap-x-3 gap-y-2">
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.usedByLabel", { defaultValue: "Used by" })}</span>
              <AttachAgentsPopover
                agents={attachAgents}
              ...
              <div className="flex items-center border border-border">
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    {t("companySkills.viewToggle", { defaultValue: "View" })}
                  </span>
                </button>
                <button
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    {t("companySkills.codeToggle", { defaultValue: "Code" })}
                  </span>
                </button>
              </div>
            )}
...
export function CompanySkills() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("nav.skills", { defaultValue: "Skills" }), href: "/skills" },
      ...(isStudioNew
        ? [{ label: studioForkFromId ? t("companySkills.forkSkillTitle", { defaultValue: "Fork skill" }) : t("companySkills.newSkillTitle", { defaultValue: "New skill" }) }]
        : activeDetail
          ? skillDetailBreadcrumbs(activeDetail, skillFoldersQuery.data).slice(1)
          : routeSkillToken
            ? [{ label: t("companySkills.detailBreadcrumb", { defaultValue: "Detail" }) }]
            : []),
    ]);
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.skillsImported", { defaultValue: "Skills imported" }),
        body: t("companySkills.skillsAdded", { defaultValue: "{{count}} skill added.", count: result.imported.length }),
      });
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("companySkills.importWarnings", { defaultValue: "Import warnings" }), body: result.warnings[0] });
      }
      setSource("");
    },
    onError: (error) => {
      reportSkillError(error, t("companySkills.importFailedTitle", { defaultValue: "Skill import failed" }), t("companySkills.importFailedFallback", { defaultValue: "Failed to import skill source." }), t("companySkills.importActionLabel", { defaultValue: "Importing skills" }));
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onMutate: (projectId) => {
      setScanStatusMessage(
        projectId ? t("companySkills.refreshingProjectSkills", { defaultValue: "Refreshing project skills..." }) : t("companySkills.scanningWorkspaces", { defaultValue: "Scanning project workspaces for skills..." }),
      );
    },
    onSuccess: async (result) => {
      setScanStatusMessage(t("companySkills.refreshingSkillsList", { defaultValue: "Refreshing skills list..." }));
      await Promise.all([
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.scanCompleteTitle", { defaultValue: "Project skill scan complete" }),
        body: summary,
      });
      if (result.conflicts[0]) {
        pushToast({
          tone: "warn",
          title: t("companySkills.conflictsFoundTitle", { defaultValue: "Skill conflicts found" }),
          body: result.conflicts[0].reason,
        });
      } else if (result.warnings[0]) {
        pushToast({
          tone: "warn",
          title: t("companySkills.scanWarningsTitle", { defaultValue: "Scan warnings" }),
    ...
    onError: (error) => {
      ...
      reportSkillError(error, t("companySkills.scanFailedTitle", { defaultValue: "Project skill scan failed" }), t("companySkills.scanFailedFallback", { defaultValue: "Failed to scan project workspaces." }), t("companySkills.scanActionLabel", { defaultValue: "Scanning projects for skills" }));
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: skill.forkedFromSkillId ? t("companySkills.forkCreatedTitle", { defaultValue: "Skill fork created" }) : t("companySkills.createdTitle", { defaultValue: "Skill created" }),
        body: t("companySkills.createdBody", { defaultValue: "{{name}} is now editable in the Paperclip workspace.", name: skill.name }),
      });
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("companySkills.creationFailedFallback", { defaultValue: "Failed to create skill." });
      setCreateError(message);
      reportSkillError(error, t("companySkills.creationFailedTitle", { defaultValue: "Skill creation failed" }), t("companySkills.creationFailedFallback", { defaultValue: "Failed to create skill." }), t("companySkills.creationActionLabel", { defaultValue: "Creating a skill" }));
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.savedTitle", { defaultValue: "Skill saved" }),
        body: result.path,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.saveFailedTitle", { defaultValue: "Save failed" }),
        body: error instanceof Error ? error.message : t("companySkills.saveFailedFallback", { defaultValue: "Failed to save skill file." }),
      });
    },
  });
  ...
  const toggleStar = useMutation({
    mutationFn: () => {
      if (!activeDetail) throw new Error(t("companySkills.selectSkillFirst", { defaultValue: "Select a skill first." }));
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.starFailedTitle", { defaultValue: "Star failed" }),
        body: error instanceof Error ? error.message : t("companySkills.starFailedFallback", { defaultValue: "Failed to update star." }),
      });
    },
  });
  ...
  const updateSkillSettings = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({ tone: "success", title: t("companySkills.settingsUpdatedTitle", { defaultValue: "Skill settings updated" }), body: skillSettingsToastBody(skill) });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.settingsUpdateFailedTitle", { defaultValue: "Skill settings update failed" }),
        body: error instanceof Error ? error.message : t("companySkills.settingsUpdateFailedFallback", { defaultValue: "Failed to update skill settings." }),
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.updatedTitle", { defaultValue: "Skill updated" }),
        body: skill.sourceRef ? t("companySkills.pinnedToBody", { defaultValue: "Pinned to {{ref}}", ref: shortRef(skill.sourceRef) }) : skill.name,
      });
    },
    onError: (error) => {
      reportSkillError(error, t("companySkills.updateFailedTitle", { defaultValue: "Update failed" }), t("companySkills.updateFailedFallback", { defaultValue: "Failed to install skill update." }), t("companySkills.updateActionLabel", { defaultValue: "Updating this skill" }));
    },
  });
  ...
  const installCatalog = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: result.action === "created" ? t("companySkills.installedTitle", { defaultValue: "Skill installed" }) : result.action === "updated" ? t("companySkills.updatedTitle", { defaultValue: "Skill updated" }) : t("companySkills.upToDateTitle", { defaultValue: "Skill is up to date" }),
        body: result.skill.name,
      });
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("companySkills.installWarnings", { defaultValue: "Install warnings" }), body: result.warnings[0] });
      }
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("companySkills.installFailedFallback", { defaultValue: "Failed to install catalog skill." });
      ...
      policyDenial.capture(error, t("companySkills.installActionLabel", { defaultValue: "Installing this skill" }));
    },
  });
    ...
    onSuccess: async (folder) => {
      ...
      if (moveAfterCreateSkillIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            tone: "error",
            title: t("companySkills.folderCreatedMoveFailedTitle", { defaultValue: "Folder created, move failed" }),
            body: moveError instanceof Error ? moveError.message : t("companySkills.moveSelectedFailedFallback", { defaultValue: "Failed to move the selected skills." }),
          });
          return;
        }
      } else {
      ...
      pushToast({ tone: "success", title: t("companySkills.folderCreatedTitle", { defaultValue: "Folder created" }), body: folder.name });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.folderSaveFailedTitle", { defaultValue: "Folder save failed" }),
        body: error instanceof Error ? error.message : t("companySkills.folderSaveFailedFallback", { defaultValue: "Failed to save folder." }),
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.folderSaveFailedTitle", { defaultValue: "Folder save failed" }),
        body: error instanceof Error ? error.message : t("companySkills.folderUpdateFailedFallback", { defaultValue: "Failed to update folder." }),
      });
    },
  });
  const moveFolder = useMutation({
    ...
    onSuccess: async (folder) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.folderMovedTitle", { defaultValue: "Folder moved" }),
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.folderMoveFailedTitle", { defaultValue: "Folder move failed" }),
        body: error instanceof Error ? error.message : t("companySkills.folderMoveFailedFallback", { defaultValue: "Failed to move folder." }),
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onSuccess: async (_, folderId) => {
      ...
      pushToast({ tone: "success", title: t("companySkills.folderDeletedTitle", { defaultValue: "Folder deleted" }), body: t("companySkills.movedToUnfiledBody", { defaultValue: "Skills moved to Unfiled." }) });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.folderDeleteFailedTitle", { defaultValue: "Folder delete failed" }),
        body: error instanceof Error ? error.message : t("companySkills.folderDeleteFailedFallback", { defaultValue: "Failed to delete folder." }),
      });
    },
  });
  const moveSkillToFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.moveFailedTitle", { defaultValue: "Move failed" }),
        body: error instanceof Error ? error.message : t("companySkills.moveFailedFallback", { defaultValue: "Failed to move skill." }),
      });
    },
  });
  ...
  async function moveSelectedSkills(folderId: string | null) {
    ...
    try {
      ...
      pushToast({ tone: "success", title: t("companySkills.skillsMovedTitle", { defaultValue: "Skills moved" }), body: t("companySkills.skillsFiled", { defaultValue: "{{count}} skill filed.", count: ids.length }) });
    } catch (moveError) {
      pushToast({
        tone: "error",
        title: t("companySkills.moveSkillsFailedTitle", { defaultValue: "Failed to move skills" }),
        body: moveError instanceof Error ? moveError.message : t("companySkills.moveSelectedFailedFallback", { defaultValue: "Failed to move the selected skills." }),
      });
    }
  }
  ...
  const ensureMyFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.couldntOpenMySkills", { defaultValue: "Couldn't open My Skills" }),
        body: error instanceof Error ? error.message : t("companySkills.personalFolderFailed", { defaultValue: "Failed to create your personal folder." }),
      });
    },
  });
  ...
  async function createFolderInline(parentId: string | null, name: string): Promise<string | null> {
    try {
      ...
      pushToast({
        tone: "error",
        title: t("companySkills.folderCreateFailedTitle", { defaultValue: "Folder create failed" }),
        body: error instanceof Error ? error.message : t("companySkills.folderCreateFailedFallback", { defaultValue: "Failed to create folder." }),
      });
      return null;
    }
  }
  ...
  async function performDialogMove(folderId: string | null) {
    ...
    try {
      ...
      pushToast({
        tone: "success",
        title: ids.length === 1 ? t("companySkills.skillMovedTitle", { defaultValue: "Skill moved" }) : t("companySkills.skillsMovedTitle", { defaultValue: "Skills moved" }),
        body: folderId
          ? t("companySkills.filedUnderBody", { defaultValue: "Filed under {{name}}.", name: skillFolderResult?.folders.find((folder) => folder.id === folderId)?.name ?? t("companySkills.folderFallback", { defaultValue: "folder" }) })
          : t("companySkills.movedToUnfiledBodyShort", { defaultValue: "Moved to Unfiled." }),
      });
    } catch (moveError) {
      pushToast({
        tone: "error",
        title: t("companySkills.moveFailedTitle", { defaultValue: "Move failed" }),
        body: moveError instanceof Error ? moveError.message : t("companySkills.moveDialogFailedFallback", { defaultValue: "Failed to move." }),
      });
    }
  }
  ...
  async function handleAttachSubmit(nextAgentIds: string[], versionId: string | null = null) {
    ...
    try {
      ...
      pushToast({ tone: "success", title: t("companySkills.agentsUpdatedTitle", { defaultValue: "Agents updated" }), body: t("companySkills.agentsAttachedBody", { defaultValue: "{{count}} agent(s) attached.", count: nextAgentIds.length }) });
    } catch (error) {
      pushToast({ tone: "error", title: t("companySkills.updateFailedTitle", { defaultValue: "Update failed" }), body: error instanceof Error ? error.message : t("companySkills.updateAgentSkillsFallback", { defaultValue: "Failed to update agent skills." }) });
    }
  }
  ...
  const deleteSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.skillRemovedTitle", { defaultValue: "Skill removed" }),
        body: t("companySkills.removedBody", { defaultValue: "{{name}} was removed from the company skill library.", name: skill.name }),
      });
    },
    onError: (error) => {
      reportSkillError(error, t("companySkills.removeFailedTitle", { defaultValue: "Remove failed" }), t("companySkills.removeFailedFallback", { defaultValue: "Failed to remove skill." }), t("companySkills.removeActionLabel", { defaultValue: "Removing this skill" }));
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Boxes} message={t("companySkills.selectCompanySkills", { defaultValue: "Select a company to manage skills." })} />;
  }
  ...
  const studioTitle = studioForkFromId ? t("companySkills.forkSkillTitle", { defaultValue: "Fork skill" }) : t("companySkills.studioCreateTitle", { defaultValue: "Create a new skill" });
  const studioDescription = studioForkFromId
    ? t("companySkills.studioForkDescription", { defaultValue: "Review the fork metadata and create an editable company copy." })
    : t("companySkills.studioCreateDescription", { defaultValue: "Create an editable company skill in the Paperclip workspace." });
  return (
    <>
      {policyDenial.denial ? (
      ...
      <Dialog open={deleteOpen} onOpenChange={closeDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("companySkills.removeSkillDescription", { defaultValue: "Remove this skill from the company library. If any agents still use it, removal will be blocked until it is detached." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <p>
              {deleteTargetDetail
                ? t("companySkills.aboutToRemoveName", { defaultValue: "You are about to remove {{name}}.", name: deleteTargetDetail.name })
                : t("companySkills.aboutToRemoveSkill", { defaultValue: "You are about to remove this skill." })}
            </p>
            {deleteTargetDetail?.usedByAgents?.length ? (
              <div className="rounded-md border border-border px-3 py-3 text-muted-foreground">
                {t("companySkills.currentlyUsedBy", { defaultValue: "Currently used by {{names}}.", names: deleteTargetDetail.usedByAgents.map((agent) => agent.name).join(", ") })}.
              </div>
            ) : null}
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              <p className="text-muted-foreground">
                {t("companySkills.detachToEnableRemoval", { defaultValue: "Detach this skill from all agents to enable removal." })}
              </p>
            ) : null}
          </div>
          <DialogFooter>
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              <Button variant="ghost" onClick={() => closeDeleteDialog(false)}>
                {t("companySkills.closeButton", { defaultValue: "Close" })}
              </Button>
            ) : (
              <>
                ...
                >
                  {deleteSkill.isPending ? t("companySkills.removingDots", { defaultValue: "Removing..." }) : t("companySkills.removeSkillButton", { defaultValue: "Remove skill" })}
                </Button>
              </>
            )}
          </DialogFooter>
      ...
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.addSkillSourceTitle", { defaultValue: "Add a skill source" })}</DialogTitle>
            <DialogDescription>
              {t("companySkills.addSkillSourceDescription", { defaultValue: "Paste a local path, GitHub URL, or `skills.sh` command into the field first." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <a
            ...
            >
              <span>
                ...
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.browseSkillsShHint", { defaultValue: "Find install commands and paste one here." })}
                </span>
              </span>
            ...
            >
              <span>
                ...
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.searchGitHubHint", { defaultValue: "Look for repositories with `SKILL.md`, then paste the repo URL here." })}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.importSkillTitle", { defaultValue: "Import a skill" })}</DialogTitle>
            <DialogDescription>
              {t("companySkills.importSkillDescription", { defaultValue: "Paste a local path, GitHub URL, or `skills.sh` command to import a skill into this company." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="flex items-center gap-2 border-b border-border pb-2">
              ...
              <Button size="sm" onClick={handleAddSkillSource} disabled={importSkill.isPending}>
                {importSkill.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : t("companySkills.importButton", { defaultValue: "Import" })}
              </Button>
            </div>
            <a
              href="https://skills.sh"
            ...
            >
              <span>
                <span className="block font-medium">{t("companySkills.browseSkillsSh", { defaultValue: "Browse skills.sh" })}</span>
                <span className="mt-1 block text-muted-foreground">{t("companySkills.browseSkillsShHint", { defaultValue: "Find install commands and paste one here." })}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("companySkills.searchGitHub", { defaultValue: "Search GitHub" })}</span>
                <span className="mt-1 block text-muted-foreground">{t("companySkills.searchGitHubHintShort", { defaultValue: "Look for repositories with `SKILL.md`, then paste the repo URL." })}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
          </div>
      ...
      <MobileFolderSheet
        ...
        selection={folderSelection}
        allLabel={t("companySkills.allSkills", { defaultValue: "All skills" })}
        itemLabelPlural={t("companySkills.skillsNounPluralCap", { defaultValue: "Skills" })}
      ...
      <MoveToFolderDialog
        ...
        result={railSkillFolderResult}
        title={moveDialog?.title ?? t("companySkills.moveToFolderTitle", { defaultValue: "Move to folder" })}
      ...
      {isStudioNew ? (
          ...
          <div className="px-4 py-4">
            <div className="max-w-3xl">
              {studioForkFromId && studioForkDetailQuery.isLoading ? (
                ...
                <EmptyState icon={Boxes} message={t("companySkills.forkSourceNotFound", { defaultValue: "Fork source skill not found." })} />
              ) : (
                <NewSkillWizard
        ...
        <DiscoveryGrid
          ...
          onOpenMoveCard={showInstalledFolders ? (card) => {
            ...
            setMoveDialog({
              ...
              title: t("companySkills.moveCardTitle", { defaultValue: "Move \"{{name}}\"", name: card.name }),
              subtitle: t("companySkills.chooseDestination", { defaultValue: "Choose a destination folder." }),
          ...
          onMoveCard={showInstalledFolders ? (card, folderId) => {
            ...
            pushToast({
              tone: "success",
              title: t("companySkills.skillMovedTitle", { defaultValue: "Skill moved" }),
              body: folderId
                ? t("companySkills.movedCardBody", { defaultValue: "Moved \"{{name}}\" to {{folder}}.", name: card.name, folder: skillFolderResult?.folders.find((folder) => folder.id === folderId)?.name ?? t("companySkills.folderFallback", { defaultValue: "folder" }) })
                : t("companySkills.movedCardToUnfiledBody", { defaultValue: "Moved \"{{name}}\" to Unfiled.", name: card.name }),
              action: {
                label: t("companySkills.undoLabel", { defaultValue: "Undo" }),
        ...
        <SkillDetailPage
          ...
          onMoveToFolder={activeDetail ? () => setMoveDialog({
            ...
            title: t("companySkills.moveCardTitle", { defaultValue: "Move \"{{name}}\"", name: activeDetail.name }),
            subtitle: t("companySkills.chooseDestination", { defaultValue: "Choose a destination folder." }),
        ...
        <div className="min-h-(--sz-calc-30)">
          <div className="border-b border-border px-4 py-3">
            ...
            >
              <ChevronLeft className="h-4 w-4" />
              {t("companySkills.backToStore", { defaultValue: "Back to store" })}
            </Link>
          </div>
        ...
        <div className="min-h-(--sz-calc-30)">
          {skillsQuery.isLoading ? (
            ...
            <EmptyState icon={Boxes} message={t("companySkills.skillNotFound", { defaultValue: "Skill not found." })} />
          )}
        </div>
      )}
    </>
