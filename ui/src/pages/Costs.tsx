  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costsPage.financeLedger", { defaultValue: "Finance ledger" })}</CardTitle>
        <CardDescription>
          {t("costsPage.financeLedgerDescription", { defaultValue: "Account-level charges that do not map to a single inference request." })}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costsPage.debits", { defaultValue: "Debits" })}
          value={formatCents(debitCents)}
          subtitle={t("costsPage.debitsSubtitle", { defaultValue: "{{count}} total events in range", count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costsPage.credits", { defaultValue: "Credits" })}
          value={formatCents(creditCents)}
          subtitle={t("costsPage.creditsSubtitle", { defaultValue: "Refunds, offsets, and credit returns" })}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costsPage.net", { defaultValue: "Net" })}
          value={formatCents(netCents)}
          subtitle={t("costsPage.netSubtitle", { defaultValue: "Debit minus credit for the selected period" })}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costsPage.estimated", { defaultValue: "Estimated" })}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costsPage.estimatedSubtitle", { defaultValue: "Estimated debits that are not yet invoice-authoritative" })}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  const presetLabels: Record<DatePreset, string> = {
    mtd: t("costsPage.preset.mtd", { defaultValue: "Month to Date" }),
    "7d": t("costsPage.preset.7d", { defaultValue: "Last 7 Days" }),
    "30d": t("costsPage.preset.30d", { defaultValue: "Last 30 Days" }),
    ytd: t("costsPage.preset.ytd", { defaultValue: "Year to Date" }),
    all: t("costsPage.preset.all", { defaultValue: "All Time" }),
    custom: t("costsPage.preset.custom", { defaultValue: "Custom" }),
  };
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costsPage.allProviders", { defaultValue: "All providers" })}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costsPage.allBillers", { defaultValue: "All billers" })}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costsPage.selectCompany", { defaultValue: "Select a company to view costs." })} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("costsPage.pageDescription", { defaultValue: "Inference spend, platform fees, credits, and live quota windows." })}
                </p>
            </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("timeline.to", { defaultValue: "to" })}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("costsPage.inferenceSpend", { defaultValue: "Inference spend" })}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("costsPage.inferenceSpendSubtitle", { defaultValue: "{{tokens}} tokens across request-scoped events", tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costsPage.budget", { defaultValue: "Budget" })}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : t("costsPage.open", { defaultValue: "Open" })
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costsPage.budgetPausedSubtitle", { defaultValue: "{{agents}} agents paused · {{projects}} projects paused", agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costsPage.spendOfBudget", { defaultValue: "{{spend}} of {{budget}}", spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                    : t("costsPage.noMonthlyCap", { defaultValue: "No monthly cap configured" })
              }
              icon={Coins}
            />
            <MetricTile
              label={t("costsPage.financeNet", { defaultValue: "Finance net" })}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("costsPage.financeNetSubtitle", { defaultValue: "{{debits}} debits · {{credits}} credits", debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("costsPage.financeEvents", { defaultValue: "Finance events" })}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costsPage.financeEventsSubtitle", { defaultValue: "{{amount}} estimated in range", amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("costsPage.overview", { defaultValue: "Overview" })}</TabsTrigger>
          <TabsTrigger value="budgets">{t("costsPage.budgets", { defaultValue: "Budgets" })}</TabsTrigger>
          <TabsTrigger value="providers">{t("costsPage.providers", { defaultValue: "Providers" })}</TabsTrigger>
          <TabsTrigger value="billers">{t("costsPage.billers", { defaultValue: "Billers" })}</TabsTrigger>
          <TabsTrigger value="finance">{t("costsPage.finance", { defaultValue: "Finance" })}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costsPage.selectStartEnd", { defaultValue: "Select a start and end date to load data." })}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costsPage.inferenceLedger", { defaultValue: "Inference ledger" })}</CardTitle>
                    <CardDescription>
                      {t("costsPage.inferenceLedgerDescription", { defaultValue: "Request-scoped inference spend for the selected period." })}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costsPage.budgetAmount", { defaultValue: "Budget {{amount}}", amount: formatCents(spendData.summary.budgetCents) })
                            : t("dashboard.unlimitedBudget", { defaultValue: "Unlimited budget" })}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("costsPage.usage", { defaultValue: "usage" })}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costsPage.utilizationOfBudget", { defaultValue: "{{percent}}% of monthly budget consumed in this range.", percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costsPage.byAgent", { defaultValue: "By agent" })}</CardTitle>
                    <CardDescription>{t("costsPage.byAgentDescription", { defaultValue: "What each agent consumed in the selected period." })}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costsPage.noCostEvents", { defaultValue: "No cost events yet." })}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costsPage.inOutTokens", { defaultValue: "in {{input}} · out {{output}}", input: formatTokens(row.inputTokens + row.cachedInputTokens), output: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costsPage.apiRuns", { defaultValue: "{{count}} api", count: row.apiRunCount })}
                                    {" · "}
                                    {t("costsPage.subscriptionRuns", { defaultValue: "{{count}} subscription", count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("costsPage.tokensShort", { defaultValue: "{{count}} tok", count: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costsPage.byProject", { defaultValue: "By project" })}</CardTitle>
                      <CardDescription>{t("costsPage.byProjectDescription", { defaultValue: "Run costs attributed through project-linked tasks." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costsPage.noProjectCosts", { defaultValue: "No project-attributed run costs yet." })}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("costsPage.unattributed", { defaultValue: "Unattributed" })}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costsPage.noFinanceEventsYet", { defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land." })} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costsPage.budgetControlPlane", { defaultValue: "Budget control plane" })}</CardTitle>
                  <CardDescription>
                    {t("costsPage.budgetControlPlaneDescription", { defaultValue: "Hard-stop spend limits for agents and projects. Provider subscription quota stays separate and appears under Providers." })}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costsPage.activeIncidents", { defaultValue: "Active incidents" })}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costsPage.activeIncidentsSubtitle", { defaultValue: "Open soft or hard threshold crossings" })}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costsPage.pendingApprovals", { defaultValue: "Pending approvals" })}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costsPage.pendingApprovalsSubtitle", { defaultValue: "Budget override approvals awaiting board action" })}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costsPage.pausedAgents", { defaultValue: "Paused agents" })}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costsPage.pausedAgentsSubtitle", { defaultValue: "Agent heartbeats blocked by budget" })}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costsPage.pausedProjects", { defaultValue: "Paused projects" })}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costsPage.pausedProjectsSubtitle", { defaultValue: "Project execution blocked by budget" })}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costsPage.activeIncidents", { defaultValue: "Active incidents" })}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costsPage.activeIncidentsHint", { defaultValue: "Resolve hard stops here by raising the budget or explicitly keeping the scope paused." })}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t("costsPage.scopeBudgets", { defaultValue: "{{scope}} budgets", scope: scopeType })}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costsPage.companyBudgetsDescription", { defaultValue: "Company-wide monthly policy." })
                            : scopeType === "agent"
                              ? t("costsPage.agentBudgetsDescription", { defaultValue: "Recurring monthly spend policies for individual agents." })
                              : t("costsPage.projectBudgetsDescription", { defaultValue: "Lifetime spend policies for execution-bound projects." })}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costsPage.noBudgetPolicies", { defaultValue: "No budget policies yet. Set agent and project budgets from their detail pages, or use the existing company monthly budget control." })}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costsPage.noCostEventsInPeriod", { defaultValue: "No cost events in this period." })}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costsPage.noBillableEvents", { defaultValue: "No billable events in this period." })}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costsPage.byBiller", { defaultValue: "By biller" })}</CardTitle>
                      <CardDescription>{t("costsPage.byBillerDescription", { defaultValue: "Account-level financial events grouped by who charged or credited them." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costsPage.noFinanceEvents", { defaultValue: "No finance events yet." })}</p>
                      ) : (
