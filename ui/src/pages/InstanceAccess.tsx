...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("instanceSettings.breadcrumb", { defaultValue: "Instance settings" }), href: "/company/settings/instance/general" },
      { label: t("instanceAccess.breadcrumb", { defaultValue: "Access" }) },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceAccess.accessUpdatedToast", { defaultValue: "Company access updated" }), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceAccess.roleUpdatedToast", { defaultValue: "Instance role updated" }), tone: "success" });
    },
  });
  ...
  if (usersQuery.isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        {t("instanceAccess.loadingUsers", { defaultValue: "Loading instance users…" })}
      </div>
    );
  }
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">
            {t("instanceAccess.title", { defaultValue: "Instance Access" })}
          </h1>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-(--gtc-34)">
        <Card className="block space-y-4 p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">
              {t("instanceAccess.searchUsers", { defaultValue: "Search users" })}
            </span>
            <input
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("instanceAccess.activeMemberships", {
        ...
        <Card className="block space-y-4 p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">
              {t("instanceAccess.selectUserHint", { defaultValue: "Select a user to inspect instance access." })}
            </div>
          ) : userAccessQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">
              {t("instanceAccess.loadingAccess", { defaultValue: "Loading user access…" })}
            </div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error
                ? userAccessQuery.error.message
                : t("instanceAccess.loadAccessFailed", { defaultValue: "Failed to load user access." })}
            </div>
          ) : (
            <>
              ...
              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">
                    {t("instanceAccess.companyAccessTitle", { defaultValue: "Company access" })}
                  </h2>
              ...
              <div className="space-y-2">
                <h2 className="text-sm font-semibold">
                  {t("instanceAccess.currentMemberships", { defaultValue: "Current memberships" })}
                </h2>
                <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {membership.membershipRole
                            || t("instanceAccess.membershipRoleUnset", { defaultValue: "unset" })}{" • "}{membership.status}
                        </div>
                      </div>
