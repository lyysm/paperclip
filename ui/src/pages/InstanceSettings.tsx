...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.updateFailed", { defaultValue: "Failed to update heartbeat." }));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error
          ? firstError.message
          : t("instanceSettings.unknownError", { defaultValue: "Unknown error" });
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.disableOneFailed", {
                defaultValue: "Failed to disable 1 timer heartbeat: {{detail}}",
                detail
              })
            : t("instanceSettings.disableManyFailed", {
                defaultValue: "Failed to disable {{failed}} of {{total}} timer heartbeats. First error: {{detail}}",
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.disableAllFailed", { defaultValue: "Failed to disable all heartbeats." }));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        {t("instanceSettings.loading", { defaultValue: "Loading scheduler heartbeats..." })}
      </div>
    );
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceSettings.loadFailed", { defaultValue: "Failed to load scheduler heartbeats." })}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">
            {t("instanceSettings.title", { defaultValue: "Scheduler Heartbeats" })}
          </h1>
        </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span>
          ...
          {t("instanceSettings.statActive", { defaultValue: "active" })}
        </span>
        <span>
          <span className="font-semibold text-foreground">{disabledCount}</span>{" "}
          {t("instanceSettings.statDisabled", { defaultValue: "disabled" })}
        </span>
        <span>
          <span className="font-semibold text-foreground">{grouped.length}</span>{" "}
          {t("instanceSettings.statCompanies", { defaultValue: "company", count: grouped.length })}
        </span>
        {anyEnabled && (
          <Button
            variant="destructive"
            ...
            onClick={() => {
              if (
                !window.confirm(
                  t("instanceSettings.disableAllConfirm", {
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.emptyMessage", {
