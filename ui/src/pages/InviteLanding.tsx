...
function mapInviteAuthFeedback(
  ...
  if (code === "USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL") {
    return {
      tone: "info",
      message: t("inviteLanding.accountAlreadyExists", {
      ...
      <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6" data-testid="invite-pending-approval">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="border border-zinc-800 p-3">
            ...
            <p className="text-sm text-zinc-200">{t("inviteLanding.settingsMembers", { defaultValue: "Settings → Members" })}</p>
          </div>
          <p className="text-sm text-zinc-400">
            {t("inviteLanding.askToApprove", {
              defaultValue: "Ask them to visit Settings → Members to approve your request.",
            })}
          </p>
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || t("inviteLanding.thisPaperclipCompany", { defaultValue: "this Paperclip company" });
  ...
  const requestedHumanRole = invite?.humanRole
    ? t(`inviteLanding.role.${invite.humanRole}`, { defaultValue: formatHumanRole(invite.humanRole) ?? invite.humanRole })
    : null;
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isCheckingExistingMembership) {
        throw new Error(t("inviteLanding.checkingAccessRetry", { defaultValue: "Checking your company access. Try again in a moment." }));
      }
      if (isCurrentMember) {
        throw new Error(t("inviteLanding.alreadyMember", { defaultValue: "This account already belongs to the company." }));
      }
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("inviteLanding.acceptFailed", { defaultValue: "Failed to accept invite" }));
    },
  });
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.inviteUnavailableDescription", { defaultValue: "This invite may be expired, revoked, or already used." })}
          </p>
        </div>
      </div>
    );
        ...
        <div className="grid gap-6 lg:grid-cols-(--gtc-36)">
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">
                  {t("inviteLanding.invitedToJoin", { defaultValue: "You've been invited to join Paperclip" })}
                </p>
                <h1 className="mt-2 text-2xl font-semibold">
                  {invite.inviteType === "bootstrap_ceo"
                    ? t("inviteLanding.setupPaperclip", { defaultValue: "Set up Paperclip" })
                    : t("inviteLanding.joinCompany", { defaultValue: "Join {{company}}", company: companyDisplayName })}
                </h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ? t("inviteLanding.reviewAgentInvite", { defaultValue: "Review the invite details, then submit the agent information below to start the join request." })
                    : requiresHumanAccount
                      ? t("inviteLanding.createAccountFirst", { defaultValue: "Create your Paperclip account first. If you already have one, switch to sign in and continue the invite with the same email." })
                      : t("inviteLanding.accountReady", { defaultValue: "Your account is ready. Review the invite details, then accept it to continue." })}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("inviteLanding.company", { defaultValue: "Company" })}</div>
              ...
              <div className="border border-zinc-800 p-3">
                ...
                <div className="mt-1 text-sm text-zinc-100">
                  {showsAgentForm ? t("inviteLanding.agentJoinRequest", { defaultValue: "Agent join request" }) : requestedHumanRole ?? t("inviteLanding.companyAccess", { defaultValue: "Company access" })}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
            ...
            {sessionQuery.data ? (
              <div className="border border-emerald-500/40 bg-emerald-500/10 p-4 text-sm text-emerald-50">
                {t("inviteLanding.signedInAs", { defaultValue: "Signed in as {{account}}.", account: sessionLabel })}
              </div>
            ) : null}
          </section>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {t("inviteLanding.agentApprovalRequest", { defaultValue: "This invite will create an approval request for a new agent in {{company}}.", company: companyDisplayName })}
                  </p>
                </div>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.agentName", { defaultValue: "Agent name" })}</span>
                  <input
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.adapterType", { defaultValue: "Adapter type" })}</span>
                  <select
                    className={fieldClassName}
                    value={adapterType}
                  ...
                  >
                    {joinAdapterOptions.map((type) => (
                      <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                        {getAdapterLabel(type)}{!ENABLED_INVITE_ADAPTERS.has(type) ? ` (${t("inviteLanding.comingSoon", { defaultValue: "Coming soon" })})` : ""}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.capabilities", { defaultValue: "Capabilities" })}</span>
                  <textarea
                    className={fieldClassName}
                    rows={4}
                    value={capabilities}
              ...
              <div className="space-y-5">
                <div>
                  <h2 className="text-lg font-semibold">
                    {authMode === "sign_up" ? t("inviteLanding.createAccount", { defaultValue: "Create your account" }) : t("inviteLanding.signInToContinue", { defaultValue: "Sign in to continue" })}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? t("inviteLanding.createAccountDescription", { defaultValue: "Start with a Paperclip account. After that, you'll come right back here to accept the invite for {{company}}.", company: companyDisplayName })
                      : t("inviteLanding.signInDescription", { defaultValue: "Use the Paperclip account that already matches this invite. If you do not have one yet, switch back to create account." })}
                  </p>
                </div>
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    if (!authCanSubmit) {
                      setAuthFeedback({ tone: "error", message: t("inviteLanding.requiredFields", { defaultValue: "Please fill in all required fields." }) });
                      return;
                    }
                    authMutation.mutate();
                  }}
                ...
                >
                  {authMode === "sign_up" ? (
                    <label className="block text-sm" htmlFor="invite-name">
                      <span className="mb-1 block text-zinc-400">{t("inviteLanding.name", { defaultValue: "Name" })}</span>
                      <input
                        id="invite-name"
                        name="name"
                        className={fieldClassName}
                  ...
                  <label className="block text-sm" htmlFor="invite-email">
                    <span className="mb-1 block text-zinc-400">{t("inviteLanding.email", { defaultValue: "Email" })}</span>
                    <input
                      id="invite-email"
                      name="email"
                      type="email"
                  ...
                  <label className="block text-sm" htmlFor="invite-password">
                    <span className="mb-1 block text-zinc-400">{t("inviteLanding.password", { defaultValue: "Password" })}</span>
                    <input
                      id="invite-password"
                      name="password"
                      type="password"
                ...
                <p className="text-xs leading-5 text-zinc-500">
                  {authMode === "sign_up"
                      ? t("inviteLanding.alreadySignedUp", { defaultValue: "Already signed up before? Use the existing-account option instead so the invite lands on the right Paperclip user." })
                    : t("inviteLanding.noAccountYet", { defaultValue: "No account yet? Switch back to create account so you can accept the invite with a new login." })}
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">
                    {isCurrentMember
                      ...
                      ? t("inviteLanding.completingAccess", { defaultValue: "Completing company access" })
                      : invite.inviteType === "bootstrap_ceo"
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ? t("inviteLanding.grantingAccess", { defaultValue: "Granting your access to {{company}}.", company: companyDisplayName })
                      : isCurrentMember
                      ? t("inviteLanding.accountAlreadyBelongs", { defaultValue: "This account already belongs to {{company}}.", company: companyDisplayName })
                      : invite.inviteType === "bootstrap_ceo"
                        ? t("inviteLanding.finishSetupDescription", { defaultValue: "This will finish setting up Paperclip." })
                        : t("inviteLanding.grantAccessDescription", { defaultValue: "This will grant or complete your access to {{company}}.", company: companyDisplayName })}
                  </p>
                </div>
                {error ? <p className="text-xs text-red-400">{error}</p> : null}
                {shouldAutoAcceptHumanInvite ? (
