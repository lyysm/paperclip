...
export function Issues() {
  ...
  const issueLinkState = useMemo(
    () =>
      createIssueDetailLocationState(
        t("issuesPage.title", { defaultValue: "Tasks" }),
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("issuesPage.title", { defaultValue: "Tasks" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  if (!selectedCompanyId) {
    return (
      <EmptyState
        icon={CircleDot}
        message={t("issuesPage.selectCompany", { defaultValue: "Select a company to view tasks." })}
      />
    );
  }
