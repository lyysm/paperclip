...
export function NewAgent() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("newAgentForm.title", { defaultValue: "New Agent" }) },
    ]);
  }, [setBreadcrumbs]);
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("newAgentForm.createFailed", { defaultValue: "Failed to create agent" }));
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      if (!isValidOpenCodeModelId(configValues.model)) {
        setFormError(t("newAgentForm.openCodeModelRequired", { defaultValue: "OpenCode requires an explicit model in provider/model format." }));
        return;
      }
    }
    createAgent.mutate(
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-lg font-semibold">{t("newAgentForm.title", { defaultValue: "New Agent" })}</h1>
        <p className="text-sm text-muted-foreground mt-1">
          {t("newAgentForm.subtitle", { defaultValue: "Advanced agent configuration" })}
        </p>
      </div>
      ...
      <div className="border border-border">
        ...
        <div className="px-4 pt-4 pb-2">
          <input
            ...
            placeholder={t("newAgentForm.namePlaceholder", { defaultValue: "Agent name" })}
            value={name}
        ...
        <div className="px-4 pb-2">
          <input
            ...
            placeholder={t("newAgentForm.titlePlaceholder", { defaultValue: "Title (e.g. VP of Engineering)" })}
            value={title}
        ...
        <div className="border-t border-border px-4 py-4">
          <div className="space-y-3">
            <div>
              <h2 className="text-sm font-medium">{t("newAgentForm.companySkills", { defaultValue: "Company skills" })}</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("newAgentForm.companySkillsHelper", { defaultValue: "Optional skills from the company library. Built-in Paperclip runtime skills are added automatically." })}
              </p>
            </div>
            {availableSkills.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {t("newAgentForm.noCompanySkills", { defaultValue: "No optional company skills installed yet." })}
              </p>
            ) : (
              <div className="space-y-3">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("newAgentForm.firstAgentNote", { defaultValue: "This will be the CEO" })}</p>
          )}
          {formError && (
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {testAgentState.pending
                    ? t("newAgentForm.testing", { defaultValue: "Testing..." })
                    : t("newAgentForm.testAgent", { defaultValue: "Test Agent" })}
                </Button>
                <Button
                  size="sm"
                ...
                >
                  {createAgent.isPending
                    ? t("newAgentForm.creating", { defaultValue: "Creating…" })
                    : t("newAgentForm.createAgent", { defaultValue: "Create agent" })}
                </Button>
              </div>
            </div>
          </div>
