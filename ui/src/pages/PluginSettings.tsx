...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("pluginSettings.breadcrumbPlugins", { defaultValue: "Plugins" }), href: "/company/settings/instance/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("pluginSettings.breadcrumbPluginDetails", { defaultValue: "Plugin Details" }) },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("pluginSettings.loadingPluginDetails", { defaultValue: "Loading plugin details..." })}</div>;
  }
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: t("pluginSettings.tabConfiguration", { defaultValue: "Configuration" }) },
            { value: "status", label: t("nav.status", { defaultValue: "Status" }) },
          ]}
          value={activeTab}
            ...
            <section className="space-y-4">
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("pluginSettings.configureViaEnvironments", { defaultValue: "Configure this plugin from Settings → Environments." })}</p>
                  <p className="mt-1 text-muted-foreground">
                    {t("pluginSettings.environmentsHint", {
                      defaultValue:
                        "{{name}} registers environment runtime settings there so the execution target stays instance-scoped while secret bindings still resolve through the selected company context.",
                      name: driverLabel || t("pluginSettings.thisPlugin", { defaultValue: "This plugin" }),
                    })}
                  </p>
                  <div className="mt-3">
                    <Link to="/company/settings/instance/environments">
                      <Button variant="outline" size="sm">{t("pluginSettings.openEnvironments", { defaultValue: "Open Environments" })}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
                  ...
                  {dashboardData ? (
                    <>
                        ...
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("nav.status", { defaultValue: "Status" })}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("pluginSettings.noWorker", { defaultValue: "No worker process registered." })}</p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        ...
                        {t("pluginSettings.lastChecked", { defaultValue: "Last checked: {{time}}", time: new Date(dashboardData.checkedAt).toLocaleTimeString() })}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("pluginSettings.runtimeUnavailable", { defaultValue: "Runtime diagnostics are unavailable right now." })}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("pluginSettings.lastLogEntries", { defaultValue: "Last {{count}} log entries", count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <CardContent>
                  {healthLoading ? (
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      ...
                      <p>{t("pluginSettings.healthChecksPending", { defaultValue: "Health checks run once the plugin is ready." })}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{t("pluginSettings.pluginIdLabel", { defaultValue: "Plugin ID" })}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("pluginSettings.pluginKeyLabel", { defaultValue: "Plugin Key" })}</span>
    ...
    <div className="space-y-3">
      ...
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t("pluginSettings.loadLocalFoldersFailed", { defaultValue: "Failed to load local folder settings." })}
        </div>
      ) : null}
      {isLoading ? (
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("pluginSettings.saveLocalFolderFailed", { defaultValue: "Failed to save local folder." }) });
    },
  });
  ...
  const handleSave = useCallback(() => {
    ...
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("pluginSettings.localFolderAbsolutePath", { defaultValue: "Local folder must be a full absolute path." }) });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? t("pluginSettings.accessReadWrite", { defaultValue: "Read/write" }) : t("pluginSettings.accessReadOnly", { defaultValue: "Read only" })}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric
          label={t("pluginSettings.configured", { defaultValue: "Configured" })}
          value={status?.configured ? t("pluginSettings.yes", { defaultValue: "Yes" }) : t("pluginSettings.no", { defaultValue: "No" })}
          ok={!!status?.configured}
        />
        <FolderStatusMetric
          label={t("pluginSettings.readable", { defaultValue: "Readable" })}
          value={status?.readable ? t("pluginSettings.yes", { defaultValue: "Yes" }) : t("pluginSettings.no", { defaultValue: "No" })}
          ok={!!status?.readable}
        />
        <FolderStatusMetric
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {t("pluginSettings.localFolderPathLabel", { defaultValue: "Local folder path" })}
        </label>
        <div className="flex items-center gap-2">
          <input
          ...
          >
            ...
            {t("pluginSettings.save", { defaultValue: "Save" })}
          </Button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          ...
          <Badge variant="destructive" className="text-(length:--text-nano)">
            {t("pluginSettings.missing", { defaultValue: "{{count}} missing", count: missingItems.length })}
          </Badge>
        ) : (
          <Badge variant="outline" className="text-(length:--text-nano)">
            {t("pluginSettings.present", { defaultValue: "Present" })}
          </Badge>
        )}
      </div>
      {items.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">
          {t("pluginSettings.noneDeclared", { defaultValue: "None declared." })}
        </p>
      )}
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">
          {t("pluginSettings.configuredRootNotInspected", { defaultValue: "Configured root was not inspected." })}
        </p>
      ) : missingItems.length > 0 ? (
...
function PluginConfigForm({ pluginId, companyId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: (configJson: Record<string, unknown>) => {
      if (!companyId) {
        throw new Error(
          t("pluginSettings.selectCompanyBeforeSave", {
            defaultValue: "Select a company before saving plugin configuration.",
          }),
        );
      }
    ...
    onSuccess: () => {
      setSaveMessage({
        type: "success",
        text: t("pluginSettings.configurationSaved", { defaultValue: "Configuration saved." }),
      });
      setTestResult(null);
      if (companyId) {
    ...
    onError: (err: Error) => {
      setSaveMessage({
        ...
        text:
          err.message ||
          t("pluginSettings.saveConfigurationFailed", { defaultValue: "Failed to save configuration." }),
      });
    },
  });
  ...
  const testMutation = useMutation({
    mutationFn: (configJson: Record<string, unknown>) => {
      if (!companyId) {
        throw new Error(
          t("pluginSettings.selectCompanyBeforeTest", {
            defaultValue: "Select a company before testing plugin configuration.",
          }),
        );
      }
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({
          type: "success",
          text: t("pluginSettings.configurationTestPassed", { defaultValue: "Configuration test passed." }),
        });
      } else {
        setTestResult({
          type: "error",
          text:
            result.message ||
            t("pluginSettings.configurationTestFailed", { defaultValue: "Configuration test failed." }),
        });
      }
    },
    onError: (err: Error) => {
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t("pluginSettings.savingConfiguration", { defaultValue: "Saving…" })}
            </>
          ) : (
        ...
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("pluginSettings.testingConfiguration", { defaultValue: "Testing…" })}
              </>
            ) : (
