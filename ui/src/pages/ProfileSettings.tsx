...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("profileSettings.breadcrumb", { defaultValue: "Profile" }) },
    ]);
  }, [setBreadcrumbs]);
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("profileSettings.boardFallback", { defaultValue: "Board" });
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.updateFailed", { defaultValue: "Failed to update profile." }));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profileSettings.selectCompanyFirst", {
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.avatarUploadFailed", { defaultValue: "Failed to upload avatar." }));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.avatarRemoveFailed", { defaultValue: "Failed to remove avatar." }));
    },
  });
  ...
  if (sessionQuery.isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        {t("profileSettings.loading", { defaultValue: "Loading profile..." })}
      </div>
    );
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error
          ...
          : t("profileSettings.loadFailed", { defaultValue: "Failed to load profile." })}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("profileSettings.boardFallback", { defaultValue: "Board" });
  ...
  const uploadHint = selectedCompany
    ? t("profileSettings.uploadHintWithCompany", {
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">
            {t("profileSettings.title", { defaultValue: "Profile" })}
          </h1>
        </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage
                      ? t("profileSettings.changePhoto", { defaultValue: "Change photo" })
                      : t("profileSettings.uploadPhoto", { defaultValue: "Upload photo" })}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">
                    {sessionQuery.data.user.email ?? t("profileSettings.noEmail", { defaultValue: "No email" })}
                  </p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profileSettings.clickAvatarHint", {
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">
              {t("profileSettings.displayName", { defaultValue: "Display name" })}
            </Label>
            <Input
              id="profile-name"
              value={name}
              ...
              maxLength={120}
              placeholder={t("profileSettings.boardFallback", { defaultValue: "Board" })}
            />
          ...
          <div className="space-y-2">
            <Label htmlFor="profile-email">
              {t("profileSettings.email", { defaultValue: "Email" })}
            </Label>
            <Input
              id="profile-email"
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending
                ? t("companySettings.saving", { defaultValue: "Saving..." })
                : t("profileSettings.saveProfile", { defaultValue: "Save profile" })}
            </Button>
          </div>
        </form>
