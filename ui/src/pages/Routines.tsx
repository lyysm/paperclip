...
const defaultRoutineGroupLabels: RoutineGroupLabels = {
  ...
  builtInRoutines: "Built-in routines",
};
  ...
  if (groupByValue === "folder") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        ...
        const labelCompare = (leftFolder?.name ?? labels.unknownFolder).localeCompare(
          rightFolder?.name ?? labels.unknownFolder,
          undefined,
          { sensitivity: "base" },
        );
      ...
      .map((key) => ({
        key,
        label: key === "__unfiled" ? labels.unfiled : (folderById.get(key)?.name ?? labels.unknownFolder),
        items: groups[key]!,
      }));
  }
...
export function buildRoutineSections(
  ...
  const customGroups = buildRoutineGroups(customRoutines, groupByValue, projectById, agentById, folderById, labels)
    .filter((group) => group.items.length > 0)
    .map((group) => (
      builtInRoutines.length > 0 && groupByValue === "none" && group.key === "__all"
        ? { ...group, label: labels.customRoutines }
        : group
    ));
...
export function Routines() {
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.created", { defaultValue: "Routine created" }),
        body: routine.assigneeAgentId
          ? t("routines.createdWithAgent", { defaultValue: "Add the first trigger to turn it into a live workflow." })
          : t("routines.createdDraft", { defaultValue: "Draft saved. Add a default agent before enabling automation." }),
        tone: "success",
      });
    ...
    onSuccess: async (folder) => {
      ...
      if (moveAfterCreateIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            title: t("routines.folderCreatedMoveFailed", { defaultValue: "Folder created, move failed" }),
            body: moveError instanceof Error ? moveError.message : t("routines.moveSelectedFailed", { defaultValue: "Paperclip could not move the selected routines." }),
            tone: "error",
          });
          return;
        }
      ...
      pushToast({ title: t("routines.folderCreated", { defaultValue: "Folder created" }), body: folder.name, tone: "success" });
    },
    onError: (mutationError) => {
      pushToast({
        title: t("routines.folderSaveFailed", { defaultValue: "Failed to save folder" }),
        body: mutationError instanceof Error ? mutationError.message : t("routines.folderSaveFailedBody", { defaultValue: "Paperclip could not save the folder." }),
        tone: "error",
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.folderUpdateFailed", { defaultValue: "Folder save failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("routines.folderUpdateFailedBody", { defaultValue: "Paperclip could not update the folder." }),
        tone: "error",
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onSuccess: async (_, folderId) => {
      ...
      pushToast({ title: t("routines.folderDeleted", { defaultValue: "Folder deleted" }), body: t("routines.itemsMovedUnfiled", { defaultValue: "Items moved to Unfiled." }), tone: "success" });
    },
    onError: (mutationError) => {
      pushToast({
        title: t("routines.folderDeleteFailed", { defaultValue: "Folder delete failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("routines.folderDeleteFailedBody", { defaultValue: "Paperclip could not delete the folder." }),
        tone: "error",
      });
    },
  });
  const moveRoutineToFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.moveFailed", { defaultValue: "Move failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("routines.moveFailedBody", { defaultValue: "Paperclip could not move the routine." }),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.updateFailedBody", { defaultValue: "Paperclip could not update the routine." }),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.runFailedBody", { defaultValue: "Paperclip could not start the routine run." }),
        tone: "error",
      });
    },
  });
  ...
  const routineSections = useMemo(
    () => buildRoutineSections(sortedRoutines, routineViewState.groupBy, projectById, agentById, folderById, {
      unfiled: t("routines.unfiled", { defaultValue: "Unfiled" }),
      unknownFolder: t("routines.unknownFolder", { defaultValue: "Unknown folder" }),
      noProject: t("routines.noProject", { defaultValue: "No project" }),
      unknownProject: t("routines.unknownProject", { defaultValue: "Unknown project" }),
      unassigned: t("routines.unassigned", { defaultValue: "Unassigned" }),
      unknownAgent: t("routines.unknownAgent", { defaultValue: "Unknown agent" }),
      customRoutines: t("routines.customRoutines", { defaultValue: "Custom routines" }),
      builtInRoutines: t("routines.builtInRoutines", { defaultValue: "Built-in routines" }),
    }),
  ...
  async function moveSelectedRoutines(folderId: string | null) {
    ...
    try {
      ...
      pushToast({ title: t("routines.routinesMoved", { defaultValue: "Routines moved" }), body: t("routines.routinesFiled", { defaultValue: "{{count}} routine{{plural}} filed.", count: ids.length, plural: ids.length === 1 ? "" : "s" }), tone: "success" });
    } catch (moveError) {
      pushToast({
        title: t("routines.moveRoutinesFailed", { defaultValue: "Failed to move routines" }),
        body: moveError instanceof Error ? moveError.message : t("routines.moveSelectedFailed", { defaultValue: "Paperclip could not move the selected routines." }),
        tone: "error",
      });
    }
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routines.defaultAgentRequiredShort", { defaultValue: "Default agent required" }),
        body: t("routines.setDefaultAgentBeforeEnable", { defaultValue: "Set a default agent before enabling routine automation." }),
        tone: "warn",
      });
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            {t("routines.description", { defaultValue: "Recurring work definitions that materialize into auditable execution tasks." })}
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routines.title", { defaultValue: "Routines" }) },
        ...
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {t("routines.routineCount", { defaultValue: "{{count}} routine{{plural}}", count: visibleRoutines.length, plural: visibleRoutines.length === 1 ? "" : "s" })}
            </p>
            <div className="flex items-center gap-1">
              <Popover>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", t("routines.updated", { defaultValue: "Updated" })],
                      ["created", t("routines.createdLabel", { defaultValue: "Created" })],
                      ["lastRun", t("routines.lastRun", { defaultValue: "Last run" })],
                      ["title", t("routines.titleLabel", { defaultValue: "Title" })],
                      ...
                      >
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc" ? t("routines.asc", { defaultValue: "Asc" }) : t("routines.desc", { defaultValue: "Desc" })}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["folder", t("routines.folder", { defaultValue: "Folder" })],
                      ["project", t("routines.project", { defaultValue: "Project" })],
                      ["assignee", t("routines.agent", { defaultValue: "Agent" })],
                      ["none", t("routines.none", { defaultValue: "None" })],
              ...
              {routineViewState.groupBy === "folder" && !hasRoutineFolders ? (
                <Button variant="outline" size="sm" onClick={() => openCreateFolder()}>
                  ...
                  {t("routines.newFolder", { defaultValue: "New folder" })}
                </Button>
              ) : null}
              {showFolderRail ? (
                <Button variant="ghost" size="sm" className="text-xs" onClick={() => setSelectMode((current) => !current)}>
                  {selectMode ? t("common.done", { defaultValue: "Done" }) : t("routines.select", { defaultValue: "Select" })}
                </Button>
              ) : null}
            </div>
          </div>
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {t("routines.newRoutineDescription", { defaultValue: "Define the recurring work first. Default project and agent are optional for draft routines." })}
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("routines.routineTitlePlaceholder", { defaultValue: "Routine title" })}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  ...
                  />
                  <span>{t("routines.filedIn", { defaultValue: "filed in" })}</span>
                  <Select
              ...
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t("routines.advancedDeliveryDescription", { defaultValue: "Keep policy controls secondary to the work definition." })}</p>
                  </div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {concurrencyPolicies.map((value) => (
                            <SelectItem key={value} value={value}>{t(`routines.concurrencyOption.${value}.title`, { defaultValue: value.replaceAll("_", " ") })}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      <p className="text-xs text-muted-foreground">{t(`routines.concurrencyOption.${draft.concurrencyPolicy}.description`, { defaultValue: concurrencyPolicyDescriptions[draft.concurrencyPolicy] })}</p>
                    </div>
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          {catchUpPolicies.map((value) => (
                            <SelectItem key={value} value={value}>{t(`routines.catchUpOption.${value}.title`, { defaultValue: value.replaceAll("_", " ") })}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      <p className="text-xs text-muted-foreground">{t(`routines.catchUpOption.${draft.catchUpPolicy}.description`, { defaultValue: catchUpPolicyDescriptions[draft.catchUpPolicy] })}</p>
                    </div>
                  </div>
                </CollapsibleContent>
              </Collapsible>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routines.afterCreationDescription", { defaultValue: "After creation, Paperclip takes you straight to trigger setup. Draft routines stay paused until you add a default agent." })}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("routines.creating", { defaultValue: "Creating..." }) : t("routines.createRoutine", { defaultValue: "Create routine" })}
              </Button>
        ...
        <div className={cn(showFolderRail && "flex gap-4")}>
          {showFolderRail ? (
            <FolderRail
              ...
              selection={folderSelection}
                allLabel={t("routines.allRoutines", { defaultValue: "All routines" })}
                itemLabelPlural={t("routines.routinesLower", { defaultValue: "routines" })}
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2">
              {folderSelection === "all" ? <FolderIconHeader label={t("routines.allRoutines", { defaultValue: "All routines" })} count={sortedRoutines.length} /> : (
                <div className="flex min-w-0 items-center gap-2 text-sm">
                  ...
                  <span className="truncate font-medium">{folderSelection === "unfiled" ? t("routines.unfiled", { defaultValue: "Unfiled" }) : activeFolder?.name ?? t("routines.folder", { defaultValue: "Folder" })}</span>
                  <span className="text-muted-foreground">{t("routines.routineCount", { defaultValue: "{{count}} routine{{plural}}", count: sortedRoutines.length, plural: sortedRoutines.length === 1 ? "" : "s" })}</span>
                </div>
              )}
            </div>
          ) : null}
          ...
          {visibleRoutines.length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={t("routines.noActiveRoutines", { defaultValue: "No active routines. Use Create routine to define the first recurring workflow." })}
              />
            </div>
            ...
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={folderSelection === "all" ? t("routines.noRoutinesMatchView", { defaultValue: "No routines match this view." }) : t("routines.folderEmpty", { defaultValue: "This folder is empty." })}
              />
              {folderSelection !== "all" ? (
                <div className="mt-3 flex justify-center">
                  <Button size="sm" onClick={openCreateRoutine}>
                    <Plus className="mr-2 h-3.5 w-3.5" />
                    {t("routines.newRoutineInFolder", { defaultValue: "New routine in this folder" })}
                  </Button>
                </div>
              ) : null}
            </div>
                          ...
                          extraMenuItems={
                            <MoveToMenu
                              ...
                              onMove={(folderId) => {
                                ...
                                pushToast({
                                  title: t("routines.routineMoved", { defaultValue: "Routine moved" }),
                                  body: folderId
                                    ? t("routines.routineMovedToFolder", { defaultValue: "Moved \"{{title}}\" to {{folder}}.", title: routine.title, folder: routineFolders?.folders.find((folder) => folder.id === folderId)?.name ?? t("routines.folder", { defaultValue: "folder" }) })
                                    : t("routines.routineMovedToUnfiled", { defaultValue: "Moved \"{{title}}\" to Unfiled.", title: routine.title }),
                                  tone: "success",
                                  action: {
      ...
      <MobileFolderSheet
        ...
        selection={folderSelection}
        allLabel={t("routines.allRoutines", { defaultValue: "All routines" })}
        itemLabelPlural={t("routines.title", { defaultValue: "Routines" })}
        onSelect={setFolderSelection}
        onCreate={() => openCreateFolder()}
      />
...
function FolderIconHeader({ label, count }: { label: string; count: number }) {
  ...
  return (
    <div className="flex min-w-0 items-center gap-2 text-sm">
      ...
      <span className="text-muted-foreground">{t("routines.routineCount", { defaultValue: "{{count}} routine{{plural}}", count, plural: count === 1 ? "" : "s" })}</span>
    </div>
  );
}
