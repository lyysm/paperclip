...
import type { StatusTranslate } from "@/lib/status-labels";
...
function scopeLabel(translate: StatusTranslate, scope: CompanySearchScope | SubGroupKey): string {
  switch (scope) {
    case "all":
      return translate("searchPage.all", { defaultValue: "All" });
    case "issues":
      return translate("nav.tasks", { defaultValue: "Tasks" });
    case "comments":
      return translate("searchPage.comments", { defaultValue: "Comments" });
    case "documents":
      return translate("searchPage.documents", { defaultValue: "Documents" });
    case "artifacts":
      return translate("nav.artifacts", { defaultValue: "Artifacts" });
    case "agents":
      return translate("nav.agents", { defaultValue: "Agents" });
    case "projects":
      return translate("nav.projects", { defaultValue: "Projects" });
  }
}
...
export function Search() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("nav.search", { defaultValue: "Search" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-page="search">
      <div className="border-b border-border px-4 py-3 sm:px-6">
        <h1 className="sr-only">{t("nav.search", { defaultValue: "Search" })}</h1>
        <div className="relative">
          ...
          <Input
            ...
            placeholder={t("searchPage.placeholder", {
              defaultValue: "Search tasks, comments, documents, artifacts, agents, projects…",
            })}
            aria-label={t("searchPage.queryAria", { defaultValue: "Search query" })}
          ...
          {draftQuery.length > 0 ? (
            <button
              type="button"
              onClick={handleClear}
              aria-label={t("searchPage.clearSearch", { defaultValue: "Clear search" })}
        ...
        <div className="mt-2 flex min-h-6 flex-wrap items-center gap-1.5 text-(length:--text-micro) text-muted-foreground">
          ...
          {operatorSuggestions.length > 0 ? (
            <div className="flex flex-wrap items-center gap-1.5" data-testid="search-operator-suggestions">
              {operatorSuggestions.map((suggestion) => (
                <button
                  ...
                  type="button"
                  aria-label={t("searchPage.insertOperator", { defaultValue: "Insert operator {{token}}", token: suggestion.token })}
            ...
            <span className="truncate">
              {t("searchPage.tryPrefix", { defaultValue: "Try" })}{" "}
              ...
              {t("searchPage.tryOr", { defaultValue: "or" })}{" "}
  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{t("searchPage.initialTitle", { defaultValue: "Type to search company memory." })}</h2>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("searchPage.tipIdentifier", { defaultValue: "Identifier lookup:" })}</span>{" "}
            {t("searchPage.tipIdentifierPre", { defaultValue: "type" })}{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">PAP-123</code>{" "}
            {t("searchPage.tipIdentifierPost", { defaultValue: "to jump straight to a task." })}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("searchPage.tipQuoted", { defaultValue: "Quoted phrases:" })}{" "}
            </span>
            {t("searchPage.tipQuotedBody", { defaultValue: "wrap a phrase in quotes to match the exact sequence." })}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("searchPage.tipCmdK", { defaultValue: "⌘K:" })}</span>{" "}
            {t("searchPage.tipCmdKBody", { defaultValue: "reopens the command palette pre-seeded with your current query." })}
          </li>
        </ul>
      </div>
    );
  ...
  if (hasError) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("searchPage.errorTitle", { defaultValue: "Couldn’t run that search" })}</div>
        <p className="text-sm text-muted-foreground">
          {status
            ? t("searchPage.serverReturned", { defaultValue: "The server returned {{status}}.", status })
            : t("searchPage.requestFailed", { defaultValue: "The request failed." })}{" "}
          {t("searchPage.errorBody", {
            defaultValue: "Your input and filters are still here, so you can retry or fall back to the Tasks filter.",
          })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          <Button onClick={refetch} variant="default" size="sm">
            {t("searchPage.retry", { defaultValue: "Retry" })}
          </Button>
          <Button onClick={navigateIssuesFallback} variant="outline" size="sm">
            {t("searchPage.openTasksFilter", { defaultValue: "Open Tasks filter view" })}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("searchPage.searchingFor", { defaultValue: "Searching for “{{query}}”…", query: trimmedQuery })}
        </div>
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">
          {t("searchPage.noResultsFor", { defaultValue: "No results for “{{query}}”", query: trimmedQuery })}
        </div>
        <p className="text-sm text-muted-foreground">
          {t("searchPage.noMatchInScope", {
            defaultValue:
              "We couldn’t find a match in {{scope}}. Try widening the scope or rephrasing your query.",
            scope: (scope === "all"
              ? t("searchPage.allScopes", { defaultValue: "All scopes" })
              : scopeLabel(t, scope)).toLowerCase(),
          })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
          ...
          <Button onClick={openNewIssue} size="sm" variant="default">
            ...
            {t("searchPage.createTaskFromQuery", { defaultValue: "Create task from this query" })}
          </Button>
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("searchPage.zeroTipTokens", { defaultValue: "Try fewer tokens or a single distinctive term." })}</li>
          <li>
            {t("searchPage.zeroTipIdentifierPre", { defaultValue: "Use an identifier shortcut like" })}{" "}
            <code className="rounded bg-muted px-1 py-0.5">PAP-123</code>.
          </li>
          <li>{t("searchPage.zeroTipQuoted", { defaultValue: "Wrap multi-word phrases in quotes." })}</li>
        </ul>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        <span>
          {allMatchTotal > totalResults
            ? t("searchPage.resultsOf", { defaultValue: "{{shown}} of {{total}} results", shown: totalResults, total: allMatchTotal })
            : totalResults === 1
              ? t("searchPage.oneResult", { defaultValue: "1 result" })
              : t("searchPage.resultsCount", { defaultValue: "{{count}} results", count: totalResults })}
          {` · ${t("searchPage.sortedBy", { defaultValue: "sorted by {{label}}", label: sortLabel })}`}
          {activeFilterCount > 0
            ? ` · ${t("searchPage.filtersActive", { defaultValue: "{{count}} filters active", count: activeFilterCount })}`
            : ""}
        </span>
        {isFetching ? <span aria-live="polite" className="normal-case tracking-normal">{t("searchPage.updating", { defaultValue: "Updating…" })}</span> : null}
      </div>
      <div className="flex flex-col pb-10">
        {scope === "all" ? (
          subgroups.map((group, groupIndex) => (
            <section
              key={group.key}
              aria-label={scopeLabel(t, group.key)}
              className={cn("flex flex-col", groupIndex > 0 && "mt-6")}
            >
              <IssueGroupHeader
                label={scopeLabel(t, group.key)}
                trailing={
