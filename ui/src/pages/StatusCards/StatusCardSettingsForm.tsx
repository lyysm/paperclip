...
const TRIGGER_ROWS: { key: TriggerKey; labelKey: string; defaultLabel: string; noisy?: boolean }[] = [
  { key: "statusTransitions", labelKey: "triggerStatusTransitions", defaultLabel: "Became blocked / needs review / done / cancelled" },
  { key: "membershipChanges", labelKey: "triggerMembershipChanges", defaultLabel: "New issue matches the query · issue leaves the query" },
  { key: "humanComments", labelKey: "triggerHumanComments", defaultLabel: "Human comments" },
  { key: "assigneeChanges", labelKey: "triggerAssigneeChanges", defaultLabel: "Assignee changes" },
  { key: "anyUpdate", labelKey: "triggerAnyUpdate", defaultLabel: "Any update at all (noisy — includes in-progress churn)", noisy: true },
];
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-2">
        <h3 className="text-sm font-semibold">{t("statusCards.settings.autoUpdatePolicy", { defaultValue: "Auto-update policy" })}</h3>
        <div className="space-y-2">
          <RadioRow
            selected={policy.mode === "manual"}
            title={t("statusCards.settings.manualOnly", { defaultValue: "Manual only — updates when I press refresh" })}
            badge={
              <span className="ml-auto rounded-full bg-muted px-2 py-0.5 text-(length:--text-nano) font-medium uppercase tracking-wide text-muted-foreground">
                {t("statusCards.settings.default", { defaultValue: "Default" })}
              </span>
            }
          ...
          <RadioRow
            ...
            title={t("statusCards.settings.scheduled", { defaultValue: "On a schedule, only if something changed" })}
            onSelect={() => setMode("interval")}
          >
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{t("statusCards.settings.checkEvery", { defaultValue: "Check every" })}</span>
              <Select
                value={String(policy.intervalMinutes ?? 15)}
                onValueChange={(next) => setPolicy({ intervalMinutes: Number(next) })}
              >
                <SelectTrigger size="sm" className="w-28" aria-label={t("statusCards.settings.checkInterval", { defaultValue: "Check interval" })}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {INTERVAL_OPTIONS.map((minutes) => (
                    <SelectItem key={minutes} value={String(minutes)}>
                      {t("statusCards.settings.minutes", { defaultValue: "{{minutes}} min", minutes })}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <RadioRow
            ...
            title={t("statusCards.settings.reactive", { defaultValue: "As soon as something changes (debounced)" })}
            onSelect={() => setMode("reactive")}
          >
            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              <span>{t("statusCards.settings.wait", { defaultValue: "Wait" })}</span>
              <Select
                value={String(policy.debounceSeconds ?? 60)}
                onValueChange={(next) => setPolicy({ debounceSeconds: Number(next) })}
              >
                <SelectTrigger size="sm" className="w-24" aria-label={t("statusCards.settings.debounce", { defaultValue: "Debounce" })}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {DEBOUNCE_OPTIONS.map((seconds) => (
                    <SelectItem key={seconds} value={String(seconds)}>
                      {t("statusCards.settings.seconds", { defaultValue: "{{seconds}}s", seconds })}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <span className="text-xs">{t("statusCards.settings.afterLastChangeMax", { defaultValue: "after the last change · max" })}</span>
              <Input
                type="number"
                min={1}
                max={60}
                ...
                aria-label={t("statusCards.settings.maxUpdatesPerHour", { defaultValue: "Max updates per hour" })}
              />
              <span className="text-xs">{t("statusCards.settings.updatesPerHour", { defaultValue: "updates/hour" })}</span>
            </div>
          </RadioRow>
        </div>
      </section>
      ...
      */}
      {autoUpdating ? (
        <Collapsible className="rounded-md border border-border">
          <CollapsibleTrigger className="group flex w-full items-center justify-between gap-2 px-3 py-2.5 text-sm font-semibold">
            {t("statusCards.settings.advanced", { defaultValue: "Advanced" })}
            <ChevronDown className="h-4 w-4 text-muted-foreground transition-transform group-data-[state=open]:rotate-180" />
          </CollapsibleTrigger>
          <CollapsibleContent className="space-y-6 border-t border-border px-3 py-3">
            <section className="space-y-2">
              <h3 className="text-sm font-semibold">{t("statusCards.settings.countAsChange", { defaultValue: "Count as a change" })}</h3>
              <div className="space-y-2">
                {TRIGGER_ROWS.map((row) => (
                  <label key={row.key} className="flex items-start gap-2.5 text-sm">
                    <Checkbox
                      ...
                      className="mt-0.5"
                      aria-label={t(`statusCards.settings.${row.labelKey}`, { defaultValue: row.defaultLabel })}
                    />
                    <span className={cn(row.noisy && "text-muted-foreground")}>
                      {t(`statusCards.settings.${row.labelKey}`, { defaultValue: row.defaultLabel })}
                    </span>
                  </label>
                ))}
              </div>
            ...
            <section className="space-y-3">
              <h3 className="text-sm font-semibold">{t("statusCards.settings.guardrails", { defaultValue: "Guardrails" })}</h3>
              <label className="flex items-start gap-2.5 text-sm">
                <Checkbox checked={Boolean(activeHours)} onCheckedChange={(checked) => setActiveHoursEnabled(Boolean(checked))} className="mt-0.5" aria-label={t("statusCards.settings.limitActiveHours", { defaultValue: "Limit to active hours" })} />
                <span>{t("statusCards.settings.onlyActiveHours", { defaultValue: "Only auto-update during active hours" })}</span>
              </label>
              {activeHours ? (
                <div className="flex flex-wrap items-center gap-2 pl-6 text-sm">
                  <Input
                    ...
                    className="h-8 w-32"
                    aria-label={t("statusCards.settings.activeHoursStart", { defaultValue: "Active hours start" })}
                  />
                  ...
                  <Input
                    ...
                    className="h-8 w-32"
                    aria-label={t("statusCards.settings.activeHoursEnd", { defaultValue: "Active hours end" })}
                  />
                  <Input
                    value={activeHours.timezone}
                    onChange={(event) => setPolicy({ activeHours: { ...activeHours, timezone: event.target.value } })}
                    className="h-8 w-40"
                    placeholder={t("statusCards.settings.timezone", { defaultValue: "Timezone" })}
                    aria-label={t("statusCards.settings.activeHoursTimezone", { defaultValue: "Active hours timezone" })}
                  />
                </div>
              ) : null}
              <div className="flex flex-wrap items-center gap-2 text-sm">
                <span className="w-32 shrink-0">{t("statusCards.settings.dailyTokenCap", { defaultValue: "Daily token cap" })}</span>
                <Input
                  type="number"
                  min={0}
                  step={1000}
                  ...
                  className="h-8 w-36"
                  placeholder={t("statusCards.settings.noCap", { defaultValue: "no cap" })}
                  aria-label={t("statusCards.settings.dailyTokenCap", { defaultValue: "Daily token cap" })}
                />
              </div>
            </section>
          </CollapsibleContent>
      ...
      <div className="flex items-center gap-2 text-sm">
        <span className="font-semibold">{t("statusCards.settings.estimatedCost", { defaultValue: "Estimated cost" })}</span>
        <span className="text-muted-foreground">=</span>
        <Tooltip>
          ...
          <TooltipContent className="max-w-(--sz-18rem) text-left">
            ...
            <p className="mt-1 opacity-80">{t("statusCards.settings.estimateDisclaimer", { defaultValue: "Rough estimate from typical update sizes; actual cost is tracked per update." })}</p>
          </TooltipContent>
        </Tooltip>
      </div>
    </div>
