...
function rangeError(range: DateRangeState): string | null {
  if (!range.fromDate || !range.toDate) return t("timeline.chooseStartEnd", { defaultValue: "Choose a start and end date." });
  if (!rangeWindow(range)) return t("timeline.startBeforeEnd", { defaultValue: "Start date must be before end date." });
  return null;
}
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-border px-3.5 py-2 text-xs text-muted-foreground">
      <span className="flex items-center gap-1.5">
        ...
        {t("timeline.legendDelegated", { defaultValue: "Delegated" })}
      </span>
      <span className="flex items-center gap-1.5">
        <span className="h-2.5 w-4 rounded-sm" style={{ backgroundColor: TIMELINE_COLORS.automation }} />
        {t("timeline.legendAutomation", { defaultValue: "Automation" })}
      </span>
      <span className="flex items-center gap-1.5">
        <span
        ...
        />
        {t("timeline.legendCancelled", { defaultValue: "Cancelled" })}
      </span>
      <span className="flex items-center gap-1.5">
        <span className="h-3.5 w-0.5" style={{ backgroundColor: TIMELINE_COLORS.now }} />
        {t("timeline.legendNow", { defaultValue: "Now" })}
      </span>
    </div>
  );
}
...
function TimelineSummaryStats({
  ...
  const stats: { label: string; value: string; icon: LucideIcon }[] = [
    { label: t("timeline.statRuns", { defaultValue: "Runs" }), value: formatInteger(summary.runs), icon: GanttChartSquare },
    { label: t("nav.agents", { defaultValue: "Agents" }), value: formatInteger(summary.agents), icon: Bot },
    { label: t("timeline.statRunTime", { defaultValue: "Run time" }), value: formatDuration(0, summary.activeMs), icon: Clock3 },
    {
      label: t("timeline.statTokens", { defaultValue: "Tokens used" }),
      value: summary.totalTokens > 0 ? formatCompactInteger(summary.totalTokens) : t("timeline.notTracked", { defaultValue: "Not tracked" }),
      icon: Coins,
    },
  ];
...
export function Timeline() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("nav.timeline", { defaultValue: "Timeline" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  const rangeControls = (
    <label className="flex min-w-0 flex-wrap items-center gap-2 text-xs text-muted-foreground">
      ...
      <Segmented
        ...
        options={[
          { value: "today", label: t("timeline.today", { defaultValue: "Today" }) },
          { value: "7d", label: t("timeline.last7Days", { defaultValue: "7 days" }) },
          { value: "30d", label: t("timeline.last30Days", { defaultValue: "30 days" }) },
        ]}
      />
      <Input
        type="date"
  ...
  const toolbar = (
    <div className="flex flex-wrap items-start gap-3">
      ...
      <div className="ml-auto flex items-center gap-1 pt-3" aria-label={t("timeline.zoomControls", { defaultValue: "Timeline zoom controls" })}>
        <Button
          type="button"
          variant="outline"
          size="icon-xs"
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <EmptyState
          icon={GanttChartSquare}
          message={t("timeline.loadError", { defaultValue: "Couldn't load the timeline. The aggregation endpoint may be unavailable." })}
        />
      )}
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                {t("timeline.runsCount", { defaultValue: "{{count}} runs", count: data.spans.length })}{" · "}
                {t("timeline.windowFromTo", {
                  defaultValue: "{{from}} to {{to}}",
                  from: new Date(data.window.from).toLocaleString(),
                  to: new Date(data.window.to).toLocaleString(),
                })}
                {data.window.capped ? t("timeline.windowCapped", { defaultValue: " · window capped" }) : ""}
              </p>
              {rangeControls}
            </div>
          </div>
