...
export function AgentSkillRow({
  ...
  const trailing = readOnly ? (
    <Lock
      ...
      aria-label={t("agentSkills.readOnly", { defaultValue: "Read-only" })}
    />
  ) : (
    (() => {
      const toggle = (
        <ToggleSwitch
          ...
          aria-label={t(checked ? "agentSkills.disableSkill" : "agentSkills.enableSkill", {
            defaultValue: checked ? "Disable {{name}}" : "Enable {{name}}",
            name: data.name,
          })}
        />
      );
