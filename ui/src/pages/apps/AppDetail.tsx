...
export function AppDetail() {
  ...
  const appName = connection
    ...
    : t("appsPage.detail.main.app", { defaultValue: "App" });

  useEffect(() => {
    if (!activeTab) return;
    setBreadcrumbs([
      {
        label: selectedCompany?.name ?? t("appsPage.detail.main.company", { defaultValue: "Company" }),
        href: "/dashboard",
      },
      { label: t("appsPage.detail.main.apps", { defaultValue: "Apps" }), href: "/apps" },
      { label: appName, href: appTabHref(connectionId, "setup") },
      { label: appTabLabel(activeTab) },
    ]);
    return () => setBreadcrumbs([]);
  }, [setBreadcrumbs, t, selectedCompany?.name, appName, connectionId, activeTab]);
  ...
  const persist = useMutation({
    ...
    onError: (error) =>
      pushToast({
        title: t("appsPage.detail.main.couldntSaveThat", { defaultValue: "Couldn't save that" }),
        body: error instanceof Error
          ? error.message
          : t("appsPage.detail.main.pleaseTryAgain", { defaultValue: "Please try again." }),
        tone: "error",
      }),
  ...
  const persistInstall = useMutation({
    ...
    onError: (error) =>
      pushToast({
        title: t("appsPage.detail.main.couldntSaveInstalls", { defaultValue: "Couldn't save installs" }),
        body: error instanceof Error
          ? error.message
          : t("appsPage.detail.main.pleaseTryAgain", { defaultValue: "Please try again." }),
        tone: "error",
  ...
  const rename = useMutation({
    ...
    onError: (error) =>
      pushToast({
        title: t("appsPage.detail.main.couldntRenameApp", { defaultValue: "Couldn't rename the app" }),
        body: error instanceof Error
          ? error.message
          : t("appsPage.detail.main.pleaseTryAgain", { defaultValue: "Please try again." }),
        tone: "error",
  ...
  const startOAuth = useMutation({
    ...
    onError: (error) =>
      pushToast({
        title: t("appsPage.detail.main.couldntStartSignIn", { defaultValue: "Couldn't start sign-in" }),
        body: error instanceof Error
          ? error.message
          : t("appsPage.detail.main.pleaseTryAgain", { defaultValue: "Please try again." }),
        tone: "error",
  ...
  const removeApp = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.apps.attention(selectedCompanyId!) });
      pushToast({
        title: t("appsPage.detail.main.appRemoved", { defaultValue: "App removed" }),
        body: t("appsPage.detail.main.appRemovedBody", {
          defaultValue: "{{name}} no longer has access. You can connect it again any time.",
          name: appName,
        }),
        tone: "success",
      });
    ...
    onError: (error) =>
      pushToast({
        title: t("appsPage.detail.main.couldntRemoveApp", { defaultValue: "Couldn't remove the app" }),
        body: error instanceof Error
          ? error.message
          : t("appsPage.detail.main.pleaseTryAgain", { defaultValue: "Please try again." }),
        tone: "error",
  ...
  const toggleEnabled = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      pushToast({
        title: updated.enabled
          ? t("appsPage.detail.main.appResumed", { defaultValue: "App resumed" })
          : t("appsPage.detail.main.appPaused", { defaultValue: "App paused" }),
        body: updated.enabled
          ? t("appsPage.detail.main.appResumedBody", {
              defaultValue: "{{name}} is available to agents again.",
              name: humanizeConnectionDisplayName(updated),
            })
          : t("appsPage.detail.main.appPausedBody", {
              defaultValue: "{{name}} is paused for agents.",
              name: humanizeConnectionDisplayName(updated),
            }),
        tone: "success",
      });
    ...
    onError: (error) =>
      pushToast({
        title: t("appsPage.detail.main.couldntUpdateApp", { defaultValue: "Couldn't update the app" }),
        body: error instanceof Error
          ? error.message
          : t("appsPage.detail.main.pleaseTryAgain", { defaultValue: "Please try again." }),
        tone: "error",
  ...
  const refreshTools = useMutation({
    ...
    onError: (error) =>
      pushToast({
        title: t("appsPage.detail.main.couldntRefreshActions", { defaultValue: "Couldn't refresh actions" }),
        body: error instanceof Error
          ? error.message
          : t("appsPage.detail.main.pleaseTryAgain", { defaultValue: "Please try again." }),
        tone: "error",
  ...
  if (!selectedCompanyId) {
    return (
      <div className="p-6 text-sm text-muted-foreground">
        {t("appsPage.detail.main.selectCompany", { defaultValue: "Select a company to manage apps." })}
      </div>
    );
  }
    ...
    return (
      <div className="max-w-3xl p-6">
        <p className="text-sm text-muted-foreground">
          {t("appsPage.detail.main.appNotFound", { defaultValue: "We couldn't find that app." })}
        </p>
        <Button className="mt-4" variant="outline" onClick={() => navigate("/apps/connections")}>
          {t("appsPage.detail.main.backToApps", { defaultValue: "Back to apps" })}
        </Button>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          {renaming ? (
            ...
            >
              <Input
                aria-label={t("appsPage.detail.main.appNameLabel", { defaultValue: "App name" })}
                value={nameDraft}
              ...
              <Button type="submit" size="sm" disabled={renamePending || !nameDraft.trim()}>
                {renamePending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : t("appsPage.detail.main.save", { defaultValue: "Save" })}
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={onRenameCancel} disabled={renamePending}>
                {t("appsPage.detail.main.cancel", { defaultValue: "Cancel" })}
              </Button>
            </form>
          ) : (
            <div className="flex items-center gap-1.5">
              ...
              <Button
                ...
                aria-label={t("appsPage.detail.main.renameAppLabel", { defaultValue: "Rename app" })}
          ...
          <div className="mt-1 flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {t("appsPage.detail.main.actionsAvailable", {
                defaultValue: "{{count}} actions available",
                count: actionCount,
              })}
            </span>
          </div>
...
function statusFor(connection: ToolConnection): StatusInfo {
  if (connection.enabled === false || connection.status === "disabled") {
    return { label: t("appsPage.detail.main.statusPaused", { defaultValue: "Paused" }), tone: "paused" };
  }
  if (isAttentionHealthStatus(connection.healthStatus)) {
    return {
      label: t("appsPage.detail.main.statusNeedsAttention", { defaultValue: "Needs attention" }),
      tone: "attention",
    };
  }
  return { label: t("appsPage.detail.main.statusConnected", { defaultValue: "Connected" }), tone: "connected" };
}
