...
export function AppNotConnected() {
  ...
  const appName = application?.name ?? t("appsPage.notConnected.appFallback", { defaultValue: "App" });
  useEffect(() => {
    if (!activeTab) return;
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("appsPage.notConnected.companyFallback", { defaultValue: "Company" }), href: "/dashboard" },
      { label: t("appsPage.notConnected.appsNavLabel", { defaultValue: "Apps" }), href: "/apps" },
  ...
  const remove = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({
        title: t("appsPage.notConnected.removedTitle", { defaultValue: "App removed" }),
        body: t("appsPage.notConnected.removedBody", {
    ...
    onError: (error) => {
      pushToast({
        title: t("appsPage.notConnected.removeFailedTitle", { defaultValue: "Couldn’t remove the app" }),
        body: error instanceof Error ? error.message : t("appsPage.notConnected.tryAgain", { defaultValue: "Please try again." }),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <div className="p-6 text-sm text-muted-foreground">{t("appsPage.notConnected.selectCompany", { defaultValue: "Select a company to manage apps." })}</div>;
  }
  ...
  if (!application) {
    return (
      <div className="max-w-3xl space-y-3 p-6 text-sm text-muted-foreground">
        <p>{t("appsPage.notConnected.appMissing", { defaultValue: "This app doesn’t exist anymore." })}</p>
        <Button variant="outline" size="sm" onClick={() => navigate("/apps/connections")}>{t("appsPage.notConnected.backToApps", { defaultValue: "Back to apps" })}</Button>
      </div>
    );
  }
    ...
    <div className="max-w-3xl space-y-6 pb-12">
      ...
      {activeTab === "review" && (
        previousConnection ? (
          ...
          <EmptyTab
            title={t("appsPage.notConnected.reviewEmptyTitle", { defaultValue: "Nothing is waiting for your OK right now." })}
            body={t("appsPage.notConnected.reviewEmptyBody", { defaultValue: "Review requests will appear here after this app is connected." })}
          />
        )
      )}
      ...
      {activeTab === "test" && (
          <EmptyTab
            title={t("appsPage.notConnected.testEmptyTitle", { defaultValue: "Reconnect to test this app." })}
            body={t("appsPage.notConnected.testEmptyBody", { defaultValue: "Testing becomes available after this app is connected again." })}
          />
      )}
      {activeTab === "activity" && (
        previousConnection ? (
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center rounded-full border border-border bg-background px-2 py-0.5 text-xs font-medium text-muted-foreground">
            {connectedCount > 0
              ? t("appsPage.notConnected.connectedCount", { defaultValue: "{{count}} connected", count: connectedCount })
              : t("appsPage.notConnected.notConnectedStatus", { defaultValue: "Not connected" })}
          </span>
        </div>
        {description && (
      ...
      <div className="space-y-6">
        <section className="space-y-3">
          <div>
            <h2 className="text-sm font-bold text-foreground">
              {t("appsPage.notConnected.alreadyConnectedTo", {
                defaultValue: "Already connected to {{appName}}",
                appName: applicationName,
              })}
            </h2>
            <p className="mt-0.5 text-sm text-muted-foreground">
              {t("appsPage.notConnected.alreadyConnectedHint", { defaultValue: "Open a connection to edit it, or add another account." })}
            </p>
          </div>
            ...
            {activeConnections.map((connection) => {
              const secondary = connectionDisplaySecondaryHint(connection) ??
                (connection.lastUsedAt
                  ? t("appsPage.notConnected.lastUsedAgo", {
                      defaultValue: "Last used {{time}}",
                      time: timeAgo(connection.lastUsedAt),
                    })
                  : t("appsPage.notConnected.notUsedYet", { defaultValue: "Not used yet" }));
              const status = connection.enabled === false || connection.status === "disabled"
                ? t("appsPage.notConnected.statusPaused", { defaultValue: "Paused" })
                : isToolConnectionAttentionHealth(connection.healthStatus)
                  ? t("appsPage.notConnected.statusNeedsAttention", { defaultValue: "Needs attention" })
                  : t("appsPage.notConnected.statusConnected", { defaultValue: "Connected" });
              return (
                <button
                  key={connection.id}
                  type="button"
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-primary">{t("appsPage.notConnected.editAction", { defaultValue: "Edit →" })}</span>
                </button>
              );
            })}
          </div>
        ...
        <section className="rounded-xl border border-border bg-card px-5 py-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-sm font-bold text-foreground">{t("appsPage.notConnected.connectAnother", { defaultValue: "Connect another" })}</h2>
              <p className="mt-0.5 text-sm text-muted-foreground">
                {t("appsPage.notConnected.connectAnotherBody", {
                  defaultValue: "Add another {{appName}} account without changing the connections above.",
                  appName: applicationName,
                })}
              </p>
            </div>
            <Button onClick={onConnect}>{t("appsPage.notConnected.connectAnother", { defaultValue: "Connect another" })}</Button>
          </div>
        </section>
      </div>
    );
      ...
      <section className="rounded-xl border border-border bg-card px-5 py-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-bold text-foreground">
              {previousConnection
                ? t("appsPage.notConnected.reconnectThisApp", { defaultValue: "Reconnect this app" })
                : t("appsPage.notConnected.connectThisApp", { defaultValue: "Connect this app" })}
            </h2>
            <p className="mt-0.5 text-sm text-muted-foreground">
              {previousConnection
                ? t("appsPage.notConnected.reconnectKeptBody", { defaultValue: "We kept the previous setup. Add a working key to bring it back online." })
                : t("appsPage.notConnected.connectRequiredBody", { defaultValue: "Agents can't use it until it's connected." })}
            </p>
          </div>
          <Button onClick={onConnect}>
            {previousConnection
              ? t("appsPage.notConnected.reconnectButton", { defaultValue: "Reconnect" })
              : t("appsPage.notConnected.connectButton", { defaultValue: "Connect" })}
          </Button>
        </div>
      </section>
...
function PreviousSetup({
  ...
  return (
    <section className="rounded-xl border border-border bg-card px-5 py-4">
      <h2 className="text-sm font-bold text-foreground">{t("appsPage.notConnected.previousSetupTitle", { defaultValue: "Previous setup" })}</h2>
      {connection.healthMessage && (
        <p className="mt-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
          {t("appsPage.notConnected.lastError", {
            defaultValue: "Last error: {{message}}",
            message: connection.healthMessage,
          })}
        </p>
      )}
      <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-(--gtc-59)">
        <dt className="text-muted-foreground">{t("appsPage.notConnected.addressLabel", { defaultValue: "Address" })}</dt>
        <dd className="break-all font-mono text-foreground">{previousAddress}</dd>
        <dt className="text-muted-foreground">{t("appsPage.notConnected.connectionTypeLabel", { defaultValue: "Connection type" })}</dt>
        <dd className="text-foreground">{connectionTransportLabel(connection.transport)}</dd>
        <dt className="text-muted-foreground">{t("appsPage.notConnected.lastUsedLabel", { defaultValue: "Last used" })}</dt>
        <dd className="text-foreground">
          {connection.lastUsedAt ? timeAgo(connection.lastUsedAt) : t("appsPage.notConnected.never", { defaultValue: "Never" })}
        </dd>
      </dl>
    </section>
  );
...
function PermissionsTab({ previousConnection }: { previousConnection: ToolConnection | null }) {
  ...
  return (
    <section className="rounded-xl border border-border bg-card px-5 py-4">
      <h2 className="text-sm font-bold text-foreground">{t("appsPage.notConnected.permissionsPausedTitle", { defaultValue: "Permissions paused" })}</h2>
      <p className="mt-1 text-sm text-muted-foreground">
        {t("appsPage.notConnected.permissionsPausedBody", { defaultValue: "Reconnect this app to edit who can use it and which actions need a human first." })}
      </p>
      {previousConnection && (
        <p className="mt-3 text-xs text-muted-foreground">
          {t("appsPage.notConnected.permissionsRetainedNote", { defaultValue: "Previous setup is retained for reconnect, but access controls stay read-only until the app is online." })}
        </p>
      )}
    </section>
  );
  ...
  return (
    <div className="space-y-6">
      {previousConnection ? (
        ...
        <EmptyTab
          title={t("appsPage.notConnected.advancedEmptyTitle", { defaultValue: "No previous connection details" })}
          body={t("appsPage.notConnected.advancedEmptyBody", { defaultValue: "Technical details will appear here after this app is connected." })}
        />
      )}
