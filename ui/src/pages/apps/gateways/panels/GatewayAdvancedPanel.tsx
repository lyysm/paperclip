...
export function GatewayAdvancedPanel({
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("appsPage.gateways.advanced.archiveSuccessTitle", { defaultValue: "Gateway archived" }),
        body: t("appsPage.gateways.advanced.archiveSuccessBody", {
    ...
    onError: (error) =>
      pushToast({
        title: t("appsPage.gateways.advanced.archiveErrorTitle", {
  ...
  async function copy(value: string, label: string) {
    try {
      ...
      pushToast({ title: t("appsPage.gateways.advanced.copiedTitle", { defaultValue: "Copied" }), body: label, tone: "success" });
    } catch {
      pushToast({
        title: t("appsPage.gateways.advanced.copyFailedTitle", { defaultValue: "Copy failed" }),
        body: t("appsPage.gateways.advanced.clipboardUnavailableBody", {
    ...
    <div className="space-y-5">
      <section className="space-y-2">
        <h3 className="text-sm font-semibold text-foreground">
          {t("appsPage.gateways.advanced.transport", { defaultValue: "Transport" })}
        </h3>
        <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
          <Row
            label={t("appsPage.gateways.advanced.transport", { defaultValue: "Transport" })}
            value="streamable_http"
          />
          <Row
            label={t("appsPage.gateways.advanced.authentication", { defaultValue: "Authentication" })}
            value="bearer"
          />
          <Row
            label={t("appsPage.gateways.advanced.protocolVersion", { defaultValue: "Protocol version" })}
            value="2025-03-26"
          />
          <Row
            label={t("appsPage.gateways.advanced.publicId", { defaultValue: "Public ID" })}
            value={gateway.gatewayPublicId}
            mono
          />
        </dl>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            onClick={() =>
              void copy(
                endpoint,
                t("appsPage.gateways.advanced.endpointUrlLabel", { defaultValue: "Endpoint URL" }),
              )
            }
          >
            <Copy className="mr-1 h-3.5 w-3.5" />
            {t("appsPage.gateways.advanced.copy", { defaultValue: "Copy" })}
          </Button>
        </div>
      </section>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-foreground">
            {t("appsPage.gateways.advanced.rawConfigHeading", { defaultValue: "Raw configuration" })}
          </h3>
          <Button
            variant="outline"
            size="sm"
            onClick={() =>
              void copy(
                rawConfig,
                t("appsPage.gateways.advanced.gatewayConfigJsonLabel", {
                  defaultValue: "Gateway config JSON",
                }),
              )
            }
          >
            <Copy className="mr-1 h-3.5 w-3.5" />
            {t("appsPage.gateways.advanced.copyJson", { defaultValue: "Copy JSON" })}
          </Button>
        </div>
      ...
      <section className="space-y-2 rounded-lg border border-destructive/40 p-4">
        <h3 className="text-sm font-semibold text-destructive">
          {t("appsPage.gateways.advanced.dangerZoneHeading", { defaultValue: "Danger zone" })}
        </h3>
        <p className="text-sm text-muted-foreground">
          {t("appsPage.gateways.advanced.dangerDescription", {
            defaultValue:
        ...
        {confirming ? (
          <div className="space-y-2">
            <Input
              ...
              aria-label={t("appsPage.gateways.advanced.confirmArchiveAriaLabel", {
            ...
            <div className="flex gap-2">
              ...
              >
                {archiveMutation.isPending
                  ? t("appsPage.gateways.advanced.archiving", { defaultValue: "Archiving…" })
                  : t("appsPage.gateways.advanced.archiveGatewayButton", {
                      defaultValue: "Archive gateway",
                    })}
              </Button>
              <Button variant="ghost" size="sm" onClick={() => { setConfirming(false); setConfirmName(""); }}>
                {t("appsPage.gateways.advanced.cancel", { defaultValue: "Cancel" })}
              </Button>
            </div>
          </div>
        ) : (
          <Button variant="outline" size="sm" className="text-destructive" onClick={() => setConfirming(true)}>
            {t("appsPage.gateways.advanced.archiveGatewayButton", { defaultValue: "Archive gateway" })}
          </Button>
        )}
      </section>
    </div>
