  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="text-sm font-semibold text-foreground">{t("appsPage.browse.connectYourOwnTool", { defaultValue: "Connect your own tool" })}</div>
        <div className="text-xs text-muted-foreground">
          {t("appsPage.browse.byoDescription", { defaultValue: "Paste the URL from a custom or self-hosted MCP server and review its actions before enabling it." })}
        </div>
      </div>
      <span className="shrink-0 text-xs font-semibold text-primary">{t("appsPage.browse.connectArrow", { defaultValue: "Connect →" })}</span>
    </button>
  );
}
...
export function AdvancedToolsLink() {
  ...
  return (
    ...
    >
      ...
      {t("appsPage.browse.developerTools", { defaultValue: "Developer tools (advanced)" })}
    </Link>
  );
}
