...
function statusBadgeLabel(status: RemoteSecretImportCandidate["status"]) {
  switch (status) {
    case "duplicate":
      return t("secretsPage.importVault.statusImported", { defaultValue: "Imported" });
    case "conflict":
      return t("secretsPage.importVault.statusConflict", { defaultValue: "Conflict" });
    case "ready":
    default:
      return t("secretsPage.importVault.statusReady", { defaultValue: "Ready" });
  }
}
    ...
    case "imported":
      return (
        ...
        >
          ...
          {t("secretsPage.importVault.created", { defaultValue: "Created" })}
        </Badge>
      );
    case "skipped":
      return (
        ...
        >
          ...
          {t("secretsPage.importVault.skipped", { defaultValue: "Skipped" })}
        </Badge>
      );
    case "error":
    default:
      return (
        ...
        >
          ...
          {t("secretsPage.importVault.failed", { defaultValue: "Failed" })}
        </Badge>
      );
  }
}
...
function formatRelativeShort(value: string | null | undefined): string {
  ...
  if (seconds < 60) {
    return t("secretsPage.importVault.secondsAgo", { count: seconds, defaultValue: "{{count}}s ago" });
  }
  const minutes = Math.floor(seconds / 60);
  if (minutes < 60) {
    return t("secretsPage.importVault.minutesAgo", { count: minutes, defaultValue: "{{count}}m ago" });
  }
  const hours = Math.floor(minutes / 60);
  if (hours < 48) {
    return t("secretsPage.importVault.hoursAgo", { count: hours, defaultValue: "{{count}}h ago" });
  }
  const days = Math.floor(hours / 24);
  if (days < 30) {
    return t("secretsPage.importVault.daysAgo", { count: days, defaultValue: "{{count}}d ago" });
  }
...
function readableErrorMessage(error: unknown): string {
  if (error instanceof ApiError) {
    return error.message
      || t("secretsPage.requestFailed", { defaultValue: "Request failed: {{status}}", status: error.status });
  }
...
function validateDraftRow(
  ...
  if (!draft.name.trim()) return t("secretsPage.importVault.nameRequired", { defaultValue: "Name is required." });
  if (draft.name.length > 160) {
    return t("secretsPage.importVault.nameTooLong", { defaultValue: "Name must be 160 characters or fewer." });
  }
  if (!draft.key.trim()) return t("secretsPage.importVault.keyRequired", { defaultValue: "Key is required." });
  if (!KEY_PATTERN.test(draft.key)) {
    return t("secretsPage.importVault.keyPattern", {
      defaultValue: "Key may only contain lowercase letters, numbers, dot, underscore, or hyphen.",
    });
  }
  if (draft.key.length > 120) {
    return t("secretsPage.importVault.keyTooLong", { defaultValue: "Key must be 120 characters or fewer." });
  }
  if (draft.description.length > 500) {
    return t("secretsPage.importVault.descriptionTooLong", {
...
export function ImportFromVaultDialog({
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      const vaultName =
        awsVaults.find((vault) => vault.id === vaultId)?.displayName
        ?? t("secretsPage.importVault.aws", { defaultValue: "AWS" });
      if (result.errorCount === draftList.length && result.errorCount > 0) {
        toast.pushToast({
          title: t("secretsPage.importVault.importFailed", { defaultValue: "Import failed" }),
          body: t("secretsPage.importVault.noneImported", {
        ...
        toast.pushToast({
          title:
            result.errorCount > 0
              ? t("secretsPage.importVault.importWithErrors", { defaultValue: "Import completed with errors" })
              : t("secretsPage.importVault.importComplete", { defaultValue: "Import complete" }),
          body: t("secretsPage.importVault.importSummary", {
            defaultValue: "{{imported}} created · {{skipped}} skipped · {{failed}} failed",
  ...
  function handleLoadMore() {
    ...
    secretsApi
      ...
      .catch((error) => {
        toast.pushToast({
          title: t("secretsPage.importVault.loadMoreFailed", { defaultValue: "Could not load more results" }),
          body: readableErrorMessage(error),
          tone: "error",
        });
      })
  ...
  function handleClose(force = false) {
    ...
    if (!force && step !== "result" && selection.size > 0 && !importResult) {
      const ok = window.confirm(
        t("secretsPage.importVault.discardPending", {
          count: selection.size,
          defaultValue:
            selection.size === 1
              ? "Discard {{count}} pending import?"
              : "Discard {{count}} pending imports?",
        }),
      );
      if (!ok) return;
    }
        ...
        <header className="flex items-start justify-between gap-3 border-b border-border/60 px-5 py-4">
          <div className="flex flex-col gap-1">
            <DialogTitle className="text-base font-semibold">
              {t("secretsPage.importVault.title", { defaultValue: "Import from AWS Secrets Manager" })}
            </DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              {t("secretsPage.importVault.description", {
          ...
          <button
            ...
            onClick={() => handleClose()}
            aria-label={t("secretsPage.importVault.close", { defaultValue: "Close import dialog" })}
          >
            <X className="h-4 w-4" />
          </button>
        </header>
        ...
        <footer className="flex items-center justify-between gap-3 border-t border-border/60 bg-muted/20 px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {step === "review" && (
              ...
              >
                {t("secretsPage.importVault.back", { defaultValue: "Back" })}
              </Button>
            )}
            {step === "select" && (
              <Button
              ...
              >
                {t("secretsPage.importVault.continueReview", { defaultValue: "Continue → Review" })}
              </Button>
            )}
            {step === "review" && (
              <Button
              ...
              >
                {importMutation.isPending ? (
                  <>
                    ...
                    {t("secretsPage.importVault.importing", { defaultValue: "Importing…" })}
                  </>
                ) : (
                  t("secretsPage.importVault.importCount", {
...
function Stepper({ step }: { step: Step }) {
  ...
  const steps: { id: Step; label: string }[] = [
    { id: "select", label: t("secretsPage.importVault.stepSelect", { defaultValue: "Select" }) },
    { id: "review", label: t("secretsPage.importVault.stepReview", { defaultValue: "Review" }) },
    { id: "result", label: t("secretsPage.importVault.stepResult", { defaultValue: "Result" }) },
  ];
  ...
  if (noEligibleVaults) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6" data-testid="select-empty-vaults">
        <EmptyState
          icon={Cloud}
          message={t("secretsPage.importVault.noVaults", {
            defaultValue: "No AWS provider vault configured. Add one to import secrets.",
          })}
          action={
            onManageVaults
              ? t("secretsPage.importVault.manageVaults", { defaultValue: "Manage vaults" })
              : undefined
          }
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
        <label className="text-xs uppercase tracking-wide text-muted-foreground">
          {t("secretsPage.importVault.vault", { defaultValue: "Vault" })}
        </label>
        {awsVaults.length === 1 && eligible.length === 1 ? (
          ...
          >
            <SelectTrigger
              size="sm"
              className="text-xs"
              aria-label={t("secretsPage.importVault.selectVault", { defaultValue: "Select AWS vault" })}
            >
              <SelectValue
                placeholder={t("secretsPage.importVault.selectVaultPlaceholder", {
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {blocked && (
                        ...
                        >
                          {t(`secretsPage.vaultStatus.${vault.status}`, { defaultValue: vault.status })}
                        </Badge>
                      )}
                    </span>
                  </SelectItem>
        ...
        <div className="relative ml-auto w-64">
          ...
          <Input
            ...
            placeholder={t("secretsPage.importVault.searchPlaceholder", {
              defaultValue: "Search by name, ARN, tag",
            })}
            className="pl-7 pr-7 text-xs"
            aria-label={t("secretsPage.importVault.searchAria", { defaultValue: "Search remote secrets" })}
        ...
        <Button
          ...
          aria-label={t("secretsPage.importVault.refreshAria", { defaultValue: "Refresh remote secrets" })}
        >
      ...
      {selectedNotVisible > 0 && (
        <div className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-1.5 text-xs text-muted-foreground">
          <span>
            {t("secretsPage.importVault.selectedNotVisible", {
          ...
          >
            {showOnlySelected
              ? t("secretsPage.importVault.showAll", { defaultValue: "Show all" })
              : t("secretsPage.importVault.showSelected", { defaultValue: "Show selected" })}
          </Button>
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-testid="vault-table-scroll">
              ...
              <tr>
                <th className="px-3 py-2 text-left">
                  <Checkbox
                    ...
                    aria-label={t("secretsPage.importVault.selectAllLoaded", {
                ...
                <th className="px-2 py-2 text-left font-medium">
                  {t("secretsPage.importVault.remoteName", { defaultValue: "Remote name" })}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("secretsPage.importVault.reference", { defaultValue: "Reference" })}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("secretsPage.importVault.lastChanged", { defaultValue: "Last changed" })}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("secretsPage.importVault.suggestedName", { defaultValue: "Suggested name" })}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("secretsPage.importVault.state", { defaultValue: "State" })}
                </th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        ...
                        aria-label={t("secretsPage.importVault.selectRemoteName", {
                        ...
                        {candidate.status === "duplicate" &&
                          candidate.conflicts.find((c) => c.type === "exact_reference")?.existingSecretId && (
                            <span className="text-(length:--text-micro) text-muted-foreground">
                              {t("secretsPage.importVault.alreadyImported", { defaultValue: "Already imported" })}
                            </span>
                          )}
                      </div>
        ...
        {hasNextPage && !previewError && (
          <div className="flex items-center justify-between border-t border-border/60 px-5 py-2 text-xs text-muted-foreground">
            <span>
              {t("secretsPage.importVault.loadedCount", {
                defaultValue: "{{count}} loaded",
                count: candidates.length,
              })}
              {selectableInLoaded.length > 0 && (
                <span>
                  {t("secretsPage.importVault.selectableCount", {
            ...
            >
              {pageLoading ? (
                <>
                  <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />{" "}
                  {t("secretsPage.importVault.loading", { defaultValue: "Loading…" })}
                </>
              ) : (
                t("secretsPage.importVault.loadMore", {
    ...
    >
      ...
      <div className="flex-1">
        <div className="font-medium">
          {isPermission
            ? t("secretsPage.importVault.awsDeniedList", { defaultValue: "AWS denied list access" })
            : isThrottling
              ? t("secretsPage.importVault.awsThrottled", { defaultValue: "AWS throttled the listing request" })
              : t("secretsPage.importVault.loadRemoteFailed", { defaultValue: "Could not load remote secrets" })}
        </div>
        <div className="mt-1 text-xs leading-relaxed text-destructive/80">
          {isPermission
            ? t("secretsPage.importVault.awsPermissionDetail", {
                defaultValue:
        ...
        <div className="mt-2 flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={onRetry}>
            ...
            {t("secretsPage.importVault.retry", { defaultValue: "Retry" })}
          </Button>
          {isPermission && (
            <a
            ...
            >
              {t("secretsPage.importVault.iamReference", { defaultValue: "IAM reference" })}{" "}
...
function EmptyCandidates({ query }: { query: string }) {
  ...
  if (query) {
    return (
      <EmptyState
        icon={Search}
        message={t("secretsPage.importVault.noSearchMatches", {
  ...
  return (
    <EmptyState
      icon={Database}
      message={t("secretsPage.importVault.noVisibleSecrets", {
  ...
  if (drafts.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6">
        <EmptyState
          icon={Info}
          message={t("secretsPage.importVault.noSecretsSelected", {
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-3 border-b border-border/60 bg-muted/20 px-5 py-3 text-xs">
        <span className="font-medium">
          {t("secretsPage.importVault.readyCount", {
        ...
        {blocked > 0 && (
          <span className="text-amber-600 dark:text-amber-400">
            {t("secretsPage.importVault.blockedCount", {
            ...
            >
              <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">
                        {t("secretsPage.importVault.paperclipName", { defaultValue: "Paperclip name" })}
                      </span>
                      <Input
                        value={draft.name}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">
                        {t("secretsPage.importVault.descriptionOptional", {
                ...
                <Button
                  ...
                  aria-label={t("secretsPage.importVault.removeRemote", {
...
function ResultStep({ result, draftList }: ResultStepProps) {
  ...
  const heading =
    result.errorCount === result.results.length && result.errorCount > 0
      ? t("secretsPage.importVault.importFailed", { defaultValue: "Import failed" })
      : result.errorCount === 0 && result.skippedCount === 0
        ? t("secretsPage.importVault.allImported", {
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-border/60 px-5 py-3" data-testid="result-summary">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="text-emerald-600 dark:text-emerald-400">
            {t("secretsPage.importVault.createdCount", {
              defaultValue: "✓ {{count}} created",
              count: result.importedCount,
            })}
          </span>
          <span>
            {t("secretsPage.importVault.skippedCount", {
              defaultValue: "⊘ {{count}} skipped",
              count: result.skippedCount,
            })}
          </span>
          <span className="text-destructive">
            {t("secretsPage.importVault.failedCount", {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {grouped.created.length > 0 && (
          <ResultGroup
            label={t("secretsPage.importVault.created", { defaultValue: "Created" })}
        ...
        {grouped.skipped.length > 0 && (
          <ResultGroup
            label={t("secretsPage.importVault.skipped", { defaultValue: "Skipped" })}
        ...
        {grouped.failed.length > 0 && (
          <ResultGroup
            label={t("secretsPage.importVault.failed", { defaultValue: "Failed" })}
            rows={grouped.failed}
            draftLookup={draftLookup}
          />
        )}
...
function FooterStatus({
  ...
  if (step === "select") {
    return (
      <div className="text-xs text-muted-foreground">
        {totalSelected === 0
          ? t("secretsPage.importVault.selectRemote", { defaultValue: "Select remote secrets to import" })
          : t("secretsPage.importVault.selectedCount", {
  ...
  if (step === "review") {
    return (
      <div className="text-xs text-muted-foreground">
        {t("secretsPage.importVault.readyShort", {
          defaultValue: "{{count}} ready",
          count: readyReviewCount,
        })}
        {blockedReviewCount > 0 && (
          <span className="ml-2 text-amber-600 dark:text-amber-400">
            {t("secretsPage.importVault.blockedShort", {
  ...
  if (result) {
    return (
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>
          {t("secretsPage.importVault.createdCountPlain", {
            defaultValue: "{{count}} created",
            count: result.importedCount,
          })}
        </span>
        <span>
          {t("secretsPage.importVault.skippedCountPlain", {
            defaultValue: "{{count}} skipped",
            count: result.skippedCount,
          })}
        </span>
        <span>
          {t("secretsPage.importVault.failedCountPlain", {
            defaultValue: "{{count}} failed",
            count: result.errorCount,
          })}
        </span>
