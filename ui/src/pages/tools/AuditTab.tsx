...
function plainReason(event: ToolGatewayActivityEvent): string {
  ...
  if (code === "permitted_connections_not_installed") {
    return t("toolsPage.audit.reason.permittedConnectionsMissing", { defaultValue: "Permitted connections were not installed, so their tools were not added to this run." });
  }
  switch (event.normalizedOutcome) {
    case "allowed":
      return t("toolsPage.audit.reason.allowed", { defaultValue: "Allowed by your rules." });
    case "blocked":
      if (code === "rate_limited") return t("toolsPage.audit.reason.rateLimited", { defaultValue: "Blocked because it ran too many times in a short window." });
      if (code?.includes("secret")) return t("toolsPage.audit.reason.secretBlocked", { defaultValue: "Blocked to keep a sensitive value from leaving." });
      return t("toolsPage.audit.reason.blocked", { defaultValue: "Blocked by a rule." });
    case "asked_first":
      return t("toolsPage.audit.reason.askedFirst", { defaultValue: "Held for someone to approve before it could run." });
    case "waiting":
      return t("toolsPage.audit.reason.waiting", { defaultValue: "Waiting — the app it needs wasn't ready yet." });
    case "failed":
      return t("toolsPage.audit.reason.failed", { defaultValue: "The app was allowed to run it, but returned an error." });
    default:
      return t("toolsPage.audit.reason.recorded", { defaultValue: "Recorded by Paperclip." });
  }
}
...
function OutcomeChip({ outcome }: { outcome: ToolAuditOutcome }) {
  ...
  return <StatusBadge status={meta.status} label={outcomeLabel(outcome)} />;
}

function outcomeLabel(outcome: ToolAuditOutcome | string): string {
  if (outcome === ALL) return t("toolsPage.audit.allOutcomes", { defaultValue: "All outcomes" });
  const key = outcome === "asked_first" ? "askedFirst" : outcome;
  return t(`toolsPage.audit.outcome.${key}`, { defaultValue: OUTCOME_META[outcome as ToolAuditOutcome]?.label ?? "Recorded" });
}

function windowLabel(windowKey: ToolAuditWindow): string {
  return t(`toolsPage.audit.window.${windowKey}`, { defaultValue: WINDOW_FILTERS.find((item) => item.value === windowKey)?.label ?? windowKey });
}
...
function ActivityRow({
  ...
  const who = event.agentDisplayName ?? t("toolsPage.audit.agentFallback", { defaultValue: "An agent" });
  const action = event.toolDisplayName ?? t("toolsPage.audit.actionFallback", { defaultValue: "an action" });
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          {isRuntimeMcpDeliveryDiagnostic ? (
            <span className="block text-foreground">
              {t(
                (permittedNotInstalledCount ?? permittedNotInstalledConnections.length) === 1
                  ? "toolsPage.audit.mcpDiagnosticSingular"
                  : "toolsPage.audit.mcpDiagnostic",
                {
                  defaultValue: (permittedNotInstalledCount ?? permittedNotInstalledConnections.length) === 1
            ...
            <span className="block text-foreground">
              <span className="font-medium">{who}</span> {t("toolsPage.audit.usedAction", { defaultValue: "used" })} <span className="font-medium">{action}</span>
              {app ? (
                <>
                  {" "}{t("toolsPage.audit.inApp", { defaultValue: "in" })} <span className="font-medium">{app}</span>
                </>
              ) : null}
            </span>
          )}
        ...
        <div className="space-y-3 border-t border-border bg-muted/30 px-4 py-3 pl-10 text-sm">
          ...
          <div className="flex flex-wrap gap-3 text-xs">
            {issueId ? (
              <Link to={`/issues/${issueId}`} className="text-primary hover:underline">
                {t("toolsPage.audit.viewTask", { defaultValue: "View task" })}
              </Link>
            ) : null}
            {runId && agentId ? (
              <Link to={`/agents/${agentId}/runs/${runId}`} className="text-primary hover:underline">
                {t("toolsPage.audit.viewRun", { defaultValue: "View run" })}
              </Link>
            ) : null}
          </div>
          ...
          <div>
            ...
            >
              ...
              {t("toolsPage.audit.details", { defaultValue: "Details" })}
            </button>
            {detailsOpen ? (
              <div className="mt-2 space-y-1.5 text-xs">
                {rawTool ? <DetailFact label={t("toolsPage.audit.factActionName", { defaultValue: "Action name" })} value={rawTool} mono /> : null}
                <DetailFact label={t("toolsPage.audit.factReasonCode", { defaultValue: "Reason code" })} value={reasonCode} mono />
                <DetailFact label={t("toolsPage.audit.factActorType", { defaultValue: "Actor type" })} value={event.actorType ?? "—"} />
                {runId ? <DetailFact label={t("toolsPage.audit.factRunId", { defaultValue: "Run ID" })} value={runId} mono /> : null}
                {transport ? <DetailFact label={t("toolsPage.audit.factTransport", { defaultValue: "Transport" })} value={transport} mono /> : null}
                {requestMethod && endpoint ? <DetailFact label={t("toolsPage.audit.factHttpRequest", { defaultValue: "HTTP request" })} value={`${requestMethod} ${endpoint}`} mono /> : null}
                {mcpMethod ? <DetailFact label={t("toolsPage.audit.factMcpMethod", { defaultValue: "MCP method" })} value={mcpMethod} mono /> : null}
                {requestId ? <DetailFact label={t("toolsPage.audit.factRequestId", { defaultValue: "Request ID" })} value={requestId} mono /> : null}
                {request ? <DetailFact label={t("toolsPage.audit.factDispatched", { defaultValue: "Dispatched" })} value={request.dispatched === true ? t("toolsPage.audit.yes", { defaultValue: "Yes" }) : t("toolsPage.audit.no", { defaultValue: "No" })} /> : null}
                {httpStatus !== undefined ? <DetailFact label={t("toolsPage.audit.factHttpStatus", { defaultValue: "HTTP status" })} value={String(httpStatus)} mono /> : null}
                {contentType ? <DetailFact label={t("toolsPage.audit.factContentType", { defaultValue: "Content type" })} value={contentType} mono /> : null}
                {responseBytes !== undefined ? <DetailFact label={t("toolsPage.audit.factResponseSize", { defaultValue: "Response size" })} value={t("toolsPage.audit.bytes", { defaultValue: "{{count}} bytes", count: responseBytes })} /> : null}
                {upstreamRequestId ? <DetailFact label={t("toolsPage.audit.factUpstreamId", { defaultValue: "Upstream ID" })} value={upstreamRequestId} mono /> : null}
                {isRuntimeMcpDeliveryDiagnostic ? (
                  <>
                    <DetailFact label={t("toolsPage.audit.factDeliveredMcpServers", { defaultValue: "Delivered MCP servers" })} value="0" mono />
                    {permittedNotInstalledConnections.map((connection) => {
                      const connectionId = detailString(connection, "id");
                      const connectionName = detailString(connection, "name") ?? t("toolsPage.audit.unnamedConnection", { defaultValue: "Unnamed connection" });
                      return connectionId ? (
                        <div key={connectionId} className="flex gap-2">
                          <span className="shrink-0 text-muted-foreground">{t("toolsPage.audit.notInstalled", { defaultValue: "Not installed" })}</span>
                ...
                {argumentsText ? (
                  <div className="space-y-1">
                    <span className="text-muted-foreground">{t("toolsPage.audit.parametersRedacted", { defaultValue: "Parameters (redacted)" })}</span>
...
export function AuditTab({ companyId }: { companyId: string }) {
  ...
  return (
    <div className="space-y-4">
      <ToolsPageHeader
        title={t("toolsPage.audit.title", { defaultValue: "Activity" })}
        description={t("toolsPage.audit.description", { defaultValue: "What your agents actually did with your apps, newest first. Each line is one decision — allowed, blocked, asked first, waiting, or failed." })}
      />

      <div className="flex flex-wrap items-center gap-2">
        <Select value={app} onValueChange={setApp}>
          <SelectTrigger className="w-40">
            <SelectValue placeholder={t("toolsPage.audit.app", { defaultValue: "App" })} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={ALL}>{t("toolsPage.audit.allApps", { defaultValue: "All apps" })}</SelectItem>
        ...
        <Select value={agent} onValueChange={setAgent}>
          <SelectTrigger className="w-40">
            <SelectValue placeholder={t("toolsPage.audit.agent", { defaultValue: "Agent" })} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={ALL}>{t("toolsPage.audit.allAgents", { defaultValue: "All agents" })}</SelectItem>
            {(agents.data ?? []).map((a) => (
              <SelectItem key={a.id} value={a.id}>
                {a.name}
              </SelectItem>
        ...
        <Input
          placeholder={t("toolsPage.audit.searchActivity", { defaultValue: "Search activity…" })}
          value={searchInput}
        ...
        {hasActiveFilters ? (
          <Button variant="ghost" size="sm" onClick={clearFilters}>
            {t("toolsPage.audit.clearFilters", { defaultValue: "Clear filters" })}
          </Button>
        ) : null}
      </div>
        ...
        hasActiveFilters ? (
          <Card>
            <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
              ...
              <div>
              <p className="text-sm font-medium text-foreground">{t("toolsPage.audit.noMatches", { defaultValue: "No activity matches these filters" })}</p>
              <p className="mt-1 max-w-md text-sm text-muted-foreground">
                  {t("toolsPage.audit.noMatchesDescription", { defaultValue: "Try a wider time window or different filters." })}
                </p>
              </div>
          ...
          <Card>
            <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
              <ScrollText className="h-10 w-10 text-muted-foreground/40" />
              <div>
                <p className="text-sm font-medium text-foreground">{t("toolsPage.audit.emptyTitle", { defaultValue: "Nothing here yet" })}</p>
                <p className="mt-1 max-w-md text-sm text-muted-foreground">
                  {t("toolsPage.audit.emptyDescription", { defaultValue: "As soon as your agents start using connected apps, what they do shows up here." })}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="flex justify-center">
          ...
          >
            {activity.isFetchingNextPage
              ? t("toolsPage.audit.loading", { defaultValue: "Loading…" })
              : t("toolsPage.audit.loadMore", { defaultValue: "Load more" })}
          </Button>
        </div>
      ) : null}

      <p className="text-xs text-muted-foreground">
        {t("toolsPage.audit.footer", { defaultValue: "Recorded by Paperclip — entries can't be edited. Sensitive values are never stored." })}
      </p>
    </div>
  );
}
