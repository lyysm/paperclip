...
function formatOwner(gateway: ToolMcpGatewayWithTokens, agentNames: Map<string, string>) {
  if (gateway.agentId) {
    return t("toolsPage.gateways.ownerAgent", {
      defaultValue: "Agent {{name}}",
      name: agentNames.get(gateway.agentId) ?? shortId(gateway.agentId) ?? "unknown",
    });
  }
  if (gateway.createdByAgentId) {
    return t("toolsPage.gateways.ownerAgent", {
      defaultValue: "Agent {{name}}",
      name: agentNames.get(gateway.createdByAgentId) ?? shortId(gateway.createdByAgentId) ?? "unknown",
    });
  }
  if (gateway.createdByUserId) {
    return t("toolsPage.gateways.ownerBoardUser", {
      defaultValue: "Board user {{id}}",
      id: shortId(gateway.createdByUserId) ?? "unknown",
    });
  }
  return t("toolsPage.gateways.ownerBoard", { defaultValue: "Board" });
}
...
function formatScope(
  ...
  if (gateway.contextScopeType !== "none" && gateway.contextScopeId) {
    const scopeId = shortId(gateway.contextScopeId) ?? "unknown";
    if (gateway.contextScopeType === "project") {
      return t("toolsPage.gateways.scopeProject", {
        defaultValue: "Project {{name}}",
        name: projectNames.get(gateway.contextScopeId) ?? scopeId,
      });
    }
    if (gateway.contextScopeType === "agent") {
      return t("toolsPage.gateways.scopeAgent", {
        defaultValue: "Agent {{name}}",
        name: agentNames.get(gateway.contextScopeId) ?? scopeId,
      });
    }
    if (gateway.contextScopeType === "issue") {
      return t("toolsPage.gateways.scopeIssue", { defaultValue: "Issue {{id}}", id: scopeId });
    }
    if (gateway.contextScopeType === "routine") {
      return t("toolsPage.gateways.scopeRoutine", { defaultValue: "Routine {{id}}", id: scopeId });
    }
    if (gateway.contextScopeType === "company") {
      return t("toolsPage.gateways.scopeCompany", { defaultValue: "Company" });
    }
    return t("toolsPage.gateways.scopeNamed", {
      defaultValue: "{{scope}} {{id}}",
      scope: gateway.contextScopeType,
      id: scopeId,
    });
  }
  if (gateway.projectId) {
    return t("toolsPage.gateways.scopeProject", {
      defaultValue: "Project {{name}}",
      name: projectNames.get(gateway.projectId) ?? shortId(gateway.projectId) ?? "unknown",
    });
  }
  if (gateway.issueId) {
    return t("toolsPage.gateways.scopeIssue", {
      defaultValue: "Issue {{id}}",
      id: shortId(gateway.issueId) ?? "unknown",
    });
  }
  if (gateway.agentId) {
    return t("toolsPage.gateways.scopeAgent", {
...
function formatAllowedTools(profile: ToolProfileWithDetails | undefined) {
  if (!profile) return t("toolsPage.gateways.profileUnavailable", { defaultValue: "Profile unavailable" });
  ...
  const count =
    profile.summary.accessMode === "all_except"
      ...
      : allowed;
  if (count === 0) return t("toolsPage.gateways.noToolsAllowed", { defaultValue: "No tools allowed" });
  return t(count === 1 ? "toolsPage.gateways.toolsAllowedSingular" : "toolsPage.gateways.toolsAllowed", {
    defaultValue: count === 1 ? "1 tool allowed" : "{{count}} tools allowed",
    count,
  });
}

function gatewayStatusLabel(status: string) {
  return t(`toolsPage.gateways.status.${status}`, { defaultValue: status });
}
...
export function GatewaysTab({ companyId }: { companyId: string }) {
  ...
  const createGatewayMutation = useMutation({
    ...
    onSuccess: async (gateway) => {
      ...
      setCreating(false);
      pushToast({ title: t("toolsPage.gateways.toast.gatewayCreated", { defaultValue: "Gateway created" }), body: gateway.name, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({
        title: t("toolsPage.gateways.toast.gatewayNotCreated", { defaultValue: "Gateway was not created" }),
  ...
  const createTokenMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({
        title: t("toolsPage.gateways.toast.tokenIssued", { defaultValue: "Token issued" }),
        body: t("toolsPage.gateways.toast.tokenIssuedBody", {
    ...
    onError: (error) => {
      pushToast({
        title: t("toolsPage.gateways.toast.tokenNotIssued", { defaultValue: "Token was not issued" }),
        body: error instanceof Error ? error.message : String(error),
        tone: "error",
      });
    },
  ...
  const revokeTokenMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({ title: t("toolsPage.gateways.toast.tokenRevoked", { defaultValue: "Token revoked" }), body: token.name, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({
        title: t("toolsPage.gateways.toast.tokenNotRevoked", { defaultValue: "Token was not revoked" }),
        body: error instanceof Error ? error.message : String(error),
        tone: "error",
      });
    },
  ...
  async function copyText(value: string, label: string) {
    try {
      ...
      pushToast({ title: t("toolsPage.gateways.toast.copied", { defaultValue: "Copied to clipboard" }), body: label, tone: "success" });
    } catch (error) {
      pushToast({
        title: t("toolsPage.gateways.toast.copyFailed", { defaultValue: "Copy failed" }),
        body: error instanceof Error
          ? error.message
          : t("toolsPage.gateways.toast.clipboardUnavailable", { defaultValue: "Clipboard access is unavailable." }),
        tone: "error",
      });
    }
  }
  ...
  function submitCreateGateway(event: FormEvent<HTMLFormElement>) {
    ...
    if (!createDraft.profileId) {
      pushToast({
        title: t("toolsPage.gateways.toast.pickProfile", { defaultValue: "Pick a profile" }),
        body: t("toolsPage.gateways.toast.pickProfileBody", { defaultValue: "A gateway needs an access profile before it can be created." }),
        tone: "warn",
      });
      return;
    }
  ...
  function submitCreateToken(event: FormEvent<HTMLFormElement>, gatewayId: string) {
    ...
    if (draft.allowedActions.length === 0) {
      pushToast({
        title: t("toolsPage.gateways.toast.pickTokenActions", { defaultValue: "Pick token actions" }),
        body: t("toolsPage.gateways.toast.pickTokenActionsBody", { defaultValue: "Gateway tokens need at least one allowed MCP action." }),
        tone: "warn",
      });
      return;
    }
  ...
  if (gatewaysQuery.isLoading) {
    return <LoadingState label={t("toolsPage.gateways.loading", { defaultValue: "Loading gateways..." })} />;
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <ToolsPageHeader
          title={t("toolsPage.gateways.title", { defaultValue: "Named MCP gateways" })}
          description={t("toolsPage.gateways.description", {
        ...
        >
          ...
          {t("toolsPage.gateways.createGateway", { defaultValue: "Create gateway" })}
        </Button>
      </div>
        ...
        <form className="space-y-3 rounded-md border border-border p-4" onSubmit={submitCreateGateway}>
          <div className="grid gap-3 md:grid-cols-(--gtc-60)">
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.gatewayName", { defaultValue: "Gateway name" })}</span>
              <input
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                value={createDraft.name}
                onChange={(event) => setCreateDraft((current) => ({ ...current, name: event.target.value }))}
                placeholder={t("toolsPage.gateways.gatewayNamePlaceholder", { defaultValue: "Engineering laptops" })}
                required
              />
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.accessProfile", { defaultValue: "Access profile" })}</span>
              <select
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              ...
              >
                <option value="" disabled>
                  {profileLoading
                    ? t("toolsPage.gateways.loadingProfiles", { defaultValue: "Loading profiles..." })
                    : t("toolsPage.gateways.chooseProfile", { defaultValue: "Choose a profile" })}
                </option>
          ...
          <label className="space-y-1.5 text-sm">
            <span className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.descriptionLabel", { defaultValue: "Description" })}</span>
            <textarea
              className="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              value={createDraft.description}
              onChange={(event) => setCreateDraft((current) => ({ ...current, description: event.target.value }))}
              placeholder={t("toolsPage.gateways.descriptionPlaceholder", { defaultValue: "Who this endpoint is for and when it should be rotated." })}
            />
          </label>
          {activeProfiles.length === 0 && !profileLoading ? (
            <p className="text-xs text-muted-foreground">{t("toolsPage.gateways.createProfileFirst", { defaultValue: "Create an access profile before adding a gateway." })}</p>
          ) : null}
          <div className="flex flex-wrap justify-end gap-2">
            <Button type="button" variant="ghost" size="sm" onClick={() => setCreating(false)}>
              {t("toolsPage.gateways.cancel", { defaultValue: "Cancel" })}
            </Button>
            <Button type="submit" size="sm" disabled={createDisabled || !createDraft.name.trim() || !createDraft.profileId}>
              {createGatewayMutation.isPending
                ? t("toolsPage.gateways.creating", { defaultValue: "Creating..." })
                : t("toolsPage.gateways.createGateway", { defaultValue: "Create gateway" })}
            </Button>
          </div>
        </form>
            ...
            return (
              <section key={gateway.id} className="space-y-4 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">{gatewayStatusLabel(gateway.status)}</span>
                    </div>
                    {gateway.description ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => void copyText(endpoint, t("toolsPage.gateways.gatewayEndpoint", { defaultValue: "Gateway endpoint" }))}
                    >
                      <Copy className="mr-1.5 h-3.5 w-3.5" />
                      {t("toolsPage.gateways.copyEndpoint", { defaultValue: "Copy endpoint" })}
                    </Button>
                    <Button type="button" variant="outline" size="sm" onClick={() => startIssuing(gateway.id)}>
                      <KeyRound className="mr-1.5 h-3.5 w-3.5" />
                      {t("toolsPage.gateways.issueToken", { defaultValue: "Issue token" })}
                    </Button>
                  </div>
                </div>
                ...
                <dl className="grid gap-x-4 gap-y-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.ownerLabel", { defaultValue: "Owner" })}</dt>
                    <dd className="mt-0.5 text-foreground">{formatOwner(gateway, agentNames)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.scopeLabel", { defaultValue: "Scope" })}</dt>
                    <dd className="mt-0.5 text-foreground">{formatScope(gateway, projectNames, agentNames)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.allowedToolsLabel", { defaultValue: "Allowed tools" })}</dt>
                    <dd className="mt-0.5 text-foreground">
                      {profile
                        ? t("toolsPage.gateways.allowedToolsViaProfile", {
                            defaultValue: "{{tools}} via {{name}}",
                            tools: formatAllowedTools(profile),
                            name: profile.name,
                          })
                        : t("toolsPage.gateways.profileId", {
                            defaultValue: "Profile {{id}}",
                            id: shortId(gateway.profileId) ?? "unknown",
                          })}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.lastActivity", { defaultValue: "Last activity" })}</dt>
                    <dd className="mt-0.5 text-foreground">
                      {lastActivity
                        ? <RelativeTime value={lastActivity} />
                        : t("toolsPage.gateways.neverUsed", { defaultValue: "Never used" })}
                    </dd>
                  </div>
                </dl>
                ...
                {issuingGatewayId === gateway.id ? (
                  <form className="space-y-3 rounded-md border border-border p-3" onSubmit={(event) => submitCreateToken(event, gateway.id)}>
                    <div className="grid gap-3 md:grid-cols-2">
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.tokenName", { defaultValue: "Token name" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("toolsPage.gateways.tokenNamePlaceholder", { defaultValue: "Dotta's MacBook" })}
                          required
                        />
                      </label>
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.clientLabel", { defaultValue: "Client label" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("toolsPage.gateways.clientLabelPlaceholder", { defaultValue: "Cursor on work laptop" })}
                          required
                        />
                      </label>
                    </div>
                    <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.ownerNote", { defaultValue: "Owner note" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("toolsPage.gateways.ownerNotePlaceholder", { defaultValue: "Who owns this token and why it exists" })}
                          required
                        />
                      </label>
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("toolsPage.gateways.expires", { defaultValue: "Expires" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          type="date"
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      >
                        {createTokenMutation.isPending
                          ? t("toolsPage.gateways.issuing", { defaultValue: "Issuing..." })
                          : t("toolsPage.gateways.issueToken", { defaultValue: "Issue token" })}
                      </Button>
                    </div>
                  </form>
                  ...
                  <div className="space-y-2 rounded-md border border-border bg-muted/40 p-3 text-sm">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="font-medium text-foreground">
                        {t("toolsPage.gateways.newTokenFor", { defaultValue: "New token for {{name}}", name: createdToken.name })}
                      </div>
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
                        onClick={() => void copyText(createdToken.token, t("toolsPage.gateways.gatewayBearerToken", { defaultValue: "Gateway bearer token" }))}
                      >
                        <Copy className="mr-1.5 h-3.5 w-3.5" />
                        {t("toolsPage.gateways.copyToken", { defaultValue: "Copy token" })}
                      </Button>
                    </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div>
                    <div className="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                      ...
                      {t("toolsPage.gateways.tokens", { defaultValue: "Tokens" })}
                    </div>
                    <div className="space-y-1 text-sm">
                      {gateway.tokens.length === 0 ? (
                        <p className="text-muted-foreground">{t("toolsPage.gateways.noTokens", { defaultValue: "No tokens issued." })}</p>
                      ) : (
                            ...
                            <div key={token.id} className="space-y-1 py-1">
                              <div className="flex items-center justify-between gap-3">
                                ...
                                <div className="flex shrink-0 items-center gap-2">
                                  <span className="text-xs text-muted-foreground">
                                    {token.revokedAt ? (
                                      <>
                                        {t("toolsPage.gateways.revoked", { defaultValue: "revoked" })} <RelativeTime value={token.revokedAt} />
                                      </>
                                    ) : token.expiresAt ? (
                                      <>
                                        {t("toolsPage.gateways.expiresIn", { defaultValue: "expires" })} <RelativeTime value={token.expiresAt} />
                                      </>
                                    ) : (
                                      t("toolsPage.gateways.noExpiry", { defaultValue: "no expiry" })
                                    )}
                                  </span>
                                  {!revoked ? (
                                    <Button
                                      ...
                                      aria-label={t("toolsPage.gateways.revokeAriaLabel", { defaultValue: "Revoke {{name}}", name: token.name })}
                                    >
                                      <RotateCcw className="mr-1 h-3.5 w-3.5" />
                                      {t("toolsPage.gateways.revoke", { defaultValue: "Revoke" })}
                                    </Button>
                                  ) : null}
                                </div>
                              </div>
                              {confirming ? (
                                <div className="flex flex-wrap items-center justify-end gap-2 text-xs text-muted-foreground">
                                  <span>{t("toolsPage.gateways.revokeConfirm", { defaultValue: "Revoke this token now?" })}</span>
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    size="sm"
                                  ...
                                  >
                                    ...
                                    {t("toolsPage.gateways.confirm", { defaultValue: "Confirm" })}
                                  </Button>
                                </div>
                              ) : null}
                            </div>
                  ...
                  <div>
                    <div className="mb-1.5 text-xs font-medium text-muted-foreground">
                      {t("toolsPage.gateways.clientSnippets", { defaultValue: "Client snippets" })}
                    </div>
                    <div className="space-y-1 text-sm">
                      {snippets.length === 0 ? (
                        <p className="text-muted-foreground">{t("toolsPage.gateways.noSnippets", { defaultValue: "No snippets available." })}</p>
                      ) : (
                              ...
                              <Button
                                ...
                                onClick={(event) => {
                                  ...
                                  void copyText(
                                    ...
                                    t("toolsPage.gateways.snippetLabel", { defaultValue: "{{label}} snippet", label: snippet.label }),
                                  );
                                }}
                              >
                                <Copy className="mr-1 h-3.5 w-3.5" />
                                {t("toolsPage.gateways.copy", { defaultValue: "Copy" })}
                              </Button>
                            </summary>
