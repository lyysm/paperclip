...
export function ToolsAccess() {
  ...
  const tabLabel = toolTabLabel(activeTab);

  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("toolsPage.access.company", { defaultValue: "Company" }), href: "/dashboard" },
      { label: t("toolsPage.access.apps", { defaultValue: "Apps" }), href: "/apps" },
      ...(advanced
        ? [{ label: t("toolsPage.access.advancedSetup", { defaultValue: "Advanced setup" }) }]
        : [
            { label: t("toolsPage.access.advancedSetup", { defaultValue: "Advanced setup" }), href: advancedTabHref("run-your-own") },
            { label: tabLabel },
          ]),
    ]);
  ...
  if (!selectedCompanyId) {
    return <div className="p-6 text-sm text-muted-foreground">{t("toolsPage.access.selectCompany", { defaultValue: "Select a company to open advanced setup." })}</div>;
  }
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-5 p-4 sm:p-6">
        <header>
          <div className="flex items-center gap-2.5">
            <h1 className="text-xl font-bold text-foreground">{t("toolsPage.access.advancedSetup", { defaultValue: "Advanced setup" })}</h1>
            <span className="inline-flex items-center rounded-full bg-foreground px-2.5 py-0.5 text-(length:--text-micro) font-bold text-background">
              {t("toolsPage.access.advanced", { defaultValue: "Advanced" })}
            </span>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("toolsPage.access.advancedDescription", {
            ...
            <Link to="/apps" className="font-medium text-primary hover:underline">
              {t("toolsPage.access.connectItThere", { defaultValue: "connect it there instead" })}
            </Link>
            .
          </p>
        </header>
        ...
        <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          {t("toolsPage.access.lookingForDeveloper", { defaultValue: "Looking for the developer surface?" })}{" "}
          <Link to={advancedTabHref("profiles")} className="font-medium text-primary hover:underline">
            {t("toolsPage.access.openDeveloperTools", { defaultValue: "Open developer tools" })}
          </Link>
        </p>
      </div>
    );
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 p-4 sm:p-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-foreground">{t("toolsPage.access.developerTools", { defaultValue: "Developer tools" })}</h1>
        </div>
        <p className="mt-1.5 max-w-2xl text-sm text-muted-foreground">
          {t("toolsPage.access.developerDescription", {
            defaultValue: "Apps is the simple way to connect tools. This Developer area is for wiring your own servers, tokens, and rules by hand — most teams never need it.",
          })}
        </p>
      </div>
